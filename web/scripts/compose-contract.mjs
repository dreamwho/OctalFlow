...
const magicProxyImage = "${DREAMYO_MAGIC_PROXY_IMAGE:-metacubex/mihomo:v1.19.30}";
const magicProxySecret = "${DREAMYO_MAGIC_PROXY_SECRET:?请在 .env 中配置至少 32 位魔法代理控制密钥}";
const magicProxyListenHosts = new Set(["0.0.0.0", "127.0.0.1"]);
const magicProxyEntrypoint = ["/usr/local/bin/dreamyo-mihomo-entrypoint.sh"];
const magicProxyEntrypointMount = "./docker/mihomo/entrypoint.sh:/usr/local/bin/dreamyo-mihomo-entrypoint.sh:ro";
const magicProxyRuntimeVolume = "dreamyo-magic-proxy-runtime";
...
const maintenanceToken = "${DREAMYO_MAINTENANCE_TOKEN:?请在 .env 中配置至少 32 位维护令牌}";
...
export function validateMihomoBootstrapContracts({ repoRoot }) {
    return ["docker/mihomo/bootstrap.yaml", "docker/mihomo/bootstrap-host.yaml"].map((file) => {
        ...
        const provider = bootstrap?.["proxy-providers"]?.["dreamyo-Subscription"];
        ...
        ensure(provider?.type === "file", "必须声明 dreamyo-Subscription file provider");
...
export function validateComposeContract(source, profile) {
    ...
    const proxyListenHost = profile.hostNetwork ? "127.0.0.1" : "0.0.0.0";
        ...
        magicProxy.healthcheck?.test?.some((value) => String(value).includes("Authorization: Bearer $$DREAMYO_MAGIC_PROXY_SECRET")),
    ...
    ensure(appEnvironment.DREAMYO_MAGIC_PROXY_CONTROLLER_URL === proxyUrls.controller, "app 的 Mihomo Controller 地址不正确");
    ensure(appEnvironment.DREAMYO_MAGIC_PROXY_SECRET === magicProxySecret, "app 未声明 Mihomo Controller 密钥");
    ...
    ensure(appEnvironment.DREAMYO_INSTALL_TOKEN === installToken, "app 未声明强制一次性安装令牌");
    ensure(!("DREAMYO_INSTALL_TOKEN" in workerEnvironment), "generation-worker 不得获得一次性安装令牌");
    ensure(appEnvironment.DREAMYO_MAINTENANCE_TOKEN === maintenanceToken, "app 未声明强制维护令牌");
    ensure(appEnvironment.DREAMYO_WORKER_TOKEN === workerToken, "app 未声明强制 Worker 令牌");
    ensure(appEnvironment.DREAMYO_GEMINIAI_API_KEY === geminiAiToken, "app 未声明强制 GeminiAI 内部密钥");
    ...
    ensure(workerEnvironment.DREAMYO_WORKER_API_ORIGIN === profile.workerOrigin, `Worker API 地址必须为 ${profile.workerOrigin}`);
    if (profile.appPort) {
        ensure(appEnvironment.PORT === profile.appPort, `app 监听端口必须为 ${profile.appPort}`);
        ensure(appEnvironment.DREAMYO_INTERNAL_ORIGIN === profile.internalOrigin, `app 内部地址必须为 ${profile.internalOrigin}`);
        ensure(appEnvironment.DREAMYO_TRUSTED_PROXY_HOPS === profile.trustedProxyHops, `app 可信代理层数必须为 ${profile.trustedProxyHops}`);
        ensure(
    ...
    ensure(appEnvironment.DREAMYO_DATABASE_PROVIDER === "postgres", "app 必须使用 PostgreSQL provider");
    ...
    ensure(!("DREAMYO_DATABASE_PROVIDER" in workerEnvironment), "generation-worker 不应直接访问数据库 provider");
    ensure(app.volumes?.includes("dreamyo-data:/app/web/.data"), "app 缺少持久数据卷挂载");
    ensure(app.volumes?.includes(appMagicProxyRuntimeMount), "app 必须挂载共享 runtime 命名卷");
    ensure(Object.hasOwn(compose?.volumes || {}, "dreamyo-data"), "缺少 dreamyo-data 顶层数据卷");
    ...
    if (profile.embeddedPostgres) {
        ...
        ensure(Object.hasOwn(compose?.volumes || {}, "dreamyo-postgres"), "内置 PostgreSQL 拓扑缺少数据库数据卷");
    } else {
        ...
        ensure(!Object.hasOwn(compose?.volumes || {}, "dreamyo-postgres"), "外部数据库拓扑不得声明无用的 PostgreSQL 数据卷");
    }
    ...
    if (profile.hostNetwork) {
        ...
        ensure(appEnvironment.DREAMYO_GEMINIAI_URL === "http://127.0.0.1:8080", "宝塔 app 必须通过回环地址访问 geminiai");
        ensure(app.network_mode === "host", "宝塔 app 必须使用 host 网络");
        ensure(worker.network_mode === "host", "宝塔 generation-worker 必须使用 host 网络");
        ensure("DREAMYO_TRUSTED_PROXY_HOPS" in appEnvironment, "宝塔拓扑缺少反向代理层数配置");
    } else {
        ...
        ensure(appEnvironment.DREAMYO_GEMINIAI_URL === "http://geminiai:8080", "app 必须通过 Compose 内网访问 geminiai");
        ensure(!app.network_mode && !worker.network_mode, "宝塔专用 host 网络不得泄漏到其他拓扑");
        ensure(!("DREAMYO_TRUSTED_PROXY_HOPS" in appEnvironment), "宝塔专用反向代理默认值不得泄漏到其他拓扑");
    }
