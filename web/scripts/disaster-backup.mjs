...
try {
    ...
    const databaseFile = "database/dreamyo.dump";
    const databasePath = path.join(recoveryPointDir, databaseFile);
    await mkdir(path.dirname(databasePath), { recursive: true });
    await runPostgresTool(process.env.DREAMYO_PG_DUMP_PATH || "pg_dump", ["--format=custom", "--compress=6", "--no-owner", "--no-privileges", "--file", databasePath], databaseUrl);
    ...
    const manifest = {
        ...
        objectStorage,
        requiredSecrets: ["DATABASE_URL", "DREAMYO_ENCRYPTION_KEY"],
    };
