...
async function handleFixtureRequest({ request, response, url, body, tasks, requests, nextTaskId, options }) {
    ...
    if (request.method === "POST" && path === "/videos/generations") {
        if (
            ...
            return sendJson(response, 415, { code: "invalid_content_type", message: "dreamyo recommended video requests must use application/json", data: null });
        const payload = jsonBody(body);
        ...
        const id = nextTaskId("dreamyo-video");
        tasks.set(id, { kind: "dreamyo-video", status: "completed" });
    ...
    const dreamyoVideoId = path.match(/^\/videos\/generations\/([^/]+)$/)?.[1];
    if (request.method === "GET" && dreamyoVideoId) {
        const id = decodeURIComponent(dreamyoVideoId);
    ...
    if (request.method === "POST" && path === "/get_voice")
        return sendJson(response, 200, { base_resp: { status_code: 0, status_msg: "success" }, system_voice: [{ voice_id: "fixture-system-voice", voice_name: "Fixture 系统音色" }], voice_cloning: [], voice_generation: [] });
...
if (import.meta.url === pathToFileURL(process.argv[1] || "").href) {
    const port = Number(process.env.DREAMYO_PROTOCOL_FIXTURE_PORT) || 4010;
    const host = process.env.DREAMYO_PROTOCOL_FIXTURE_HOST || "127.0.0.1";
    const fixture = createProtocolFixtureServer({
        imagePath: process.env.DREAMYO_PROTOCOL_FIXTURE_IMAGE,
        videoPath: process.env.DREAMYO_PROTOCOL_FIXTURE_VIDEO,
        responseDelayMs: process.env.DREAMYO_PROTOCOL_FIXTURE_DELAY_MS,
        failImage: process.env.DREAMYO_PROTOCOL_FIXTURE_FAIL_IMAGE === "1",
    });
