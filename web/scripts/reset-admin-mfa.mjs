...
async function resetPostgresProvider() {
    ...
    try {
        if (args.listAdmins) {
            listAdmins((await client.query("SELECT id, username, email, status, mfa_enabled_at FROM dreamyo_users WHERE role = 'admin' ORDER BY created_at ASC")).rows);
            return;
        }
        const selector = adminSelector();
        await client.query("BEGIN");
        const result = await client.query(`SELECT id, username, email, status, mfa_secret_ciphertext, mfa_enabled_at FROM dreamyo_users WHERE role = 'admin' AND ${selector.sql} FOR UPDATE`, selector.values);
        const user = exactAdmin(result.rows);
        if (!user.mfa_secret_ciphertext && !user.mfa_enabled_at) fail("指定管理员没有可重置的 MFA 设置。");
        await client.query("UPDATE dreamyo_users SET mfa_secret_ciphertext = NULL, mfa_enabled_at = NULL, updated_at = now() WHERE id = $1", [user.id]);
        await client.query("DELETE FROM dreamyo_sessions WHERE user_id = $1", [user.id]);
...
async function resetFileProvider() {
    const dataDir = path.resolve(args.dataDir || process.env.DREAMYO_DATA_DIR || path.join(webRoot, ".data"));
...
function postgresSslConfig() {
    if (!["1", "true", "yes", "on"].includes(process.env.DREAMYO_DATABASE_SSL?.trim().toLowerCase() || "")) return undefined;
...
function printHelp() {
    console.log(`dreamyo 管理员 MFA 恢复
