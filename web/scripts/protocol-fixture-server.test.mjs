...
describe("protocol fixture server", () => {
    ...
    it("serves a configured fixture image without changing the default contract", async () => {
        await new Promise((resolve, reject) => fixture.server.close((error) => (error ? reject(error) : resolve())));
        temporaryDirectory = await mkdtemp(path.join(tmpdir(), "dreamyo-protocol-image-"));
    ...
    it("serves the dreamyo recommended JSON video contract", async () => {
        ...
        expect(created).toMatchObject({ id: "fixture-dreamyo-video-1", task_id: "fixture-dreamyo-video-1", status: "queued" });
