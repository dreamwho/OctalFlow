...
export function validateRenderBlueprint({ repoRoot, source, dockerfile: dockerfileSource }) {
    ...
    const web = services.find((service) => service?.name === "octalaicanvas");
    const worker = services.find((service) => service?.name === "octalaicanvas-generation-worker");
    const database = databases.find((entry) => entry?.name === "octalaicanvas-postgres");
    const runtimeGroup = environmentGroups.find((group) => group?.name === "octalaicanvas-runtime");
    ...
    ensure(hasGroup(web?.envVars, "octalaicanvas-runtime") && hasGroup(worker?.envVars, "octalaicanvas-runtime"), "Web 与 Worker 必须引用同一运行时环境组");
    ensure(environmentMap(runtimeGroup?.envVars).OCTALAICANVAS_WORKER_TOKEN?.generateValue === true, "运行时环境组必须生成共享 Worker 令牌");
    ensure(webEnvironment.OCTALAICANVAS_DATA_DIR?.value === "/app/web/.data", "Web 数据目录必须位于持久盘");
    ensure(webEnvironment.DATABASE_URL?.fromDatabase?.name === "octalaicanvas-postgres", "Web 必须引用 Blueprint PostgreSQL");
    ensure(webEnvironment.OCTALAICANVAS_ENCRYPTION_KEY?.generateValue === true, "Web 必须生成稳定加密密钥");
    ensure(webEnvironment.OCTALAICANVAS_INSTALL_TOKEN?.generateValue === true, "Web 必须生成一次性安装令牌");
    ensure(webEnvironment.OCTALAICANVAS_MAINTENANCE_TOKEN?.generateValue === true, "Web 必须生成独立维护令牌");
    ensure(!workerEnvironment.OCTALAICANVAS_INSTALL_TOKEN, "Render Worker 不得获得一次性安装令牌");
    ensure(!workerEnvironment.OCTALAICANVAS_MAINTENANCE_TOKEN, "Render Worker 不得获得外部维护令牌");
    ensure(workerEnvironment.OCTALAICANVAS_WORKER_API_ORIGIN?.fromService?.name === "octalaicanvas" && workerEnvironment.OCTALAICANVAS_WORKER_API_ORIGIN?.fromService?.property === "hostport", "Worker 必须通过 Render 私网调用 Web");
    ensure(!workerEnvironment.DATABASE_URL && !workerEnvironment.OCTALAICANVAS_DATABASE_PROVIDER, "Render Worker 不应直接持有数据库配置");
    ensure(database?.plan === "basic-256mb" && database?.databaseName === "octalaicanvas" && database?.user === "octalaicanvas", "Render PostgreSQL 必须使用持久生产实例和稳定名称");
