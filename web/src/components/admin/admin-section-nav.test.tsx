...
describe("admin navigation order", () => {
    it("keeps the existing business classifications and generation operations entry", () => {
        expect(adminSectionGroups.map((group) => group.title)).toEqual(["经营分析", "商品运营", "财务管理", "上游配置", "系统管理", "内容运营"]);
        expect(adminSectionGroups.find((group) => group.title === "经营分析")?.items.map((item) => item.label)).toContain("生成运维");
        expect(adminSectionGroups.find((group) => group.title === "商品运营")?.items.map((item) => item.label)).toEqual(["套餐管理", "订单管理", "促销活动", "优惠券", "邀请奖励"]);
        expect(adminSectionGroups.find((group) => group.title === "系统管理")?.items.map((item) => item.label)).toContain("本地媒体");
        expect(adminSectionGroups.find((group) => group.title === "财务管理")?.items.map((item) => item.label)).toEqual(["积分规则", "支付渠道", "CDK 兑换", "财务流水"]);
        expect(adminSectionGroups.find((group) => group.title === "上游配置")?.items.map((item) => item.label)).toEqual(["模型渠道", "魔法代理", "通用代理", "RunningHub", "GeminiAIStudio", "GeminiTools", "GPTAPI", "MiniMax 音频", "TokenHub", "百炼语音", "即梦 CLI", "Agent Skills"]);
