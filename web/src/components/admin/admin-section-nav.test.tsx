...
describe("admin navigation order", () => {
    it("keeps the existing business classifications and generation operations entry", () => {
        ...
        expect(adminSectionGroups.find((group) => group.title === "上游配置")?.items.map((item) => item.label)).toEqual(["模型渠道", "魔法代理", "RunningHub", "GeminiAIStudio", "GeminiTools", "GPTAPI", "IPWO 配置", "即梦 CLI", "Agent Skills"]);
