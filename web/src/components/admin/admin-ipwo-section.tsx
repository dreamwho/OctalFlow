...
import { ChatGptProxyRuntimeControl } from "@/app/admin/chatgpt-api/components/chatgpt-proxy-runtime-control";
import { ChatGptRequestLogPanel } from "@/app/admin/chatgpt-api/components/chatgpt-request-log-panel";
import { useChatGptProxyRuntime } from "@/app/admin/chatgpt-api/components/use-chatgpt-proxy-runtime";
...
export function AdminIpwoSection() {
    ...
    return (
        <div className="min-w-0 space-y-4">
            <Alert type="info" showIcon title="IPWO 代理来源" description="通过 IPWO API 按次提取代理 IP，无需维护固定节点；使用前请在供应商控制台配置服务器公网 IP 白名单。IPWO 是 GPTAPI 代理管理里的原生来源之一，保存配置不会自动启用代理。" />

            <Panel>
                <PanelHeader title="使用状态" description="控制 GPTAPI 是否把 IPWO 作为原生代理来源；与手动代理、魔法代理互斥，同一时间只会使用一种来源。" />
                <div className="space-y-3 p-3 sm:p-5" data-admin-ipwo-runtime>
                    <ChatGptProxyRuntimeControl controller={proxyRuntime} target="ipwo" />
                </div>
            </Panel>
            ...
            <Panel>
                <PanelHeader
                    title="来源配置"
                    description="使用 API 提取链接，不使用网站登录密码；链接加密保存、不回显，留空保留已保存值。"
                    actions={
                ...
                />
                <div className="space-y-4 p-3 sm:p-5" data-admin-ipwo-config>
