...
describe("media image urls", () => {
    it("builds bounded WebP previews while preserving signed query values", () => {
        ...
        expect(imagePreviewUrl("/api/admin/object-storage/files/preview?key=octalaicanvas%2Ffile.png", 256)).toBe("/api/admin/object-storage/files/preview?key=octalaicanvas%2Ffile.png&format=webp&width=256");
    });
