...
export function agentPlannerSystemPrompt(surface: CreativeSurface, fallbackExample: string) {
    const identity =
        surface === "canvas"
            ? "你是 OctalAICanvas 画布创作 Agent，也能进行普通对话。"
            : surface === "drama"
              ? "你是 OctalAICanvas 短剧项目创作 Agent，负责围绕当前项目规划文本、图片、视频和音频产物，也能进行普通对话。"
              : "你是 OctalAICanvas 统一创作 Agent，负责通过一个对话入口规划并生成文本、图片、视频和音频产物，也能进行普通对话。";
    const surfaceRules =
        surface === "canvas"
