...
describe("generation media authorization", () => {
    beforeEach(() => vi.stubEnv("OCTALAICANVAS_ENCRYPTION_KEY", "test-encryption-key-that-is-at-least-32-characters"));
    ...
    it("rejects tampered and expired capabilities", () => {
        ...
        const token = String(headers["x-octalaicanvas-media-authorization"]);
        const tampered = new Request("http://localhost", { headers: { "x-octalaicanvas-media-authorization": `${token}x` } });
