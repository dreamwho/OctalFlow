...
describe("generation media authorization", () => {
    beforeEach(() => vi.stubEnv("DREAMYO_ENCRYPTION_KEY", "test-encryption-key-that-is-at-least-32-characters"));
    ...
    it("rejects tampered and expired capabilities", () => {
        ...
        const token = String(headers["x-dreamyo-media-authorization"]);
        const tampered = new Request("http://localhost", { headers: { "x-dreamyo-media-authorization": `${token}x` } });
