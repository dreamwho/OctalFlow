...
describe("payment webhook processing", () => {
    beforeEach(() => {
        ...
        mocks.getPaymentConfig.mockResolvedValue({
            ...
            valuesByEnvName: { DREAMYO_PAYPLY_WEBHOOK_SECRET: webhookSecret },
    ...
    it("records and rejects an invalid signature before claiming the event", async () => {
        const headers = new Headers({ "x-dreamyo-signature": "invalid" });
...
function signedHeaders(body: string) {
    return new Headers({ "x-dreamyo-signature": createHmac("sha256", webhookSecret).update(body).digest("hex") });
}
