    ...
    it("sends real images to the trusted logical model route", async () => {
        fetchInternalApi.mockResolvedValueOnce(
            new Response(
                ...
                { status: 200, headers: { "Content-Type": "application/json", "x-octalaicanvas-points-cost": "2" } },
            ),
        );
        ...
        expect(headers.get("x-octalaicanvas-points-idempotency-key")).toMatch(/^creative-review:[a-f0-9]{32}$/);
    });
    ...
    it("refunds an invalid structured review and preserves the result as unavailable", async () => {
        fetchInternalApi.mockResolvedValueOnce(
            new Response(JSON.stringify({ output: [{ type: "function_call", name: "review_creative_outputs", arguments: JSON.stringify({ status: "passed" }) }] }), {
                status: 200,
                headers: { "Content-Type": "application/json", "x-octalaicanvas-points-cost": "3", "x-octalaicanvas-points-record-id": "points-review-3" },
            }),
        );
    ...
    it("refunds malformed review JSON", async () => {
        fetchInternalApi.mockResolvedValueOnce(
            new Response(JSON.stringify({ output: [{ type: "function_call", name: "review_creative_outputs", arguments: "{" }] }), {
                status: 200,
                headers: { "Content-Type": "application/json", "x-octalaicanvas-points-cost": "0", "x-octalaicanvas-points-record-id": "points-review-free" },
            }),
        );
