...
export function encryptSecretValue(value: string) {
    ...
    if (!key) throw new Error("DREAMYO_ENCRYPTION_KEY 未配置或格式无效，不能保存敏感配置");
...
export function decryptSecretValue(value: string) {
    ...
    try {
        ...
        throw new Error("敏感配置解密失败，请检查 DREAMYO_ENCRYPTION_KEY 是否与加密时一致");
    }
}

export function getEncryptionKeyStatus() {
    const raw = process.env.DREAMYO_ENCRYPTION_KEY?.trim() || "";
    if (!raw) return { ready: false, message: "缺少 DREAMYO_ENCRYPTION_KEY。" };
    if (isPlaceholderKey(raw)) return { ready: false, message: "DREAMYO_ENCRYPTION_KEY 仍是示例占位值。" };
    if (!parseConfiguredKey(raw)) return { ready: false, message: "DREAMYO_ENCRYPTION_KEY 必须是 64 位十六进制或 32 字节 Base64。" };
