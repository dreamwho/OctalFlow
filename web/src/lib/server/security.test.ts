...
describe("checkRateLimit", () => {
    ...
    it("only trusts the configured number of proxy hops", () => {
        const previous = process.env.OCTALAICANVAS_TRUSTED_PROXY_HOPS;
        ...
        process.env.OCTALAICANVAS_TRUSTED_PROXY_HOPS = "2";
        ...
        if (previous === undefined) delete process.env.OCTALAICANVAS_TRUSTED_PROXY_HOPS;
        else process.env.OCTALAICANVAS_TRUSTED_PROXY_HOPS = previous;
    });
    ...
    it("allows private upstreams only through an explicit exact host allowlist", async () => {
        vi.stubEnv("OCTALAICANVAS_ALLOW_PRIVATE_UPSTREAMS", "");
        vi.stubEnv("OCTALAICANVAS_PRIVATE_UPSTREAM_HOSTS", "");
        ...
        vi.stubEnv("OCTALAICANVAS_PRIVATE_UPSTREAM_HOSTS", "127.0.0.1, provider.internal");
