...
const mocks = vi.hoisted(() => ({
    ...
    connectionString: "postgres://dreamyo:test@localhost:5432/dreamyo",
...
vi.mock("@/lib/auth/store", () => ({
    DEFAULT_SITE_SETTINGS: { title: "dreamyo", logoUrl: "/logo.svg" },
...
describe("install status cache", () => {
    beforeEach(() => {
        vi.stubEnv("DREAMYO_INSTALL_TOKEN", "install-token-".padEnd(48, "x"));
        invalidateInstallStatusCache();
        mocks.provider = "postgres";
        mocks.connectionString = "postgres://dreamyo:test@localhost:5432/dreamyo";
    ...
    it("runs schema DDL only through the explicit initializer", async () => {
        ...
        await expect(initializeInstallDatabase(process.env.DREAMYO_INSTALL_TOKEN)).resolves.toMatchObject({ firstAdminRequired: true, database: { schemaReady: true } });
    ...
    it("rejects repeated initialization after the first user exists", async () => {
        ...
        await expect(initializeInstallDatabase(process.env.DREAMYO_INSTALL_TOKEN)).rejects.toEqual(
