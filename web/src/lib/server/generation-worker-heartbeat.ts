...
const runtime = globalThis as typeof globalThis & { __octalaicanvasProGenerationWorkerHeartbeats?: Map<string, number> };
const fileHeartbeats = (runtime.__octalaicanvasProGenerationWorkerHeartbeats ??= new Map<string, number>());
...
export async function getGenerationWorkerHealth(now = Date.now()) {
    const staleAfterMs = normalizeGenerationWorkerStaleMs(process.env.OCTALAICANVAS_GENERATION_WORKER_STALE_MS);
