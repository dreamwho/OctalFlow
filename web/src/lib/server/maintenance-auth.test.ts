...
describe("maintenance and worker authentication", () => {
    beforeEach(() => {
        vi.stubEnv("DREAMYO_MAINTENANCE_TOKEN", maintenanceToken);
        vi.stubEnv("DREAMYO_WORKER_TOKEN", workerToken);
    });
    ...
    it("rejects deployments that reuse the maintenance token for the Worker", () => {
        vi.stubEnv("DREAMYO_WORKER_TOKEN", maintenanceToken);
    ...
    it("round-trips a signed per-user Worker context and rejects tampering", () => {
        ...
        expect(headers).toEqual({ authorization: `Bearer ${workerToken}`, "x-dreamyo-worker-user-id": "user-one" });
        expect(maintenanceWorkerContextHeaders(`${context}tampered`)).toBeNull();
    });
});

function request(token: string, userId: string) {
    return new Request("http://localhost", { headers: { authorization: `Bearer ${token}`, "x-dreamyo-worker-user-id": userId } });
}
