...
async function createStripeCheckout(order: BillingOrderRecord, options: CreatePaymentCheckoutOptions, paymentConfig: PaymentRuntimeConfig): Promise<PaymentCheckoutResult> {
    const secretKey = requiredConfig(paymentConfig, "DREAMYO_STRIPE_SECRET_KEY", "STRIPE_SECRET_KEY");
    const origin = resolveOrigin(options.origin);
    const successUrl = getPaymentRuntimeEnv(paymentConfig, "DREAMYO_STRIPE_SUCCESS_URL") || `${origin}/billing/success?orderId=${encodeURIComponent(order.id)}&session_id={CHECKOUT_SESSION_ID}`;
    const cancelUrl = getPaymentRuntimeEnv(paymentConfig, "DREAMYO_STRIPE_CANCEL_URL") || `${origin}/billing/cancel?orderId=${encodeURIComponent(order.id)}`;
    ...
    params.set("metadata[dreamyoProOrderId]", order.id);
    params.set("metadata[dreamyoProOrderNo]", order.orderNo);
    params.set("payment_intent_data[metadata][orderId]", order.id);
    params.set("payment_intent_data[metadata][orderNo]", order.orderNo);
    params.set("payment_intent_data[metadata][dreamyoProOrderId]", order.id);
    params.set("payment_intent_data[metadata][dreamyoProOrderNo]", order.orderNo);
    ...
    const response = await fetchSafeOutbound(`${stripeApiBase(paymentConfig)}/v1/checkout/sessions`, {
        ...
        headers: {
            ...
            "Idempotency-Key": `dreamyo-checkout-${order.id}`,
        },
        body: params,
...
async function createAlipayCheckout(order: BillingOrderRecord, options: CreatePaymentCheckoutOptions, paymentConfig: PaymentRuntimeConfig): Promise<PaymentCheckoutResult> {
    ...
    const appId = requiredConfig(paymentConfig, "DREAMYO_ALIPAY_APP_ID");
    const privateKey = loadPrivateKey(paymentConfig, "DREAMYO_ALIPAY_PRIVATE_KEY", "DREAMYO_ALIPAY_PRIVATE_KEY_PATH");
    const origin = resolveOrigin(options.origin);
    const gateway = getPaymentRuntimeEnv(paymentConfig, "DREAMYO_ALIPAY_GATEWAY_URL") || "https://openapi.alipay.com/gateway.do";
    const modeValue = getPaymentRuntimeEnv(paymentConfig, "DREAMYO_ALIPAY_MODE") || DEFAULT_ALIPAY_PAYMENT_MODE;
    ...
    const params: Record<string, string> = {
        ...
        version: "1.0",
        notify_url: getPaymentRuntimeEnv(paymentConfig, "DREAMYO_ALIPAY_NOTIFY_URL") || `${origin}/api/billing/webhooks/alipay`,
    ...
    if (modeValue === "official") params.return_url = getPaymentRuntimeEnv(paymentConfig, "DREAMYO_ALIPAY_RETURN_URL") || `${origin}/billing/success?orderId=${encodeURIComponent(order.id)}`;
...
async function createAlipayFaceToFaceCheckout(gateway: string, params: Record<string, string>, order: BillingOrderRecord, paymentConfig: PaymentRuntimeConfig): Promise<PaymentCheckoutResult> {
    ...
    const publicKey = loadPaymentPublicKey(paymentConfig, "DREAMYO_ALIPAY_PUBLIC_KEY", "DREAMYO_ALIPAY_PUBLIC_KEY_PATH");
...
async function createWechatNativeCheckout(order: BillingOrderRecord, options: CreatePaymentCheckoutOptions, paymentConfig: PaymentRuntimeConfig): Promise<PaymentCheckoutResult> {
    const appid = requiredConfig(paymentConfig, "DREAMYO_WECHAT_PAY_APP_ID");
    const mchid = requiredConfig(paymentConfig, "DREAMYO_WECHAT_PAY_MCH_ID");
    const serialNo = requiredConfig(paymentConfig, "DREAMYO_WECHAT_PAY_CERT_SERIAL_NO");
    const privateKey = loadPrivateKey(paymentConfig, "DREAMYO_WECHAT_PAY_PRIVATE_KEY", "DREAMYO_WECHAT_PAY_PRIVATE_KEY_PATH");
    const origin = resolveOrigin(options.origin);
    const body = JSON.stringify({
        ...
        notify_url: getPaymentRuntimeEnv(paymentConfig, "DREAMYO_WECHAT_PAY_NOTIFY_URL") || `${origin}/api/billing/webhooks/wechat`,
        amount: {
            total: order.amountCents,
            currency: order.currency,
        },
    });
    const apiBase = (getPaymentRuntimeEnv(paymentConfig, "DREAMYO_WECHAT_PAY_API_BASE") || "https://api.mch.weixin.qq.com").replace(/\/+$/, "");
...
async function createPayplyCheckout(order: BillingOrderRecord, options: CreatePaymentCheckoutOptions, paymentConfig: PaymentRuntimeConfig): Promise<PaymentCheckoutResult> {
    const checkoutUrl = requiredConfig(paymentConfig, "DREAMYO_PAYPLY_CHECKOUT_URL", "PAYPLY_CHECKOUT_URL");
    const apiKey = requiredConfig(paymentConfig, "DREAMYO_PAYPLY_API_KEY", "PAYPLY_API_KEY");
    const origin = resolveOrigin(options.origin);
    const notifyUrl = getPaymentRuntimeEnv(paymentConfig, "DREAMYO_PAYPLY_NOTIFY_URL") || `${origin}/api/billing/webhooks/payply`;
    const returnUrl = getPaymentRuntimeEnv(paymentConfig, "DREAMYO_PAYPLY_RETURN_URL") || `${origin}/billing/success?orderId=${encodeURIComponent(order.id)}`;
    const cancelUrl = getPaymentRuntimeEnv(paymentConfig, "DREAMYO_PAYPLY_CANCEL_URL") || `${origin}/billing/cancel?orderId=${encodeURIComponent(order.id)}`;
    const body = buildPayplyCheckoutPayload(paymentConfig, order, {
        merchantId: getPaymentRuntimeEnv(paymentConfig, "DREAMYO_PAYPLY_MERCHANT_ID") || undefined,
        orderId: order.id,
        orderNo: order.orderNo,
        subject: order.subject,
        ...
        cancelUrl,
        metadata: {
            dreamyoProOrderId: order.id,
            dreamyoProOrderNo: order.orderNo,
    ...
    const forcedKind = normalizeCheckoutKind(getPaymentRuntimeEnv(paymentConfig, "DREAMYO_PAYPLY_RESULT_KIND"));
    const form = normalizePaymentForm(readConfiguredPath(paymentConfig, payload, "DREAMYO_PAYPLY_FORM_FIELD", ["form", "formHtml", "html", "data.form", "data.formHtml", "data.html", "result.form", "result.formHtml"]), checkoutUrl);
    const qrContent = normalizeOptionalText(readConfiguredPath(paymentConfig, payload, "DREAMYO_PAYPLY_QR_FIELD", ["qrContent", "qrCode", "qrcode", "codeUrl", "code_url", "data.qrContent", "data.qrCode", "data.codeUrl", "result.qrContent"]), 4000);
    const url = normalizeOptionalText(
        readConfiguredPath(paymentConfig, payload, "DREAMYO_PAYPLY_URL_FIELD", ["url", "paymentUrl", "payment_url", "checkoutUrl", "checkout_url", "payUrl", "pay_url", "data.url", "data.paymentUrl", "data.payUrl", "result.url"]),
        2000,
    );
    ...
    return {
        ...
        qrContent,
        providerOrderId:
            normalizeOptionalText(readConfiguredPath(paymentConfig, payload, "DREAMYO_PAYPLY_PROVIDER_ORDER_ID_FIELD", ["providerOrderId", "tradeId", "trade_no", "id", "data.providerOrderId", "data.tradeId", "data.id", "result.id"]), 160) ||
            order.orderNo,
        providerPaymentId: normalizeOptionalText(
            readConfiguredPath(paymentConfig, payload, "DREAMYO_PAYPLY_PROVIDER_PAYMENT_ID_FIELD", ["providerPaymentId", "paymentId", "payment_id", "transactionId", "data.providerPaymentId", "data.paymentId", "data.transactionId"]),
            160,
        ),
        expiresAt: normalizeOptionalIso(readConfiguredPath(paymentConfig, payload, "DREAMYO_PAYPLY_EXPIRES_AT_FIELD", ["expiresAt", "expireAt", "expiredAt", "data.expiresAt", "data.expireAt"])) || order.expiresAt,
    };
}
...
function stripeApiBase(config: PaymentRuntimeConfig) {
    return (getPaymentRuntimeEnv(config, "DREAMYO_STRIPE_API_BASE") || "https://api.stripe.com").replace(/\/+$/, "");
}

function stripePaymentMethods(config: PaymentRuntimeConfig) {
    return getPaymentRuntimeEnv(config, "DREAMYO_STRIPE_PAYMENT_METHOD_TYPES")
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean);
}
...
function buildPayplyCheckoutPayload(config: PaymentRuntimeConfig, order: BillingOrderRecord, defaultPayload: Record<string, unknown>) {
    const template = getPaymentRuntimeEnv(config, "DREAMYO_PAYPLY_REQUEST_TEMPLATE") || getPaymentRuntimeEnv(config, "PAYPLY_REQUEST_TEMPLATE");
...
function payplyRequestMethod(config: PaymentRuntimeConfig) {
    const method = (getPaymentRuntimeEnv(config, "DREAMYO_PAYPLY_REQUEST_METHOD") || getPaymentRuntimeEnv(config, "PAYPLY_REQUEST_METHOD") || "POST").toUpperCase();
    return method === "GET" ? "GET" : "POST";
}

function payplyContentType(config: PaymentRuntimeConfig) {
    const contentType = getPaymentRuntimeEnv(config, "DREAMYO_PAYPLY_CONTENT_TYPE") || getPaymentRuntimeEnv(config, "PAYPLY_CONTENT_TYPE") || "application/json";
...
function payplyHeaders(config: PaymentRuntimeConfig, apiKey: string, contentType: string) {
    const customHeader = getPaymentRuntimeEnv(config, "DREAMYO_PAYPLY_API_KEY_HEADER");
...
function parsePayplyExtraHeaders(config: PaymentRuntimeConfig): Record<string, string> {
    const text = getPaymentRuntimeEnv(config, "DREAMYO_PAYPLY_EXTRA_HEADERS") || getPaymentRuntimeEnv(config, "PAYPLY_EXTRA_HEADERS");
    if (!text) return {};
    try {
