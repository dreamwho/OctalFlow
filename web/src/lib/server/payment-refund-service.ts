...
async function refundStripePayment(order: BillingOrderRecord, payment: PaymentTransactionRecord, options: PaymentRefundOptions, paymentConfig: PaymentRuntimeConfig): Promise<PaymentRefundResult> {
    ...
    const response = await fetchSafeOutbound(`${stripeApiBase(paymentConfig)}/v1/refunds`, {
        ...
        headers: {
            ...
            "Idempotency-Key": `octalaicanvas-refund-${order.id}`,
        },
        body: params,
    });
...
async function refundWechatPayment(order: BillingOrderRecord, payment: PaymentTransactionRecord, options: PaymentRefundOptions, paymentConfig: PaymentRuntimeConfig): Promise<PaymentRefundResult> {
    ...
    const notifyUrl = getPaymentRuntimeEnv(paymentConfig, "OCTALAICANVAS_WECHAT_PAY_REFUND_NOTIFY_URL");
...
async function refundPayplyPayment(order: BillingOrderRecord, payment: PaymentTransactionRecord, options: PaymentRefundOptions, paymentConfig: PaymentRuntimeConfig): Promise<PaymentRefundResult> {
    const refundUrl = getPaymentRuntimeValue(paymentConfig, "OCTALAICANVAS_PAYPLY_REFUND_URL", "PAYPLY_REFUND_URL");
    ...
    const providerStatus = normalizeOptionalText(readConfiguredPath(paymentConfig, payload, "OCTALAICANVAS_PAYPLY_REFUND_STATUS_FIELD", ["refundStatus", "refund_status", "status", "data.refundStatus", "data.status", "result.status"]), 80);
    ...
    return {
        provider: "payply",
        status,
        providerRefundId: normalizeOptionalText(readConfiguredPath(paymentConfig, payload, "OCTALAICANVAS_PAYPLY_REFUND_ID_FIELD", ["refundId", "refund_id", "id", "data.refundId", "data.refund_id", "data.id", "result.id"]), 160),
        rawPayload: sanitizeJson(payload),
    };
}

async function queryStripeRefund(refundId: string, config: PaymentRuntimeConfig): Promise<PaymentRefundResult> {
    const secretKey = requiredConfig(config, "OCTALAICANVAS_STRIPE_SECRET_KEY", "STRIPE_SECRET_KEY");
...
async function queryPayplyRefund(order: BillingOrderRecord, payment: PaymentTransactionRecord, current: PaymentRefundResult, config: PaymentRuntimeConfig): Promise<PaymentRefundResult | null> {
    const template = getPaymentRuntimeValue(config, "OCTALAICANVAS_PAYPLY_REFUND_QUERY_URL", "PAYPLY_REFUND_QUERY_URL");
    ...
    const providerStatus = normalizeOptionalText(readConfiguredPath(config, payload, "OCTALAICANVAS_PAYPLY_REFUND_QUERY_STATUS_FIELD", ["refundStatus", "refund_status", "status", "data.status"]), 80);
    ...
    return {
        provider: "payply",
        status,
        providerRefundId: normalizeOptionalText(readConfiguredPath(config, payload, "OCTALAICANVAS_PAYPLY_REFUND_QUERY_ID_FIELD", ["refundId", "refund_id", "id", "data.refundId", "data.id"]), 160) || current.providerRefundId,
        rawPayload: sanitizeJson(payload),
    };
}
...
function providerRefundRequestNo(order: BillingOrderRecord) {
    ...
    return `octalaicanvas-refund-${id || order.orderNo}`.slice(0, 64);
}
...
function buildPayplyRefundPayload(paymentConfig: PaymentRuntimeConfig, order: BillingOrderRecord, payment: PaymentTransactionRecord, options: PaymentRefundOptions) {
    ...
    const template = getPaymentRuntimeValue(paymentConfig, "OCTALAICANVAS_PAYPLY_REFUND_REQUEST_TEMPLATE", "PAYPLY_REFUND_REQUEST_TEMPLATE");
...
function payplyHeaders(config: PaymentRuntimeConfig, apiKey: string, contentType: string, idempotencyKey: string) {
    ...
    return { ...headers, ...parseExtraHeaders(config, "OCTALAICANVAS_PAYPLY_EXTRA_HEADERS", "PAYPLY_EXTRA_HEADERS"), ...parseExtraHeaders(config, "OCTALAICANVAS_PAYPLY_REFUND_EXTRA_HEADERS", "PAYPLY_REFUND_EXTRA_HEADERS") };
}
...
function normalizePayplyRefundStatus(value: string | undefined, paymentConfig: PaymentRuntimeConfig): Exclude<PaymentRefundStatus, "manual"> | undefined {
    ...
    const configured = getPaymentRuntimeEnv(paymentConfig, "OCTALAICANVAS_PAYPLY_REFUND_SUCCESS_STATUSES");
...
function verifyWechatResponse(rawBody: string, headers: Headers, config: PaymentRuntimeConfig) {
    ...
    const publicKey = loadPaymentPublicKey(config, "OCTALAICANVAS_WECHAT_PAY_PLATFORM_PUBLIC_KEY", "OCTALAICANVAS_WECHAT_PAY_PLATFORM_PUBLIC_KEY_PATH");
...
function payplyRefundRequestMethod(config: PaymentRuntimeConfig) {
    const method = getPaymentRuntimeValue(config, "OCTALAICANVAS_PAYPLY_REFUND_REQUEST_METHOD", "PAYPLY_REFUND_REQUEST_METHOD").toUpperCase();
    return method === "GET" ? "GET" : "POST";
}

function payplyRefundContentType(config: PaymentRuntimeConfig) {
    const contentType = getPaymentRuntimeValue(config, "OCTALAICANVAS_PAYPLY_REFUND_CONTENT_TYPE", "PAYPLY_REFUND_CONTENT_TYPE", "OCTALAICANVAS_PAYPLY_CONTENT_TYPE", "PAYPLY_CONTENT_TYPE") || "application/json";
