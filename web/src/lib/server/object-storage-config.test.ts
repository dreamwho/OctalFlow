...
describe("object storage configuration", () => {
    ...
    it("rejects unsafe endpoints and incomplete enabled configurations", async () => {
        await expect(saveObjectStorageAdminSettings({ enabled: false, endpoint: "ftp://oss.example.com", region: "auto", bucket: "media", prefix: "dreamyo", forcePathStyle: false })).rejects.toThrow("Endpoint");
        mocks.read.mockResolvedValue({ ...storedSettings, accessKeyIdCiphertext: "", secretAccessKeyCiphertext: "" });
        await expect(saveObjectStorageAdminSettings({ enabled: true, endpoint: "", region: "auto", bucket: "media", prefix: "dreamyo", forcePathStyle: false })).rejects.toThrow("Access Key");
    });
