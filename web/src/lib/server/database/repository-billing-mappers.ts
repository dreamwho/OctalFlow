...
export function mapBillingOrder(row: Record<string, unknown>): BillingOrderRecord {
    return {
        ...
        storageBytes: numberValue(row.storage_bytes),
        storageStackable: row.storage_stackable !== false,
        storageRenewable: row.storage_renewable !== false,
        storagePurchaseLimit: numberValue(row.storage_purchase_limit),
