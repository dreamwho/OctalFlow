...
describe("PostgreSQL schema lifecycle", () => {
    beforeEach(() => {
        delete (globalThis as Record<string, unknown>).__dreamyoProPostgresPool;
        delete (globalThis as Record<string, unknown>).__dreamyoProPostgresSchemaReady;
        process.env.DATABASE_URL = "postgres://dreamyo:test@localhost:5432/dreamyo";
        delete process.env.DREAMYO_DATABASE_SSL;
        delete process.env.DREAMYO_DATABASE_SSL_CA;
        delete process.env.DREAMYO_DATABASE_SSL_REJECT_UNAUTHORIZED;
    ...
    it("verifies PostgreSQL TLS certificates by default and accepts an explicit CA", async () => {
        process.env.DREAMYO_DATABASE_SSL = "1";
        process.env.DREAMYO_DATABASE_SSL_CA = "-----BEGIN CERTIFICATE-----\\ncertificate\\n-----END CERTIFICATE-----";
    ...
    it("only disables PostgreSQL certificate verification through an explicit override", async () => {
        process.env.DREAMYO_DATABASE_SSL = "1";
        process.env.DREAMYO_DATABASE_SSL_REJECT_UNAUTHORIZED = "0";
        mocks.query.mockResolvedValueOnce({ rows: [{ table_name: null }] });
    ...
    it("prefixes SQL identifiers without rewriting ordinary string literals", async () => {
        ...
        expect(mocks.query).toHaveBeenCalledWith("SELECT 'users' AS target_type, $$users.read$$ AS permission FROM dreamyo_users WHERE action = 'users.read'", undefined);
    });
    ...
    it("executes schema DDL only through explicit initialization", async () => {
        ...
        expect(mocks.query.mock.calls[1]).toEqual(["SELECT pg_advisory_xact_lock(hashtext($1))", ["dreamyo:schema"]]);
        const ddl = String(mocks.query.mock.calls[2]?.[0]);
        expect(mocks.query.mock.calls[3]?.[0]).toBe("COMMIT");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_schema_migrations");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_generation_worker_heartbeats");
        expect(ddl).toContain("CREATE SEQUENCE IF NOT EXISTS dreamyo_user_account_id_seq");
        expect(ddl).toContain("account_id bigint NOT NULL DEFAULT nextval('dreamyo_user_account_id_seq')");
        expect(ddl).toMatch(/SELECT setval\(\s*'dreamyo_user_account_id_seq'/);
        expect(ddl).toContain("users.read");
        expect(ddl).toContain("users.manage");
        expect(ddl).not.toContain("dreamyo_users.read");
        expect(ddl).not.toContain("dreamyo_users.manage");
        ...
        expect(ddl).toContain("ALTER TABLE dreamyo_users ADD CONSTRAINT users_admin_permissions_array");
        expect(ddl).toContain("conname = 'dreamyo_local_media_assets_storage_provider_check'");
        expect(ddl).toContain("ADD CONSTRAINT dreamyo_local_media_assets_storage_provider_check CHECK");
        expect(ddl).toContain("CREATE UNIQUE INDEX IF NOT EXISTS dreamyo_users_account_id_idx ON dreamyo_users (account_id)");
        expect(ddl).toContain("CREATE INDEX IF NOT EXISTS dreamyo_billing_orders_provider_payment_idx ON dreamyo_billing_orders (provider, provider_payment_id)");
        expect(ddl).toContain("webhook_secret_ciphertext text NOT NULL DEFAULT ''");
        expect(ddl).toContain("CREATE UNIQUE INDEX IF NOT EXISTS dreamyo_generation_tasks_channel_upstream_idx ON dreamyo_generation_tasks (channel_id, upstream_task_id)");
        ...
        expect(ddl).toContain("user_id text NOT NULL REFERENCES dreamyo_users(id) ON DELETE CASCADE");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_account_deletion_requests");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_gemini_tools_accounts");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_gemini_tools_oauth_sessions");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_gemini_tools_api_keys");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_gemini_tools_gateway_settings");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_gemini_tools_request_logs");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_magic_proxy_settings");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_dreamina_cli_account_state");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_dreamina_cli_request_logs");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS dreamyo_runninghub_settings");
        ...
        expect(indexNames.every((name) => name.startsWith("dreamyo_"))).toBe(true);
        ...
        expect(uniqueConstraintNames.every((name) => name.startsWith("dreamyo_"))).toBe(true);
    });

    it("continues applying additive schema updates after the sentinel table exists", async () => {
        mocks.query.mockResolvedValueOnce({ rows: [{ table_name: "dreamyo_users" }] });

        await ensurePostgresSchema();

        expect(mocks.query).toHaveBeenCalledTimes(5);
        expect(mocks.query.mock.calls[0]?.[0]).toContain("to_regclass");
        expect(mocks.query.mock.calls[2]).toEqual(["SELECT pg_advisory_xact_lock(hashtext($1))", ["dreamyo:schema"]]);
        expect(mocks.query.mock.calls[3]?.[0]).toContain("CREATE TABLE IF NOT EXISTS dreamyo_schema_migrations");
    });
});
