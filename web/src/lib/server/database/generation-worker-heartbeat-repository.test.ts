...
describe("generation Worker heartbeat PostgreSQL repository", () => {
    beforeEach(() => {
        ...
        mocks.query.mockReset().mockImplementation(async (sql: string) => {
            if (sql.includes("to_regclass")) return { rows: [{ table_name: "dreamyo_users" }] };
    ...
    it("writes, prunes, and reads heartbeats through prefixed parameterized SQL", async () => {
        ...
        const insertCall = mocks.query.mock.calls.find(([sql]) => String(sql).includes("INSERT INTO dreamyo_generation_worker_heartbeats"));
        const deleteCall = mocks.query.mock.calls.find(([sql]) => String(sql).includes("DELETE FROM dreamyo_generation_worker_heartbeats"));
        const selectCall = mocks.query.mock.calls.find(([sql]) => String(sql).includes("SELECT last_seen_at FROM dreamyo_generation_worker_heartbeats"));

        expect(statements.filter((sql) => sql.includes("SELECT to_regclass('public.dreamyo_users')"))).toHaveLength(1);
        expect(statements.filter((sql) => sql.includes("CREATE TABLE IF NOT EXISTS dreamyo_schema_migrations"))).toHaveLength(1);
