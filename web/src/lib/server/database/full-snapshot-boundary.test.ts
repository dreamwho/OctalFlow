...
const originalProvider = process.env.OCTALAICANVAS_DATABASE_PROVIDER;

afterEach(() => {
    if (originalProvider === undefined) delete process.env.OCTALAICANVAS_DATABASE_PROVIDER;
    else process.env.OCTALAICANVAS_DATABASE_PROVIDER = originalProvider;
});
