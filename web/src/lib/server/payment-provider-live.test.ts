...
describe("payment providers over a live compatible HTTP fixture", () => {
    it("creates Stripe, Alipay, WeChat and PayPly checkouts", async () => {
        const stripe = await createProviderCheckout("stripe", order, { origin: "https://app.test" }, config({ OCTALAICANVAS_STRIPE_SECRET_KEY: "stripe-secret", OCTALAICANVAS_STRIPE_API_BASE: `${origin}/stripe` }));
        ...
        const payply = await createProviderCheckout("payply", { ...order, provider: "payply" }, { origin: "https://app.test" }, config({ OCTALAICANVAS_PAYPLY_API_KEY: "payply-secret", OCTALAICANVAS_PAYPLY_CHECKOUT_URL: `${origin}/payply/checkout` }));
        ...
        expect(fixture.requests[0]?.headers["idempotency-key"]).toBe("octalaicanvas-checkout-order-live");
    ...
    it("creates Stripe, Alipay, WeChat and PayPly refunds", async () => {
        setRuntimeConfig({ OCTALAICANVAS_STRIPE_SECRET_KEY: "stripe-secret", OCTALAICANVAS_STRIPE_API_BASE: `${origin}/stripe` });
        ...
        setRuntimeConfig({ OCTALAICANVAS_PAYPLY_API_KEY: "payply-secret", OCTALAICANVAS_PAYPLY_REFUND_URL: `${origin}/payply/refund` });
        ...
        expect(alipay).toMatchObject({ status: "succeeded", providerRefundId: "octalaicanvas-refund-order-live" });
        ...
        expect(fixture.requests[0]?.headers["idempotency-key"]).toBe("octalaicanvas-refund-order-live");
        expect(fixture.requests[2]?.headers.authorization).toContain('mchid="1900000001"');
        expect(fixture.requests[3]?.headers["idempotency-key"]).toBe("octalaicanvas-refund-order-live");
    });
});
...
function alipayConfig(): PaymentRuntimeConfig {
    return config({
        ...
        OCTALAICANVAS_ALIPAY_GATEWAY_URL: `${origin}/alipay/gateway.do`,
    });
}

function wechatConfig(): PaymentRuntimeConfig {
    return config({
        OCTALAICANVAS_WECHAT_PAY_APP_ID: "wx-fixture-app",
        OCTALAICANVAS_WECHAT_PAY_MCH_ID: "1900000001",
        OCTALAICANVAS_WECHAT_PAY_CERT_SERIAL_NO: "fixture-serial",
        OCTALAICANVAS_WECHAT_PAY_PRIVATE_KEY: wechatPrivateKey,
        OCTALAICANVAS_WECHAT_PAY_PLATFORM_PUBLIC_KEY: wechatPlatformPublicKey,
        OCTALAICANVAS_WECHAT_PAY_API_BASE: `${origin}/wechat`,
    });
}
