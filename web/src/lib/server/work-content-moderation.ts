...
export async function moderateWorkContent(input: ModerationInput, options: ModerationOptions = {}) {
    ...
    const provider = env.OCTALAICANVAS_CONTENT_MODERATION_PROVIDER?.trim().toLowerCase() || "manual";
    const checkedAt = (options.now || (() => new Date()))().toISOString();

    if (provider === "keywords") return keywordModeration(input, env.OCTALAICANVAS_CONTENT_MODERATION_KEYWORDS, checkedAt);
    if (provider === "http") {
        try {
...
async function httpModeration(input: ModerationInput, env: ModerationEnv, fetchImpl: typeof fetch, checkedAt: string) {
    const endpoint = new URL(env.OCTALAICANVAS_CONTENT_MODERATION_URL || "");
    if (endpoint.protocol !== "https:" && endpoint.protocol !== "http:") throw new Error("Invalid moderation endpoint");
    const response = await fetchImpl(endpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(env.OCTALAICANVAS_CONTENT_MODERATION_API_KEY ? { Authorization: `Bearer ${env.OCTALAICANVAS_CONTENT_MODERATION_API_KEY}` } : {}) },
