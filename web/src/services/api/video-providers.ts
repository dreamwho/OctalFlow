...
export async function pollOpenAIVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        ...
        if (video.status === "completed") {
            ...
            return { status: "completed", result: { blob: content.data, remoteUrl: resolveOpenAiContentRemoteUrl(config, contentUrl, readHeader(statusResponse.headers, "x-octalaicanvas-upstream-url")) } };
        }
    ...
    for (const requestUrl of requestUrls) {
        try {
            ...
            if (videoUrl && (!status || isCompletedStatus(status) || !isPendingStatus(status))) {
                const resolvedUrl = resolveVideoMediaUrl(config, videoUrl, readHeader(response.headers, "x-octalaicanvas-upstream-url") || requestUrl);
...
export async function pollSeedanceTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        ...
        if (state.status === "succeeded") {
            const url = state.content?.video_url ? resolveVideoMediaUrl(config, state.content.video_url, readHeader(response.headers, "x-octalaicanvas-upstream-url") || requestUrl) : "";
