...
export function userDataExportFileName(contentDisposition: string | null, now = new Date()) {
    ...
    let fileName = extended || regular || `octalaicanvas-personal-data-${now.toISOString().slice(0, 10)}.json`;
    try {
        fileName = decodeURIComponent(fileName);
    } catch {
        // Keep a valid server-provided ASCII filename when decoding fails.
    }
    return fileName.replace(/[\\/:*?"<>|\u0000-\u001f]/g, "-").slice(0, 180) || "octalaicanvas-personal-data.json";
}
