...
export function DatabaseConfigBuilder() {
    ...
    const [database, setDatabase] = useState("octalaicanvas");
    const [username, setUsername] = useState("octalaicanvas");
    ...
    const snippets = useMemo(
        () =>
            buildDeploymentSnippets({
                ...
                port: port.trim() || "5432",
                database: database.trim() || "octalaicanvas",
                username: username.trim() || "octalaicanvas",
                password,
                ssl,
                encryptionKey,
                installToken,
        ...
        <section className="overflow-hidden rounded-lg border border-slate-200/80 bg-white/78 shadow-sm backdrop-blur">
            ...
            <div className="p-4">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    ...
                    <Field label="Database" value={database} onChange={setDatabase} placeholder="octalaicanvas" />
                    <Field label="User" value={username} onChange={setUsername} placeholder="octalaicanvas" />
...
function buildDeploymentSteps(mode: DeployMode) {
    if (mode === "baota") {
        return [
            ...
            { title: "保存环境变量", text: "复制“环境变量”，在 /www/wwwroot/octalaicanvas 下创建或更新 .env 文件。" },
