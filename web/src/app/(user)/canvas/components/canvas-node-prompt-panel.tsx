...
export function CanvasNodePromptPanel({ node, isRunning, onPromptChange, onConfigChange, onGenerate, onStop, mentionReferences = [], onImageSettingsOpenChange }: CanvasNodePromptPanelProps) {
    ...
    useEffect(() => {
        ...
        if (!textarea) return;
        const scale = Math.min(1.15, Math.max(0.7, window.innerHeight / 1440));
        const minHeight = Math.round(230 * scale);
        const maxHeight = Math.round(330 * scale);
        ...
        >
            <div className="relative">
                ...
                <CanvasResourceMentionTextarea
                    ...
                    onSubmit={submit}
                    aria-label="节点提示词"
                    className="thin-scrollbar w-full resize-none overflow-y-auto rounded-xl px-3 py-2 pr-11 text-sm leading-5 outline-none"
            ...
            <div className="mt-2 flex min-w-0 flex-wrap items-center gap-2">
                <div className="canvas-composer-tools flex min-w-0 flex-1 flex-wrap items-center gap-2">
                    ...
                    {mode === "image" ? (
                        <>
                            <ModelPicker className="min-w-[9rem]" config={config} value={config.model} onChange={(model) => onConfigChange(node.id, { model })} capability="image" onMissingConfig={() => openConfigDialog(true)} />
                        ...
                        <>
                            <ModelPicker className="min-w-[9rem]" config={config} value={config.model} onChange={(model) => onConfigChange(node.id, { model })} capability="video" onMissingConfig={() => openConfigDialog(true)} />
                        ...
                        <ModelPicker className="min-w-[9rem]" config={config} value={config.model} onChange={(model) => onConfigChange(node.id, { model })} capability="text" onMissingConfig={() => openConfigDialog(true)} />
                    )}
                </div>
                <Button
                    type="primary"
                    className="canvas-generate-button !h-9 !min-w-16 shrink-0 !rounded-full !px-3"
                    danger={isRunning}
