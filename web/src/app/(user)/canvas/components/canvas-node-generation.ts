...
export async function hydrateNodeGenerationContext(context: NodeGenerationContext) {
    ...
    let referenceImages = await Promise.all(context.referenceImages.map(async (image) => ({ ...image, dataUrl: await downscaleDataUrlForVision(await imageToDataUrl(image)) })));
