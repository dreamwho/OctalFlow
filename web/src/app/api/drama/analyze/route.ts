...
export async function POST(request: Request) {
    ...
    try {
        ...
        for (const candidate of rankTextPlanningCandidates(candidates.map((candidate) => ({ ...candidate, channelId: candidate.channel.id })))) {
            try {
                ...
                try {
                    ...
                    if (typeof call.pointsRemaining === "number") response.headers.set("x-octalaicanvas-points-remaining", String(call.pointsRemaining));
                    return response;
                } catch (error) {
        ...
        if (typeof refundedPointsRemaining === "number") response.headers.set("x-octalaicanvas-points-remaining", String(refundedPointsRemaining));
        return response;
    }
}
...
function readCallResult(args: string, headers: Headers) {
    const remaining = Number(headers.get("x-octalaicanvas-points-remaining"));
    return {
        args,
