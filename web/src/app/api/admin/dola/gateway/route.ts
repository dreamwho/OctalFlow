...
export async function PATCH(request: Request) {
    const access = await requireDolaAdmin();
    if ("error" in access) return access.error;
    const parsed = await readJsonBodyResult<{ enabled?: unknown; autoWatermark?: unknown; rotationLimit?: unknown; captureVerificationScreenshot?: unknown }>(request);
    if (!parsed.ok) return apiCompatError(parsed.status, parsed.message);
    const patch: { enabled?: boolean; autoWatermark?: boolean; rotationLimit?: number; captureVerificationScreenshot?: boolean } = {};
