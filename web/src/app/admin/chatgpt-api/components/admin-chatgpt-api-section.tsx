...
import { Alert, App, Button, Checkbox, Empty, Input, Modal, Pagination, Popconfirm, Progress, Space, Spin, Switch, Tabs, Tag } from "antd";
...
type LogPage = { items: Array<{ id: string; model?: string; presentation?: { summary_text?: string; status?: { label?: string }; duration?: { text?: string } } }>; total: number };
...
export function AdminChatGptApiSection({ controller }: { controller: AdminDashboardController }) {
    ...
    const [logs, setLogs] = useState<LogPage>({ items: [], total: 0 });
    ...
    const loadLogs = useCallback(async (silent = false) => {
        ...
        try {
            const nextLogs = await chatGptApiRequest<LogPage>("logs?limit=20&offset=" + (logPage - 1) * 20);
    ...
    useEffect(() => {
        if (tab !== "logs") { setLogDetailId(null); return; }
        let stopped = false;
        ...
        void follow();
        return () => { stopped = true; if (timer) clearTimeout(timer); loadRevisions.current.logs += 1; };
    }, [loadLogs, tab]);
        ...
        <div className="min-w-0 space-y-4">
            ...
            <div data-chatgpt-api-tab-panel="logs" className={tab === "logs" ? "space-y-4" : "hidden"}>
                <Panel>
                    <PanelHeader
                        title="请求日志"
                        description="从提交开始记录，自动更新当前请求状态；点击详情查看请求内容和过程日志。"
                        actions={
                            <Button loading={loading.logs} disabled={mutating} onClick={() => void loadLogs()}>
                                刷新日志
                            </Button>
                        }
                    />
                    <div className="space-y-3 p-3 sm:p-4">
                        {logs.items.length ? (
                            logs.items.map((log) => (
                                <div key={log.id} className="rounded-lg border border-zinc-200 p-3 text-sm dark:border-zinc-800">
                                    <Button className="float-right" size="small" onClick={() => setLogDetailId(log.id)}>查看详情</Button>
                                    <div className="break-words">{log.presentation?.summary_text || log.model || "请求记录"}</div>
                                    <div className="mt-1 text-xs text-zinc-500">
                                        {log.presentation?.status?.label} {log.presentation?.duration?.text}
                                    </div>
                                </div>
                            ))
                        ) : (
                        ...
                        <Pagination current={logPage} pageSize={20} total={logs.total} showSizeChanger={false} onChange={setLogPage} />
                    </div>
                </Panel>
            </div>
