...
import { ArrowUpRight, ChevronsDown, Clapperboard, FolderOpen, History, Image as ImageIcon, Palette, Play, Plus, ScanFace, ShoppingBag, Sparkles, Video, WandSparkles, X, type LucideIcon } from "lucide-react";
...
type LandingQuickAction = { label: string; detail: string; icon: LucideIcon; mode?: "image" | "video"; prompt?: string; action?: "optimize" | "inspiration" };

const LANDING_QUICK_ACTIONS: readonly LandingQuickAction[] = [
    { label: "生成精美插画", detail: "描述一个梦幻的场景", icon: ImageIcon, mode: "image", prompt: "描述一个梦幻的场景，生成一张精美插画：" },
    { label: "制作创意视频", detail: "把想法变成动态故事", icon: Video, mode: "video", prompt: "把下面的想法变成一段动态故事短片：" },
    { label: "设计品牌海报", detail: "输入主题生成多版设计", icon: Palette, mode: "image", prompt: "围绕主题生成多版品牌海报设计：" },
    { label: "优化提示词", detail: "让你的想法更精准", icon: WandSparkles, action: "optimize" },
    { label: "更多灵感", detail: "探索全部模板", icon: ArrowUpRight, action: "inspiration" },
];
...
export default function CreatePage() {
    ...
    const runQuickAction = (item: LandingQuickAction) => {
        if (item.action === "optimize") {
            if (!promptValueRef.current.trim()) {
                message.info("请先输入创作需求，再优化提示词");
                window.requestAnimationFrame(() => inputRef.current?.focus());
                return;
            }
        ...
        if (item.action === "inspiration") {
            document.querySelector("#create-inspiration")?.scrollIntoView({ behavior: "smooth", block: "start" });
            return;
        }
        if (item.mode) changeCreationMode(item.mode);
    ...
    return (
        <main data-page-hidden={pageHidden ? "true" : undefined} className={cn(studioStyles.page, "relative flex h-full min-h-0 flex-col overflow-hidden text-[#20242a] dark:text-[#f3f5f7]")}>
                    ...
                    >
                        {showConversation ? (
                            ...
                            <div className={studioStyles.landing}>
                                <div className={studioStyles.landingBackdrop} data-testid="create-landing-backdrop" aria-hidden="true">
                                    <img
                                        data-landing-backdrop-visual
                                        className={`${studioStyles.landingBackdropVisual} ${studioStyles.landingBackdropLight}`}
                                    ...
                                    <img
                                        data-landing-backdrop-visual
                                        className={`${studioStyles.landingBackdropVisual} ${studioStyles.landingBackdropDark}`}
                                    ...
                                    />
                                    <span data-landing-backdrop-veil />
                                </div>
                                <div className={studioStyles.landingFrame}>
                                    <div className={studioStyles.landingPrimary}>
                                        <div className={studioStyles.landingHero}>
                                            <div className={studioStyles.landingEyebrow}>
                                                <span /> DREAMYO CREATIVE OS
                                            </div>
                                            <h1 className={studioStyles.landingTitle}>
                                                你好，创意玩家 <span aria-hidden="true">👋</span>
                                            </h1>
                                            <p className={studioStyles.landingSubtitle}>从灵感，到完整作品。把想法告诉我，让 {siteTitle} 为你点亮创意的更多可能。</p>
                                            ...
                                            <div className={studioStyles.landingQuickGrid} aria-label="快速开始">
                                                {LANDING_QUICK_ACTIONS.map((item) => {
                                                    const Icon = item.icon;
                                                    return (
                                                        <button key={item.label} type="button" className={studioStyles.landingQuickCard} onClick={() => runQuickAction(item)} aria-label={`${item.label}，${item.detail}`}>
                                                            <span className={studioStyles.landingQuickIcon}>
                                                                <Icon aria-hidden="true" />
                                                            </span>
                                                            <span className={studioStyles.landingQuickCopy}>
                                                                <strong>{item.label}</strong>
                                                                <small>{item.detail}</small>
                                                            </span>
                                                            <ArrowUpRight aria-hidden="true" className={studioStyles.landingQuickArrow} />
                                                        </button>
                                                    );
                                                })}
                                            </div>
                                            <div className={studioStyles.scrollCue} aria-hidden="true">
                                                <span />
                                                向下探索工作台
                                            </div>
                                        </div>
                                        <div className={studioStyles.landingContent}>
                                            ...
                                            <div id="create-inspiration" className="scroll-mt-4">
                                                <CreateInspirationGallery onUsePrompt={usePublicPrompt} onUseImage={usePublicImage} />
                                            </div>
                                        </div>
                                    </div>
                                    <CreateLandingAssistant siteTitle={siteTitle} onPrompt={usePublicPrompt} />
                                </div>
                            </div>
                        )}
                    </section>
...
function CreateLandingAssistant({ siteTitle, onPrompt }: { siteTitle: string; onPrompt: (value: string) => void }) {
    const suggestions = ["帮我把这段需求整理成可执行的提示词", "推荐适合这次创作的风格与模型", "基于我的项目生成更多创意", "帮我分析这张参考图片"];

    return (
        <aside className={studioStyles.landingAssistant} aria-label="Dreamyo AI 助手" data-testid="create-design-assistant">
            <div className={studioStyles.assistantHeader}>
                <div>
                    <span className={studioStyles.assistantKicker}>创作陪伴</span>
                    <h2>
                        <Sparkles aria-hidden="true" /> {siteTitle} AI 助手
                    </h2>
                </div>
                <span className={studioStyles.assistantStatus} aria-label="在线" />
            </div>
            <p className={studioStyles.assistantWelcome}>嗨！我在这里陪你完成创作。有什么想法，随时告诉我。</p>
            <div className={studioStyles.assistantSuggestions}>
            ...
            <div className={studioStyles.assistantContext}>
                <div className={studioStyles.assistantContextTitle}>当前工作区</div>
                <Link href="/canvas" className={studioStyles.assistantProject}>
                    <span className={studioStyles.assistantProjectIcon}>
                        <FolderOpen aria-hidden="true" />
                    </span>
                    <span>
                        <strong>我的创作空间</strong>
                        <small>从画布、素材到生成结果</small>
                    </span>
                    <ArrowUpRight aria-hidden="true" />
                </Link>
            </div>
            <div className={studioStyles.assistantInputHint}>继续提问…</div>
        </aside>
    );
}
