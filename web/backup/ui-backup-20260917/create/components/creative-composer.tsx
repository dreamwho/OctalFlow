...
import { AtSign, Boxes, Check, ChevronDown, FileVideo, ImageIcon, Plus, Sparkles, WandSparkles, X } from "lucide-react";
            ...
            <div data-testid="creative-composer-compact-shell" className="pointer-events-auto mx-auto w-full max-w-[1036px] px-3 pb-3 transition-[max-width,padding] duration-200 sm:px-6 sm:pb-4">
                ...
                >
                    ...
                    <Tooltip title={busy ? "停止生成" : "发送"}>
                        <GenerationActionButton appearance="icon" running={busy} cancellable className="shrink-0" disabled={!busy && !value.trim() && !canSubmitWithoutPrompt} onClick={busy ? onCancel : onSubmit} aria-label={busy ? "停止生成" : "发送"} />
                    </Tooltip>
                </div>
            </div>
        );
        ...
        <div className={cn("mx-auto w-full", centered ? "max-w-[1080px]" : "max-w-[1120px] px-3 pb-3 sm:px-6 sm:pb-5")}>
            ...
            >
                ...
                <div className="flex min-w-0 items-center gap-2 px-0.5 pb-0.5 pt-2">
                    <div className="hide-scrollbar flex min-w-0 flex-1 items-center gap-1 overflow-x-auto sm:gap-2">
                        <Popover
                            trigger="click"
                            placement={composerPopoverPlacement}
                            ...
                            styles={{ container: { padding: 0, borderRadius: 16, overflow: "hidden" } }}
                            content={
                                <div className="hide-scrollbar max-h-[min(240px,calc(100dvh-96px))] w-[calc(100vw-56px)] max-w-[300px] overflow-y-auto py-1 sm:w-72 sm:max-w-none">
                                        ...
                                        {creativeModeOptions.map((option) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    type="button"
                                                    className={cn(
                                                        "flex min-h-10 w-full items-center gap-3 rounded-xl px-2.5 py-2 text-left transition hover:bg-[#eef3f6] dark:hover:bg-[#29323a]",
                        ...
                        <Popover
                            ...
                            content={
                                <div
                                    data-creative-skill-popover
                                    className="hide-scrollbar max-h-[300px] w-[calc(100vw-32px)] max-w-[430px] overflow-y-auto rounded-[20px] border border-[#cdddf0] bg-white/[.96] p-4 text-[#1f2b46] shadow-[0_28px_80px_rgba(73,104,165,.18),0_0_42px_rgba(79,104,255,.12),inset_0_1px_0_rgba(255,255,255,.9)] backdrop-blur-2xl dark:border-cyan-300/20 dark:bg-[#020813]/[.98] dark:text-[#f7fbff] dark:shadow-[0_28px_80px_rgba(0,0,0,.58),0_0_42px_rgba(79,104,255,.12),inset_0_1px_0_rgba(255,255,255,.07)]"
                                >
                                    <div className="flex items-start justify-between gap-3 pb-2">
                                        <div>
                                            <p className="text-base font-semibold tracking-[-.02em] text-[#13213f] dark:text-white">Skill</p>
                                            <p className="mt-1 text-[11px] text-[#637394] dark:text-[#7f90aa]">组合多个能力，输入 /名称 也可快速筛选</p>
                                        </div>
                                        {selectedSkills.length ? (
                                            <span className="shrink-0 rounded-full border border-[#b9d7eb] bg-[#eaf7fb] px-2 py-1 text-[11px] text-[#28738e] dark:border-cyan-300/20 dark:bg-cyan-300/10 dark:text-cyan-100">
                                                已选 {selectedSkills.length}
                                            </span>
                                        ) : null}
                                    </div>
                                    {skillsLoading ? <p className="px-2 py-3 text-xs text-[#637394] dark:text-[#7f90aa]">正在加载...</p> : null}
                                    {!skillsLoading && !skills.length ? <p className="px-2 py-3 text-xs text-[#637394] dark:text-[#7f90aa]">暂无可用 Skill</p> : null}
                                    {skills.length ? (
                                        <div className="mb-2 rounded-xl border border-[#d7e7ef] bg-[#f3f8fb] p-1 dark:border-white/[.08] dark:bg-white/[.035]">
                                            <div
                                                    ...
                                                    <button
                                                        ...
                                                        className={cn(
                                                            ...
                                                            skillCategory === category.id
                                                                ? "border-[#a9d8e7] bg-gradient-to-r from-[#566fff]/15 to-[#42d7e7]/15 text-[#2f4c9a]"
                                                                : "border-transparent bg-transparent text-[#6d7d95] hover:bg-[#e9f2f7] hover:text-[#294d77] dark:border-transparent dark:bg-transparent dark:text-[#7f90aa] dark:hover:bg-white/[.04] dark:hover:text-white",
                                                        )}
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={cn(
                                                            ...
                                                            selected
                                                                ? "border-[#b7d6ec] bg-gradient-to-r from-[#566fff]/10 to-[#42d7e7]/10 text-[#1f315a] dark:border-cyan-300/20 dark:bg-gradient-to-r dark:from-[#566fff]/20 dark:to-[#42d7e7]/[.08] dark:text-white"
                                                                : "border-transparent text-[#41536f] hover:border-[#b9d7eb] hover:bg-[#f0f8fb] dark:border-transparent dark:text-[#dbe5f2] dark:hover:border-cyan-300/15 dark:hover:bg-white/[.045]",
                                                        )}
                                                        onClick={() => {
                                                    ...
                                                    >
                                                        ...
                                                        <span className="flex min-w-0 flex-1 items-start gap-2">
                                                            ...
                                                            <span className="min-w-0 flex-1">
                                                                ...
                                                                <span className="mt-0.5 line-clamp-2 block text-[11px] leading-4 text-[#637394] dark:text-[#75869d]">{skill.description}</span>
                                                            </span>
                    ...
                    <Tooltip title={busy ? "停止生成" : "发送，Enter 直接发送"}>
                        <GenerationActionButton
                            appearance={centered ? "primary" : "icon"}
                            density={centered ? "standard" : "compact"}
                            running={busy}
                            cancellable
                            className={cn("shrink-0", centered && "h-11 !px-5 !text-sm")}
                            disabled={!busy && !value.trim() && !canSubmitWithoutPrompt}
                            onClick={busy ? onCancel : onSubmit}
                            aria-label={busy ? "停止生成" : "发送"}
                        >
                            {centered && !busy ? "开始创作" : null}
                        </GenerationActionButton>
                    </Tooltip>
                </div>
            </div>
...
function ComposerMediaThumbnail({ asset, compact = false, onRemove }: { asset: CreativeAsset; compact?: boolean; onRemove: (id: string) => void }) {
    ...
    return (
        ...
        >
            ...
            <button
                type="button"
                className="group/remove absolute -right-1.5 -top-1.5 z-10 grid size-7 place-items-center !border-0 !bg-transparent !p-0 !text-white shadow-none transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#28738e] focus-visible:ring-offset-1"
                onClick={(event) => {
                    event.stopPropagation();
                    onRemove(asset.id);
                }}
