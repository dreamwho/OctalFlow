...
import { ArrowUpRight, CheckCircle2, LoaderCircle, Maximize2, Paperclip, RefreshCw, Sparkles } from "lucide-react";
...
export function CreateWorkbenchOverview({ onUseAsset }: { onUseAsset: (asset: CreateOverviewAsset) => Promise<void> }) {
    ...
    return (
        <div className="create-overview mt-3 w-full space-y-3 pb-3 sm:mt-12 sm:space-y-9 sm:pb-8">
            <section aria-labelledby="create-assets-heading">
                ...
                {!loading && !error && recentAssets.length ? (
                    <div className="create-overview-assets-grid grid grid-cols-2 gap-2 pt-2 sm:grid-cols-3 sm:gap-3 sm:pt-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
            ...
            <section aria-labelledby="create-projects-heading">
                ...
                <div className="create-overview-projects-grid mt-2 grid gap-2 lg:grid-cols-[minmax(0,1.55fr)_minmax(280px,0.9fr)] sm:mt-3 sm:gap-3">
...
function TaskRow({ task }: { task: CreateOverviewTask }) {
    ...
    const icon = task.kind === "agent" ? <Sparkles className="size-4" /> : <DreamyoIcon name={isImage ? "image" : "video"} size={18} />;
    ...
    return (
        <Link href={href} className="group flex min-w-0 items-center gap-3 py-3 first:pt-0 last:pb-0">
            <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-[#f1f3f5] text-[#697381] dark:bg-[#252a31] dark:text-[#aab2bd]">{icon}</span>
