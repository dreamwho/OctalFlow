...
- [Dola/异常处理] 未知会话文本与拒识终态即时检测：针对上游大模型返回对话类拒识（如 30 秒无法直接生成、内容建议或非标准说明）而未产出视频生成卡片（creation_block）的场景，解析层即时判定任务为生成失败，打破画布无限卡死在「生成中」的死轮询；错误文本截断上限从 120 扩至 1000 字符，并将上游原本返回内容完整记录至请求日志（log.error 与 lifecycle）和任务详情中。
- [Dola/调度白名单] 调度白名单账号池状态判定修复：将白名单过滤与可用账号判定解耦，状态为 `unverified`（新导入待验证）的正常账号在指定为生效分组白名单时可正常参与任务调度与换号轮询，并实时在后台「可轮询」Tab 列出，彻底消除画布提交 Dola 视频生成报「没有可用的 Dola Cookie 账号」问题。
- [画布/提示词] 提示词展开弹窗 UI 深度优化：移除弹窗内包裹编辑器的多余内边框（仅保留模态框外框），扩大编辑高度至最大 34rem，移除底部多余的「生成」按钮；提示词输入框全面支持中文全角 `＠` 与半角 `@` 呼出素材引用，修复多行段落时光标偏移导致的引用失效，并将浮层挂载至全屏顶层安全视口。

- [Dola/账号池] 新增账号分组管理与调度分组白名单：支持按号商/批次分配分组、按分组筛选账号、批量分配分组及导入时直接归组；反代网关新增「任务调度生效分组 (白名单)」配置，仅允许指定分组参与任务轮询与换号；操作列移除高风险的「真实生成验证」，仅保留「有头测试」；核查确认 Camoufox 动态代理下每次提交为独立隔离沙箱环境。

- [云存储/计费] 新增独立存储商品，购买时固定容量、有效期、叠加/续费与限购规则；支付成功按订单唯一授予容量，退款撤销权益但保留已有文件。后台商品、用户商品卡片和结算页显示云存储容量及规则；真实支付与 OSS 联调待验收。

- [用户管理] 编辑用户时按职责显示该用户云存储已用、预留、可用空间和来源构成，数据直接来自服务端额度统计。

- [Dola/修复] 视频任务「永远生成中」根因修复：SSE 提交流元组不入遍历导致会话 ID/结果提取恒为空，恢复最近会话兜底接线与「出了点问题」通用失败终态识别；sidecar 重启遗留 running 任务自动重分类收敛并释放账号占用；有头测试结束时登录失效立即落库归入「登录失败」分类。

- [云存储] 同用户重复文件建立独立引用并只计一次实际容量；删除最后引用时清理 OSS 对象并释放配额，删除失败不提前释放。

- [桌面应用] 商用版离线退出现在立即清除本地登录凭据与会话，并以系统安全存储加密暂存待撤销设备令牌，恢复联网后补交撤销；桌面版整体仍处于实施与联调阶段。

- [画布] 彻底解决已成片视频节点重新生成时父节点误入「生成中」状态：派生新子节点生成时将 `targetRunningId` 显式预绑定至子节点唯一 ID，父节点及提示词面板不再误判定 `isRunning` 与展示停止/加载中按钮；`markSourceStatus` 状态覆写收敛至就地生成节点，已有成片内容的源节点分支生成时保持 `NODE_STATUS_SUCCESS` 与成片展示不变，仅新建子节点进入加载中；提示词面板草稿隔离扩展至视频与音频，编辑已有成片提示词不再提前修改源节点历史提示词。
- [Dola/后台] 请求日志就地处理滑块验证与触发挑战截图放大查看：日志列表与详情抽屉均可就地唤起交互式滑块验证弹窗；触发验证时 Camoufox 截取完整视口存盘并在抽屉中支持缩放查看；请求日志头部与网关设置支持验证截图抓取开关；修复 `/v1/accounts/verify` 代理白名单缺失导致的 502 错误。
- [Dola/账号池] 拆分独立「待滑块验证」账号分组：将 `verification_required` 从「风控」标签彻底分离，独立统计 Tab 计数与一键检测结论；过滤通用网络/代理异常（如 proxy_region_blocked / submission_unknown）避免误报 needs_review 及无会话的人工确认报错。
- [修复] GeminiAI Provider「服务暂时不可用」根因修复：sidecar 请求从 Node 内置 fetch 改为 npm undici fetch（dispatcher 接口不兼容导致全部失败）；sidecar 内部密钥持久化避免局部重启后 401；supervisor 启动 sidecar 前预清理固定端口残留进程。Dola 换号上限钳制 0–20，浏览器导航中断类错误归入可换号重试，失败节点错误框改为 图标+标题+原因 分层展示。
- [后台/画布] 提示词运营补编辑功能（弹窗双模式接入既有 PATCH）；新增「画布功能菜单」配置化：画布快捷生成功能（分镜大师/人物三视图等）迁移为后台可运营配置，支持分组与条目的增删改查、启停、排序，动作类型覆盖图片/视频/音频/文本并可配置默认生成参数，画布右键/悬浮/弹窗入口全部数据驱动渲染。
- [后台] 修复「取回无水印视频」失败：fallback_api 误取完整响应顶层的带水印播放地址，改为与生成链路一致优先用 vodPayload 解析；失败时透出具体错误码与原因，不再只有通用失败提示。
- [后台] 修复打开 Dola API 分区直接显示"页面加载失败"的问题：账号池选中项清理 effect 在未加载数据的不稳定引用上无限 setState（React #185），改为内容未变时保持原引用跳过更新。
- [品牌/全局] 全局错误页修复：LOGO 替换为 dreamyo 品牌图形（icon.svg 旧 ∞ 字形同步重制），页面改为自包含样式并按浅/深主题适配 C01 配色，修复残留应用 CSS 导致的按钮变黑问题。
- [Dola/反代] 统一换号失败判定：生图链路接入账号轮换预算（提交时自动换号、轮询时限额保持等待）；GeminiTools 新增独立换号次数上限；GeminiAIStudio 由 web 下发换号预算、sidecar 自动换号并经 `X-AIStudio-Rotations` 回传计数；内容风控/参数错误一律立即失败不消耗预算。
- [Dola] 生成前后端状态同步修复：换号配额用尽时任务保持轮询等待上游真实终态，不再提前判失败；前端轮询瞬时故障继续等待；后台请求日志支持对已完成记录一键取回无水印视频；账号池增加状态分组页签与批量删除，轮询只使用正常账号。
- [首页] 首屏结构对齐参考站：居中品牌图形 + 主标题 + 提示词面板（宽 780px）纵向节奏，面板去除分隔线、输入区加高；顶栏账号区（限时特惠/邀请赠积分/头像弹层）与通知中心、画布菜单、切换画布弹出层统一为暗色卡片风格。
- [首页] 顶部公告栏：后台站点配置新增公告开关/文案/跳转链接，首页顶部渲染可关闭公告条（关闭后按文案记忆）。
- [首页/全站] 页面标题体系重构：33 个路由全部设计标题（最长前缀匹配规则表），运行时 document.title 与静态 metadata 对齐；GPT-Image 档位选中后显示 智能方形竖版横版 档位名而非原始像素。
- [品牌] 横向组合标（图形+dreamyo+标语）接入登录/注册页与首页页脚；加载页换纯图标 LOGO 并按路由显示「正在加载 页面名」；深浅主题自适应（浅色反色、深色原样）。
- [画布] 生成配置与节点参数弹窗扁平紧凑化（去头部/底部区块，点击即生效）；GPT-Image 档位弹窗角标与触发按钮显示档位名；参数弹窗四角边框与内容层圆角对齐。
- [画布] 生成时序修复：输出节点（含生成中状态与连线）在 Skill 优化与上下文解析之前创建，点击生成立即出现新节点；生成中节点不再被覆盖（一律新建），Skill 失败/尾帧未就绪自动回滚预创建节点。
- [画布] 连线方向修复：非配置节点互连按素材语义定向（资源→消费），双向拖线不再产生反向死连接导致引用丢失；连线锚点按两节点相对位置自动换边，左侧拉出新节点不再绕行；项目卡「⋯」菜单复刻参考站样式。
- [画布] 项目列表页迁移至首页框架（星空背景+顶部导航+左侧快捷导航），标题「我的项目」+搜索、渐变新建卡、封面卡与「⋯」管理菜单；工具栏迁至左侧竖轨（纵向布局+右侧 hover 提示）。
- [画布] 节点生成区替换为首页同款「积分+圆形提交」组合胶囊（星芒图标/运行中停止方块）；积分实时计算并随模式模型时长变化。
- [后台/逻辑模型] 列表改卡片网格并按 启用+有渠道 → 启用无渠道 → 未启用 排序；编辑/删除直接持久化（修复保存后刷新丢失）；卡片显示最近平均生成时长（新增 model-stats 聚合接口）与彩色能力/渠道标签。
- [后台/代理管理] 会员权益、邀请有礼等头部按钮接入流光动效（prefers-reduced-motion 降级）。
- [性能] 修复任务文件全量读写放大：generation-tasks.json 单条 19.5MB 内联 base64 过期任务清理（23.4MB→1.8MB，解析 66ms→6.8ms）、mtime 读缓存跳过未变更解析、媒体登记失败不再内联 base64（重试代替落库）。
- [性能] Worker 新增 lifecycle lane 周期清理到期技术数据；数据生命周期路由接受 Worker 令牌；auth 文件分支新增 1 秒读缓存（clone-on-read、写后失效）。
...
- [GeminiAIStudio/代理管理] 分区重构为 账号与渠道 / 代理管理 页签；通用代理解锁：sidecar 新增运行时代理切换接口（自动重启浏览器会话），Web 自动推送节点出口并支持切回魔法监听；通用出口仅支持单节点（浏览器会话无法按请求轮换）。

- [GPTAPI/GeminiTools/GeminiAIStudio] 代理管理交互重构：启用代理滑块移入代理管理页，开启后选择 代理方式（魔法代理/通用代理），仅显示所选方式配置；GeminiTools 支持通用代理出口（单节点或整组，整组经运行时容量随机轮换），GeminiAIStudio 因浏览器级代理架构暂不开放通用来源并置灰说明。

- [后台/通用代理] 通用代理升级为上游配置下的独立页面（代理管理 + 请求日志两个页签）：GPTAPI 原"代理管理"分组/节点管理整体迁入；新增请求日志记录每一次经代理出口提交的请求（出口方式、代理节点与地址、状态、耗时、错误，支持分页与详情）。

- [GPTAPI/代理管理] "代理管理"与"魔法代理"Tab 合并为单个"代理管理"Tab：顶部为使用代理滑块，下方并列 魔法代理 / 通用代理 两个来源卡片与出口选择；请求日志行新增代理小卡片标记，详情新增"代理出口"字段。

- [后台/请求日志] 请求日志详情抽屉支持拖动左缘调整宽度（GPTAPI、GeminiTools、GeminiAIStudio 三处一致）：向左拖动持续加宽以显示更多内容，宽度限制在视口内，拖动时禁用文本选择并有高亮反馈。
- [GPTAPI/代理管理] 代理节点地址支持 `主机:端口:用户名:密码` 冒号格式（自动补全 http、凭据 URL 编码，兼容无认证 `主机:端口`），保存/删除校验错误透出具体中文原因，管理端支持中文代理组 ID 删除；新增实时测速弹层（日志流/进度/筛选/重试停止）与"测试 Google 访问"连通检测。

- [全局] 新增根级与工作区两层中文错误边界：客户端渲染异常时显示错误说明、错误编号与重试/重新加载，站点更新后旧缓存引用失效 chunk 时自动刷新一次，替代 Next.js 英文兜底页。

- [部署] 离线 Compose 契约补齐 chatgpt-api 服务声明（offline 与 offline-external-db 拓扑），部署契约测试恢复通过。
