...
_EMAIL_PATTERN = re.compile(r"[A-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[A-Z0-9-]+(?:\.[A-Z0-9-]+)+", re.IGNORECASE)
    ...
    async def _extract_email_from_page(self, page) -> str | None:
        value = await page.evaluate(
            ...
            () => {
                ...
                const extract = (value) => String(value || '').match(pattern)?.[0] || null;
                const selectors = ['[data-email]', '[data-identifier]', '[aria-label*="@"]', '[title*="@"]'];
                for (const element of document.querySelectorAll(selectors.join(', '))) {
                    for (const attribute of ['data-email', 'data-identifier', 'aria-label', 'title']) {
                        const email = extract(element.getAttribute(attribute));
                        if (email) return email;
                    }
                    const email = extract(element.textContent);
                    if (email) return email;
                }
        ...
        return _normalize_email_candidate(value)
    ...
    async def _login_worker(
        ...
        *,
        ...
        if (
            ...
            session.error = (
                "当前 Linux 运行环境没有 DISPLAY 或 Wayland 图形会话，无法启动有头 Google 登录；"
                "请在带图形显示/VNC 的受信任环境中重新发起，或改用受控的无头授权流程。"
            )
            logger.warning("[%s] %s", session_id, session.error)
            return
        ...
        try:
                    ...
                    for origin in storage_state.get("origins", []):
                        for item in origin.get("localStorage", []):
                            if "email" in item.get("name", "").lower():
                                detected_email = _normalize_email_candidate(item.get("value"))
                                break
                        if detected_email:
                            break
                except Exception:
            ...
            if detected_email is None:
                session.status = LoginStatus.FAILED
                session.error = "Google 授权已完成，但未能识别账号邮箱，本次账号未保存；请重新发起授权"
                logger.warning("[%s] %s", session_id, session.error)
                return
