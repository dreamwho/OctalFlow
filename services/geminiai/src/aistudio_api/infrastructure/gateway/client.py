...
class AIStudioClient:
    NATIVE_IMAGE_MODELS = {"gemini-3-pro-image", "gemini-3.1-flash-image"}
    IMAGE_ASPECT_RATIOS = {"1:1", "9:16", "16:9", "3:4", "4:3", "3:2", "2:3", "5:4", "4:5", "21:9"}
    ...
    @classmethod
    def resolve_image_size(cls, size: str, *, aspect_ratio: str | None = None, image_size: str | None = None) -> list[str | None] | None:
        ...
        if aspect_ratio is not None or image_size is not None:
            ...
            ratio = None if not ratio_value or ratio_value.lower() == "auto" else ratio_value
            resolution = (image_size or "").strip().upper()
            return [ratio, resolution] if (ratio is None or ratio in cls.IMAGE_ASPECT_RATIOS) and resolution in cls.IMAGE_SIZES else None
    ...
    async def generate_image(
        ...
        output_resolution = self.resolve_image_size(size, aspect_ratio=aspect_ratio, image_size=image_size)
        normalized_model = model.removeprefix("models/")
        if normalized_model not in self.NATIVE_IMAGE_MODELS:
            raise RequestError(400, f"不支持的 GeminiAIStudio 生图模型: {normalized_model}")
        if image_search:
            raise RequestError(400, f"{normalized_model} 不支持独立图片搜索工具")

        logger.info("生图请求: model=%s, images=%s", normalized_model, len(images) if images else 0)
        ...
        generation_config_overrides = (
            {"output_resolution": output_resolution} if output_resolution is not None else None
        )
        model_defaults = resolve_model_defaults(normalized_model)
        resolved_tools = None
