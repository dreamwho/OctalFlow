...
from aistudio_api.infrastructure.account.account_store import AccountStore, _write_private_json
...
class BrowserSession:
    ...
    def _derive_profile_dir(auth_file: str | None) -> str | None:
        ...
        profile_name = "camoufox-profile" if is_camoufox_engine() else "profile"
        return str(Path(auth_file).resolve().parent / profile_name)
    ...
    @staticmethod
    def _apply_auth_origins_sync(page, origins: list[dict[str, Any]]) -> None:
        """Restore the AI Studio origin state exported by Playwright storage_state."""
        for origin_state in origins:
            if origin_state.get("origin") != "https://aistudio.google.com":
                continue
            entries = [
            ...
            if not entries:
                continue
            page.goto(origin_state["origin"], wait_until="domcontentloaded", timeout=30000)
            page.evaluate(
                "entries => entries.forEach(({name, value}) => localStorage.setItem(name, value))",
                entries,
            )
        ...
        if should_seed_from_auth and self._auth_file and Path(self._auth_file).exists():
            try:
                ...
                if cached:
                    ...
                    self._apply_auth_origins_sync(self._hook_page, data.get("origins") or [])
        ...
        *,
        ...
        try:
            ...
            if self._ctx is not None:
                try:
                    try:
                        storage_state = self._ctx.storage_state(indexed_db=True)
                    except TypeError:
                        storage_state = self._ctx.storage_state()
                    current_cookies = storage_state.get("cookies") or current_cookies
                    current_origins = storage_state.get("origins")
                    current_indexed_db = storage_state.get("indexedDB")
                except Exception as exc:
                    log.debug("Failed to export browser storage state: %s", exc)
            ...
            state = {
                ...
                "origins": current_origins if current_origins is not None else origins,
            }
            if current_indexed_db is not None:
                state["indexedDB"] = current_indexed_db
        ...
        try:
            ...
            for i in range(45):
                ...
                if page.evaluate("mw:!!window.__bg_service"):
                    ...
                    log.info("Browser bootstrap request completed: status=%s", captured_response_status)
    ...
    def _capture_template_sync(self, model: str, force_refresh: bool = False) -> dict[str, Any]:
        ...
        page = self._ensure_hook_page_sync()
        self._select_model_sync(page, model)
        page = self._ensure_botguard_service_sync()
        if self._bootstrap_template and self._template_matches_model(self._bootstrap_template, model):
        ...
        try:
            ...
            if last_generate_response is not None:
                log.info(
                    "Template request completed: model=%s, status=%s",
                    model,
                    last_generate_response["status"],
                )
            self._restore_textarea_value_sync(textarea, original_text)
            self._templates[model] = captured
    ...
    def _select_model_sync(self, page, model: str) -> None:
        ...
        raw_preferences = page.evaluate("() => localStorage.getItem('aiStudioUserPreference')")
        try:
            preferences = json.loads(raw_preferences) if raw_preferences else {}
        except json.JSONDecodeError:
            preferences = {}
        if preferences.get("promptModel") == full_model and preferences.get("isAdvancedOpen") is True:
            return
        preferences["promptModel"] = full_model
        preferences["isAdvancedOpen"] = True
        page.evaluate(
        ...
        page.goto(
            ...
            timeout=settings.timeout_capture * 1000,
        )
        page.wait_for_selector("textarea", timeout=settings.timeout_capture * 1000)
        self._install_hooks_sync(page)

    @staticmethod
    def _template_matches_model(template: dict[str, Any], model: str) -> bool:
        try:
            body = json.loads(str(template.get("body") or ""))
            captured_model = str(body[0]).removeprefix("models/")
        except (json.JSONDecodeError, IndexError, TypeError):
            return False
        return captured_model == model.removeprefix("models/")
...
"""
    ...
    def _send_hooked_request_sync(self, body: str, timeout_ms: int) -> tuple[int, bytes]:
        ...
        if is_camoufox_engine():
            return self._submit_hooked_body_sync(page, body, timeout_ms)
    ...
    def _submit_hooked_body_sync(self, page, body: str, timeout_ms: int) -> tuple[int, bytes]:
        """Send a rewritten body through AI Studio's own request path.

        Camoufox rejects a synthetic cross-origin XHR even though the page's
        native GenerateContent request succeeds. The installed hook swaps the
        ...
        def on_route(route, request):
            nonlocal rewritten
            if not rewritten and "CountTokens" not in request.url:
                rewritten = True
                try:
                    # Capture the upstream bytes before handing the response
                    # back to the page. Firefox can evict multi-megabyte 4K
                    # response bodies before a normal Response object exposes
                    # them, while route.fetch keeps its own API response body.
                    upstream = route.fetch(post_data=body, timeout=timeout_ms)
                    responses.append((upstream.status, upstream.body()))
        ...
        try:
            ...
            textarea.fill(TEMPLATE_CAPTURE_PROMPT)
            if not self._click_run_button_sync(page):
                raise RuntimeError("failed to submit hooked AI Studio request")

            deadline = time.time() + (timeout_ms / 1000)
            while time.time() < deadline and (not rewritten or not responses):
                page.wait_for_timeout(100)
            if not rewritten:
                raise RuntimeError("AI Studio native request was not intercepted")
            if not responses:
                raise RuntimeError("hooked AI Studio request timeout")
    ...
    def _click_run_button_sync(self, page) -> bool:
        # Prefer the real submit control.  Treating a keyboard event as success
        # without observing the page is incorrect on macOS, where AI Studio uses
        # Command+Enter and silently ignores Control+Enter.
        try:
        ...
        # Fallback for layouts where the submit control class has changed.
        try:
        ...
        try:
            ...
            if textarea is not None:
                textarea.focus()
            shortcut = "Meta+Enter" if "Mac" in page.evaluate("navigator.platform || ''") else "Control+Enter"
