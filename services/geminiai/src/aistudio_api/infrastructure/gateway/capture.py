...
class RequestCaptureService:
    ...
    async def _ensure_template(self, model: str, *, force_refresh: bool = False) -> CapturedRequest:
        # BrowserSession owns the canonical template cache because it is also
        # responsible for clearing that cache whenever the active account is
        # switched. Keeping a second cache here can return an old account's
        # template while the session no longer has a replay URL.
