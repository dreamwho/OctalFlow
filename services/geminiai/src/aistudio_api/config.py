...
DEFAULT_IMAGE_MODEL = os.getenv("AISTUDIO_DEFAULT_IMAGE_MODEL", "gemini-3.1-flash-image")
DEFAULT_BROWSER_PORT = 9222
DEFAULT_STUDIO_URL = "https://aistudio.google.com/prompts/new_chat?model=gemma-4-31b-it"


def _load_studio_url() -> str:
    value = (os.getenv("AISTUDIO_STUDIO_URL") or DEFAULT_STUDIO_URL).strip()
    parsed = urlparse(value)
    if parsed.scheme != "https" or parsed.hostname != "aistudio.google.com":
        raise ValueError("AISTUDIO_STUDIO_URL must be an https://aistudio.google.com URL")
    return value
...
def _load_browser_engine() -> str:
    ...
    """
    value = (os.getenv("AISTUDIO_BROWSER", "camoufox") or "camoufox").strip().lower()
...
def _is_safe_account_id(account_id: object) -> bool:
    ...
    for root in _AUTH_SEARCH_ROOTS:
        ...
        if registry_path.exists():
            try:
                ...
                if _is_safe_account_id(active_id):
...
class Settings:
    ...
    host: str = os.getenv("AISTUDIO_HOST", "0.0.0.0")
    ...
    studio_url: str = _load_studio_url()
    # 账号轮询配置
