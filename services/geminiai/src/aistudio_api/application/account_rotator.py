...
class AccountRotator:
    ...
    async def get_next_account(self, *, exclude_account_ids: set[str] | None = None) -> AccountMeta | None:
        """获取下一个可用的账号。"""
        async with self._lock:
            excluded = exclude_account_ids or set()
            available = [(account, stats) for account, stats in self._get_available_accounts() if account.id not in excluded]

            if not available:
                # 所有账号都在冷却期，找一个冷却时间最短的
                all_accounts = [account for account in self._store.list_accounts() if account.id not in excluded]
                if not all_accounts:
                    return None
                # 选冷却结束最早的
                earliest = min(
