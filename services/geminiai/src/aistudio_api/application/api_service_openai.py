        ...
        async with busy_lock, account_request_lock():
            ...
            try:
                ...
                if await try_switch_account():
                    logger.info("429 限流，已切换账号，重试 %d/%d", attempt + 1, attempts)
                    continue
                ...
                if attempt < attempts - 1 and await try_switch_account():
                    logger.info("账号无模型权限，已切换账号，重试 %d/%d", attempt + 1, attempts)
                    continue
...
async def handle_image_generation(req: ImageRequest, client: AIStudioClient):
    validate_image_request_options(size=req.size, n=req.n, aspect_ratio=req.aspect_ratio, image_size=req.image_size)
    ...
    for attempt in range(attempts):
        async with busy_lock, account_request_lock():
            ...
            try:
                ...
                record_rotator_event("rate_limited")
                if await try_switch_account():
                    logger.info("Image 429 限流，已切换账号，重试 %d/%d", attempt + 1, attempts)
                    continue
                ...
                record_rotator_event("error")
                if attempt < attempts - 1 and await try_switch_account():
                    logger.info("图片模型无账号权限，已切换账号，重试 %d/%d", attempt + 1, attempts)
                    continue
                ...
                last_error = exc
                record_rotator_event("error")
                if exc.status in _RETRYABLE_IMAGE_REQUEST_STATUSES and attempt < attempts - 1:
                    if await try_switch_account():
                        logger.warning(
...
async def handle_image_edit(
    ...
    validate_image_request_options(size=size, n=n, aspect_ratio=aspect_ratio, image_size=image_size)
    ...
    for attempt in range(attempts):
        async with busy_lock, account_request_lock():
            ...
            try:
                ...
                record_rotator_event("rate_limited")
                if await try_switch_account():
                    logger.info("Image Edit 429 限流，已切换账号，重试 %d/%d", attempt + 1, attempts)
                    continue
                ...
                record_rotator_event("error")
                if attempt < attempts - 1 and await try_switch_account():
                    logger.info("图片编辑模型无账号权限，已切换账号，重试 %d/%d", attempt + 1, attempts)
                    continue
                raise HTTPException(403, detail=account_access_denied_detail(model)) from exc
            except RequestError as exc:
                runtime_state.record(model, "errors")
            ...
            try:
                ...
                for stream_attempt in range(effective_retry_attempts()):
                    try:
                        ...
                        runtime_state.record(model, "rate_limited")
                        record_rotator_event("rate_limited")
                        if not has_yielded_data and stream_attempt < effective_retry_attempts() - 1 and await try_switch_account():
                            logger.warning("Stream 429 限流，已切换账号，重试 %d/%d", stream_attempt + 1, effective_retry_attempts())
                            continue
                        raise
                    except RequestError as exc:
