...
def normalize_chat_request(messages, requested_model: str, tmp_dir: str = "/tmp") -> dict:
    ...
    for msg in messages:
        ...
        # OpenAI tool results can be replayed as native AI Studio function
        # responses when their originating call is present in the history.
        if role == "tool":
            raw = _message_text_content(msg.content) or ""
            fname = tool_call_names.get(msg.tool_call_id or "", "")
            if fname:
                response = _decode_json_payload(raw)
        ...
        if role == "assistant" and msg.tool_calls:
            for tool_call in msg.tool_calls:
                ...
                if function is None or not function.name:
                    continue
                arguments = _decode_json_payload(function.arguments or "{}")
                function_call = (
                    (function.name, arguments, tool_call.id)
                    if tool_call.id
...
def _decode_json_payload(value: str) -> Any:
    try:
