...
from aistudio_api.infrastructure.account.login_service import LoginService, LoginSession, LoginStatus, _normalize_email_candidate
...
def test_email_candidate_accepts_google_workspace_domains_and_normalizes_case():
    assert _normalize_email_candidate("Google Account: Operator (USER@EXAMPLE.CN)") == "user@example.cn"
...
def test_headed_login_without_linux_display_returns_actionable_error(monkeypatch):
    monkeypatch.setattr(login_module.platform, "system", lambda: "Linux")
    monkeypatch.delenv("DISPLAY", raising=False)
    monkeypatch.delenv("WAYLAND_DISPLAY", raising=False)

    service = LoginService()
    session = LoginSession(session_id="login_no_display")
    service._sessions[session.session_id] = session

    asyncio.run(service._login_worker(session.session_id, FakeAccountStore(), None, headless=False, ui_locale=None))

    assert session.status == LoginStatus.FAILED
    assert "DISPLAY" in (session.error or "")
