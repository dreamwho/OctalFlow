...
from services.proxy_service import (
    ...
    PROXY_SELECTION_NATIVE_SOURCES,
    normalize_proxy_url,
...
class ProxySelectionUnavailableError(ValueError):
    """Raised when an enabled selection lacks its explicitly chosen source."""
...
class ProxyManagementService:
    ...
    def proxy_selection(self) -> dict[str, object]:
        return self._proxy_selection_payload(self._snapshot())
    ...
    def save_proxy_selection(
        ...
        *,
        ...
        if not isinstance(enabled, bool):
            raise ValueError("proxy selection enabled must be boolean")
        if normalized_mode not in PROXY_SELECTION_MODES:
            raise ValueError("proxy selection mode is invalid")
    ...
    def _proxy_selection_payload(self, snapshot: dict[str, Any]) -> dict[str, object]:
        ...
        return {
            ...
            "native_source": (
                ...
                if selection.native_source in PROXY_SELECTION_NATIVE_SOURCES
                else "manual"
            ),
    ...
    def _ipwo_proxy_configured() -> bool:
        try:
            ...
            return bool(ipwo_proxy_service.is_configured())
        except Exception:
            return False

    @staticmethod
    def _require_ipwo_proxy() -> None:
        try:
            from services.ipwo_proxy_service import ipwo_proxy_service

            if ipwo_proxy_service.is_configured():
                return
        except Exception:
            pass
