...
def _is_quota_refresh_call(item: dict[str, Any]) -> bool:
    endpoint = _clean_text(_detail_value(item, "endpoint")).lower().rstrip("/")
    model = _clean_text(_detail_value(item, "model")).lower()
    return model == "quota_refresh" or endpoint in {
...
class DashboardMetricsService:
    ...
    def _apply_call(bucket: dict[str, Any], item: dict[str, Any]) -> None:
        if _detail_value(item, "status") in {"queued", "running"}:
            return
    ...
    def _aggregate_items(
        ...
        *,
        ...
        for item in items:
            ...
            if _is_quota_refresh_call(item):
                continue
