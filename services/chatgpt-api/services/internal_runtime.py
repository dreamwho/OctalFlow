"""dreamyo-only runtime configuration.
...
DATA_DIR_ENV = "DREAMYO_CHATGPT_DATA_DIR"
RUNTIME_KEY_ENV = "DREAMYO_CHATGPT_API_KEY"
ENCRYPTION_KEY_ENV = "DREAMYO_ENCRYPTION_KEY"
PUBLIC_BASE_URL_ENV = "DREAMYO_CHATGPT_PUBLIC_BASE_URL"
...
def _decode_encryption_key(raw: str) -> bytes:
    if not raw or _PLACEHOLDER_RE.match(raw):
        raise RuntimeError(
            "DREAMYO_ENCRYPTION_KEY is required and must not be a placeholder"
        )
    ...
    if _BASE64_KEY_RE.fullmatch(raw):
        try:
            ...
            raise RuntimeError(
                "DREAMYO_ENCRYPTION_KEY must be 64 hex characters or 32-byte Base64"
            ) from exc
        if len(decoded) == 32:
            return decoded
    raise RuntimeError(
...
def runtime_key() -> str:
    ...
    if len(value) < 32:
        raise RuntimeError(
            "DREAMYO_CHATGPT_API_KEY is required and must be at least 32 characters"
        )
    return value
...
def _normalize_public_base_url(value: str) -> str:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise RuntimeError(
            "DREAMYO_CHATGPT_PUBLIC_BASE_URL must be an absolute HTTP(S) URL"
        )
    if parsed.username is not None or parsed.password is not None:
        raise RuntimeError(
            "DREAMYO_CHATGPT_PUBLIC_BASE_URL cannot include a username or password"
        )
    if parsed.query or parsed.fragment:
        raise RuntimeError(
            "DREAMYO_CHATGPT_PUBLIC_BASE_URL cannot include a query or fragment"
        )
...
def initialize_runtime_environment() -> RuntimeSettings:
    ...
    if not raw_data_dir:
        raise RuntimeError("DREAMYO_CHATGPT_DATA_DIR is required")
    data_dir = Path(raw_data_dir).expanduser()
    if not data_dir.is_absolute():
        raise RuntimeError("DREAMYO_CHATGPT_DATA_DIR must be an absolute path")
    data_dir.mkdir(parents=True, exist_ok=True, mode=0o700)
    if not data_dir.is_dir():
        raise RuntimeError("DREAMYO_CHATGPT_DATA_DIR is not a directory")
    try:
    ...
    if inherited_source_key and inherited_source_key != key:
        raise RuntimeError(
            "CHATGPT2API_AUTH_KEY conflicts with DREAMYO_CHATGPT_API_KEY"
        )
    ...
    if inherited_base_url and not public_base_url:
        raise RuntimeError(
            "set DREAMYO_CHATGPT_PUBLIC_BASE_URL instead of CHATGPT2API_BASE_URL"
        )
    if public_base_url:
