...
class CallRecordRepository:
    ...
    def append(self, record: CallRecordWrite, *, replace: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            existing = session.scalar(select(CallRecordModel).where(CallRecordModel.id == record_id).with_for_update()) if replace else None
            if existing is None:
                session.add(CallRecordModel(**values))
            else:
                previous = (existing.payload or {}).get("detail", {}).get("status")
                if previous and previous not in {"queued", "running"}:
                    return self._row_payload(existing)
                if payload.get("detail", {}).get("status") not in {"queued", "running"}:
                    # Publish the terminal result at a fresh append cursor. Reports
                    # skip pending calls, then ingest completion exactly once without
                    # rebuilding history. Public request identity stays unchanged.
