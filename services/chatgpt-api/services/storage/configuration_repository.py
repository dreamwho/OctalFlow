...
class _DictionaryRepository:
    ...
    def _dictionary(value: object | None) -> dict[str, object]:
        return copy.deepcopy(decrypt_configuration(value))
    ...
    def update(self, values: Mapping[str, object]) -> dict[str, object]:
        ...
        try:
            ...
            result.update(patch)
            stored = self._stored_dictionary(result)
            now = datetime.now(timezone.utc)
            if row is None:
                session.add(self.model(id=1, data=stored, updated_at=now))
            elif self._dictionary(row.data) != result:
                row.data = stored
                row.updated_at = now
            session.commit()
    ...
    def replace(self, values: Mapping[str, object]) -> dict[str, object]:
        ...
        try:
            self._lock(session)
            row = session.get(self.model, 1, with_for_update=True)
            stored = self._stored_dictionary(payload)
            now = datetime.now(timezone.utc)
            if row is None:
                session.add(self.model(id=1, data=stored, updated_at=now))
            elif self._dictionary(row.data) != payload:
                row.data = stored
                row.updated_at = now
            session.commit()
