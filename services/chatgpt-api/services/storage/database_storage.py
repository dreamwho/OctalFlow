...
class DatabaseStorageBackend(StorageBackend):
    ...
    @staticmethod
    def _database_key(collection: StorageCollection, key: str) -> str:
        return account_index_key(key) if collection == "accounts" else key
    ...
    def _replace(
        ...
        *,
        ...
        desired = {
            self._database_key(collection, item_key(collection, item)): item
    ...
    def _mutate(
        ...
        target_keys = {
            *(
                self._database_key(collection, item_key(collection, item))
                for item in upserts
            ),
            *(self._database_key(collection, key) for key in delete_keys),
        }
        ...
        try:
            ...
            for key in delete_keys:
                row = existing.get(self._database_key(collection, key))
                if row is not None:
                    session.delete(row)
                    deleted += 1

            for item in upserts:
                key = self._database_key(collection, item_key(collection, item))
                serialized = self._serialize(item)
                row = existing.get(key)
                if row is None:
                    session.add(model(**{model_key: key}, data=serialized))
