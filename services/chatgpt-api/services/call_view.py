...
def _duration_ms(item: Mapping[str, Any]) -> int:
    if _value(item, "status") in {"queued", "running"}:
        started = _value(item, "started_timestamp", 0)
...
def _status_presentation(summary: Mapping[str, Any]) -> dict[str, str]:
    ...
    if display_status in {"queued", "running", "cancelled"}:
        return {"label": {"queued": "等待执行", "running": "执行中", "cancelled": "已取消"}[display_status], "tone": "warning" if display_status == "cancelled" else "info"}
