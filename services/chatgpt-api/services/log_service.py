...
class LoggedCall:
    ...
    async def run(self, handler, *args, sse: str = "openai"):
        ...
        try:
            ...
            self.log("请求已取消", status="cancelled")
            raise
    ...
    def stream(self, items):
        ...
        try:
            ...
            failed = True
            self.log("响应流已关闭", status="cancelled")
            raise
        except Exception as exc:
            failed = True
            extra = _exception_log_fields(exc, image=image_request)
    ...
    def log(self, suffix: str, result: object = None, status: str = "success", error: str = "",
        ...
        if is_text_review_failure_code(failure_code):
            status = "text_review"
            suffix = "文本"
        self.lifecycle.append({"time": beijing_now_str(), "status": status, "message": suffix})
        detail = {
            ...
            "ended_at": "" if status in {"queued", "running"} else beijing_now_str(),
        ...
        if self.log_fields:
            for key, value in self.log_fields.items():
                if value in (None, ""):
                    continue
                detail[key] = value
        ...
        if self._trace_image_perf() and status not in {"queued", "running"}:
            realtime_monitor_service.finish(detail)
        log_service.append_item({
            "id": self.call_id,
            "time": beijing_from_timestamp(self.started),
            "type": LOG_TYPE_CALL,
