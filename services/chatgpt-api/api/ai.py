...
async def filter_or_log(call: LoggedCall, text: str) -> None:
    if call.endpoint in {"/v1/images/generations", "/v1/images/edits", "/v1/chat/completions", "/v1/responses", "/v1/messages", "/v1/search"}:
        call.log("校验请求中", status="queued")
    try:
        ...
        call.log("请求校验未完成", status="failed", error="请求校验中断")
        raise
...
def create_router() -> APIRouter:
    ...
    async def edit_images(
        ...
        try:
            ...
            call.log("参考素材读取失败", status="failed", error="参考素材读取失败或请求中断")
            raise
        payload["base_url"] = resolve_image_base_url(request)
