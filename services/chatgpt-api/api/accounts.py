...
def _quota_refresh_log_status(result: dict[str, Any]) -> tuple[str, str]:
    errors = result.get("errors")
    if not isinstance(errors, list) or not errors:
        return "success", ""
    if max(0, int(result.get("synced") or 0)):
        return "partial_success", "account and quota synchronization partially failed"
    return "failed", "account and quota synchronization failed"
    ...
    async def sync_accounts_and_quota(
        ...
        refresh_call.log(
            ...
            extra={
                ...
                "selection_mode": (
                    ...
                    else "ids" if body.account_ids or body.access_tokens else "all"
                ),
            },
        )
        ...
        async def _do_sync_accounts_and_quota():
            try:
                ...
                terminal_status, terminal_error = _quota_refresh_log_status(result)
                refresh_call.log(
                    "完成",
                    result,
