...
def _legacy_admin_identity(token: str) -> dict[str, object] | None:
    """Authenticate the restricted runtime's sole built-in administrator.

    ``runtime_key()`` is also what the application middleware verifies for
    every request.  Reading it here keeps the two gates consistent when test
    fixtures (or a process supervisor) replace the isolated runtime settings
    after modules have already been imported.
    """
    auth_key = runtime_key()
    if token and hmac.compare_digest(token, auth_key):
