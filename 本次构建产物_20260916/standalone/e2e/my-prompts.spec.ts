...
test("my prompts accepts file, drop and paste covers and persists edits", async ({ page }) => {
    ...
    try {
        ...
        await expect(page.locator("[data-my-prompts-ready='ready']")).toBeVisible();
        const addButton = page.getByRole("button", { name: "添加提示词" });
        ...
        await page.reload({ waitUntil: "domcontentloaded" });
        const savedRow = page.locator("tr").filter({ hasText: title });
        await expect(savedRow).toHaveCount(1);
        await expect(savedRow.getByText("编辑后的提示词内容", { exact: true })).toBeVisible();
    } finally {
