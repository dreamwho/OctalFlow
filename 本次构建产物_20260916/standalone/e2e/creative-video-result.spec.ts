...
test.afterEach(async ({ page }) => {
    ...
    const unexpected = errors.filter((message) => {
        if (/^429 (?:GET|HEAD) .*\/api\/generation-log-assets\//.test(message)) return false;
        if (message === "Failed to load resource: the server responded with a status of 429 (Too Many Requests)") return false;
        return true;
    });
...
test("asset mentions stay as inline thumbnail references while the editor is focused", async ({ page }, testInfo) => {
    ...
    for (const type of ["image", "video"] as const) {
        ...
        const composer = page.locator('.creative-composer[data-ready="true"]').first();
        const input = composer.getByRole("textbox", { name: /输入创作要求|输入你的创作想法/ });
...
async function captureResult(locator: ReturnType<Page["getByTestId"]>, testInfo: TestInfo, name: string) {
    if (process.env.DREAMYO_VISUAL_CAPTURE !== "1") return;
