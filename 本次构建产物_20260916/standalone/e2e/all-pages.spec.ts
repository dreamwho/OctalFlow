...
const BASE_URL = `http://127.0.0.1:${Number(process.env.DREAMYO_E2E_PORT || 3100)}`;
const MISSING_CREATOR_PATH = "/u/this-creator-does-not-exist";
const USES_POSTGRES = Boolean(process.env.DREAMYO_E2E_DATABASE_URL?.trim());
const CHATGPT_RUNTIME_DISABLED = process.env.DREAMYO_CHATGPT_API_ENABLED === "0";
...
test("all authenticated pages reach their real routes and stay usable", async ({ page, request }, testInfo) => {
    ...
    const routes: RouteCase[] = [
        ...
        { path: MISSING_CREATOR_PATH, expectedStatus: 200, readyText: USES_POSTGRES ? "404" : "创作者主页暂不可用" },
...
test("signed-out, legal, installation and invalid public detail routes fail safely", async ({ browser }, testInfo) => {
    ...
    try {
        ...
        const routes: RouteCase[] = [
            { path: "/", readyHeading: "把灵感，变成作品" },
            { path: "/login", readyHeading: "登录 dreamyo" },
            { path: "/register", readyHeading: "注册 dreamyo" },
            ...
            { path: "/announcements", readyHeading: "网站公告" },
            { path: MISSING_CREATOR_PATH, expectedStatus: 200, readyText: USES_POSTGRES ? "404" : "创作者主页暂不可用" },
            { path: "/share/not-a-real-public-work", expectedStatus: 200, readyText: USES_POSTGRES ? "404" : "作品分享暂不可用" },
...
function isExpectedFileProviderLimitation(failure: ApiFailure) {
    // The missing-creator route intentionally probes this API. The browser may
    // consume the response body before the response listener reads it, so the
    // path/status contract is the stable assertion for this expected 404.
    if (failure.status === 404 && failure.path === `/api/public/users/${E2E_ADMIN.username}`) return true;
    // The visual matrix intentionally runs without the optional ChatGPT sidecar.
    // Its admin surface must render the unavailable state instead of turning a
    // documented 503 into a page failure.
    if (CHATGPT_RUNTIME_DISABLED && failure.status === 503 && (failure.path.startsWith("/api/admin/generic-proxy/") || failure.path.startsWith("/api/admin/chatgpt-api/"))) return true;
    // Rapid route/theme changes can leave a previous image stream in flight. The
    // local media guard returns 429 by design; keep that protection visible in
    // the evidence while excluding it from the route shell assertion. Reference
    // previews use the same bounded local stream and are treated identically.
    if (failure.status === 429 && (failure.path.startsWith("/api/generation-log-assets/") || failure.path.startsWith("/api/reference-assets/"))) return true;
    if (USES_POSTGRES || (failure.status !== 409 && failure.status !== 501)) return false;
    // The file provider deliberately exposes unsupported commerce/community
    // operations as 409/501. Playwright may have already consumed the body
    // while the page rendered, so the status/path contract is the stable
    // evidence for this isolated provider limitation.
    return failure.body.includes("需要启用 PostgreSQL") || FILE_PROVIDER_LIMITATIONS.get(failure.path) === failure.status || failure.status === 501;
}
