...
try {
    ...
    const dataDir = path.resolve(process.env.DREAMYO_DATA_DIR || path.join(process.cwd(), ".data"));
    ...
    await runPostgresTool(process.env.DREAMYO_PG_DUMP_PATH || "pg_dump", ["--format=custom", "--compress=6", "--no-owner", "--no-privileges", "--file", safetyDatabasePath], databaseUrl);
    ...
    await runPostgresTool(
        process.env.DREAMYO_PG_RESTORE_PATH || "pg_restore",
    ...
    const objectStorageConfig = await loadDisasterObjectStorageConfig({ databaseUrl, dataDir, encryptionKey: process.env.DREAMYO_ENCRYPTION_KEY });
...
async function restoreLocalMedia(recoveryPointDir, dataDir, manifest) {
    const stagingDir = path.join(path.dirname(dataDir), `.dreamyo-restore-${manifest.recoveryPointId}`);
    const rollbackDir = path.join(path.dirname(dataDir), `.dreamyo-restore-rollback-${manifest.recoveryPointId}`);
