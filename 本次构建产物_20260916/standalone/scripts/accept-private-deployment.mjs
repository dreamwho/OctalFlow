...
const directory = await mkdtemp("/private/tmp/dreamyo-private-acceptance-");
await chmod(directory, 0o700);
await cp(path.resolve(packageDirectory), directory, { recursive: true, mode: constants.COPYFILE_FICLONE });
await writeFile(
    path.join(directory, ".env"),
    ["COMPOSE_PROJECT_NAME=dreamyo-private-acceptance", "PORT=8866", "DATABASE_URL=postgres://migration_test:migration_fixture_only@127.0.0.1:55439/migration_docker", "NEXT_PUBLIC_SITE_URL=http://127.0.0.1:8866"].join("\n") + "\n",
    { mode: 0o600 },
);
console.log(`Acceptance workspace: ${directory}`);
execFileSync("docker", ["exec", "dreamyo-migration-test-pg-20260908", "createdb", "-U", "migration_test", "migration_docker"], { stdio: "inherit" });
...
for (const pass of ["initial", "repeat"]) {
    ...
    const child = spawn(
        ...
        [
            ...
            "host",
            "-e",
            "DREAMYO_ALLOW_PLATFORM_EMULATION=1",
            "-v",
