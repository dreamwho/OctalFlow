...
async function handlePaymentRequest({ request, response, url, body, options }) {
    ...
    if (url.pathname === "/payply/refund") {
        const refundPayload = parseObject(body.toString("utf8"));
        const refundKey =
            String(refundPayload.refundRequestNo || refundPayload.orderNo || refundPayload.orderId || "fixture")
                .replace(/[^a-zA-Z0-9_-]/g, "")
                .slice(-80) || "fixture";
        return sendJson(response, 200, { data: { status: "success", refundId: `payply_refund_fixture_${refundKey}` } });
    }
...
if (import.meta.url === pathToFileURL(process.argv[1] || "").href) {
    const port = Number(process.env.DREAMYO_PAYMENT_FIXTURE_PORT) || 4020;
    const host = process.env.DREAMYO_PAYMENT_FIXTURE_HOST || "127.0.0.1";
    const fixture = createPaymentFixtureServer({ alipayPrivateKey: process.env.DREAMYO_PAYMENT_FIXTURE_ALIPAY_PRIVATE_KEY });
