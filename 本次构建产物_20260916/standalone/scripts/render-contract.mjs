...
export function validateRenderBlueprint({ repoRoot, source, dockerfile: dockerfileSource }) {
    ...
    const web = services.find((service) => service?.name === "dreamyo");
    const worker = services.find((service) => service?.name === "dreamyo-generation-worker");
    const database = databases.find((entry) => entry?.name === "dreamyo-postgres");
    const runtimeGroup = environmentGroups.find((group) => group?.name === "dreamyo-runtime");
    ...
    ensure(hasGroup(web?.envVars, "dreamyo-runtime") && hasGroup(worker?.envVars, "dreamyo-runtime"), "Web 与 Worker 必须引用同一运行时环境组");
    ensure(environmentMap(runtimeGroup?.envVars).DREAMYO_WORKER_TOKEN?.generateValue === true, "运行时环境组必须生成共享 Worker 令牌");
    ensure(webEnvironment.DREAMYO_DATA_DIR?.value === "/app/web/.data", "Web 数据目录必须位于持久盘");
    ensure(webEnvironment.DATABASE_URL?.fromDatabase?.name === "dreamyo-postgres", "Web 必须引用 Blueprint PostgreSQL");
    ensure(webEnvironment.DREAMYO_ENCRYPTION_KEY?.generateValue === true, "Web 必须生成稳定加密密钥");
    ensure(webEnvironment.DREAMYO_INSTALL_TOKEN?.generateValue === true, "Web 必须生成一次性安装令牌");
    ensure(webEnvironment.DREAMYO_MAINTENANCE_TOKEN?.generateValue === true, "Web 必须生成独立维护令牌");
    ensure(!workerEnvironment.DREAMYO_INSTALL_TOKEN, "Render Worker 不得获得一次性安装令牌");
    ensure(!workerEnvironment.DREAMYO_MAINTENANCE_TOKEN, "Render Worker 不得获得外部维护令牌");
    ensure(workerEnvironment.DREAMYO_WORKER_API_ORIGIN?.fromService?.name === "dreamyo" && workerEnvironment.DREAMYO_WORKER_API_ORIGIN?.fromService?.property === "hostport", "Worker 必须通过 Render 私网调用 Web");
    ensure(!workerEnvironment.DATABASE_URL && !workerEnvironment.DREAMYO_DATABASE_PROVIDER, "Render Worker 不应直接持有数据库配置");
    ensure(database?.plan === "basic-256mb" && database?.databaseName === "dreamyo" && database?.user === "dreamyo", "Render PostgreSQL 必须使用持久生产实例和稳定名称");
