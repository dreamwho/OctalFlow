...
const originalEncryptionKey = process.env.DREAMYO_ENCRYPTION_KEY;
const originalDatabaseProvider = process.env.DREAMYO_DATABASE_PROVIDER;

afterEach(() => {
    if (originalEncryptionKey === undefined) delete process.env.DREAMYO_ENCRYPTION_KEY;
    else process.env.DREAMYO_ENCRYPTION_KEY = originalEncryptionKey;
    if (originalDatabaseProvider === undefined) delete process.env.DREAMYO_DATABASE_PROVIDER;
    else process.env.DREAMYO_DATABASE_PROVIDER = originalDatabaseProvider;
});
...
describe("PostgreSQL auth read paths", () => {
    it("rejects full-database auth mutations for PostgreSQL", async () => {
        process.env.DREAMYO_DATABASE_PROVIDER = "postgres";
    ...
    it("decrypts system channel API keys and webhook secrets on the settings fast path", async () => {
        process.env.DREAMYO_ENCRYPTION_KEY = "31".repeat(32);
        ...
        expect(settings.systemChannels[0].apiKey).not.toContain("dreamyo-secret:v1:");
        expect(settings.systemChannels[0].webhookSecret).toBe("0123456789abcdef0123456789abcdef");
        expect(settings.systemChannels[0].webhookSecret).not.toContain("dreamyo-secret:v1:");
    });
