...
describe("text task runtime recovery", () => {
    ...
    it("preserves maintenance authorization for the internal system proxy", () => {
        ...
        expect(headers.get("x-dreamyo-logical-model")).toBe("text-model");
        expect(headers.get("x-dreamyo-points-idempotency-key")).toBe("text-task:test:attempt:1");
    });
    ...
    it("refunds a zero-point recorded charge when the upstream task fails", async () => {
        const headers = { "x-dreamyo-points-cost": "0", "x-dreamyo-points-record-id": "record-zero" };
        const fetchMock = vi
            .fn()
