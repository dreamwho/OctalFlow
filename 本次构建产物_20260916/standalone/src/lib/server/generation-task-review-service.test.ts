...
describe("generation task manual review", () => {
    ...
    it("resumes an existing reviewed image task without replacing its upstream ID", async () => {
        ...
        expect(mocks.schedule).toHaveBeenCalledWith("image", "image-one", expect.objectContaining({ executionPhase: "submitted", upstreamTaskId: "upstream-existing", lastUpstreamStatus: "manual_review_resumed" }));
    });
