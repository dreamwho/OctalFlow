...
export async function createPublicPromptImage(pathValue: string | null, widthValue: string | null) {
    ...
    return getOrCreateCachedImageVariant(`prompt:${COMMIT}:${imagePath}:${width}`, async () => {
        const response = await fetchSafeOutbound(
            `https://raw.githubusercontent.com/${REPOSITORY}/${COMMIT}/${imagePath}`,
            {
                headers: { Accept: "image/avif,image/webp,image/png,image/jpeg", "User-Agent": "DREAMYO prompt image proxy" },
                cache: "force-cache",
