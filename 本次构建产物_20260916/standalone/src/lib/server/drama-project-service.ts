...
import { listDramaRenderTasksForProject } from "@/lib/server/drama-render-store";
...
export async function getDramaProjectForUser(userId: string, id: string) {
    ...
    const renderTasks = await listDramaRenderTasksForProject(userId, project.id);
    if (!renderTasks.length) return project;
    const fallbackEpisodeId = project.activeEpisodeId || project.episodes[0]?.id;
    const latestByEpisode = new Map<string, (typeof renderTasks)[number]>();
    for (const task of renderTasks) {
        const episodeId = task.episodeId || fallbackEpisodeId;
        if (episodeId && !latestByEpisode.has(episodeId)) latestByEpisode.set(episodeId, task);
    }
    return {
        ...project,
        episodes: project.episodes.map((episode) => {
            const task = latestByEpisode.get(episode.id);
            if (!task) return episode;
            return { ...episode, renderTask: { id: task.id, episodeId: episode.id, status: task.status, result: task.result, error: task.error } };
        }),
    };
}
