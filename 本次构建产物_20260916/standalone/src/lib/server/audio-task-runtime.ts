...
export async function createAudioTaskUpstreamStep(task: AudioTask, origin: string, cookie = "", workerUserId = ""): Promise<AudioUpstreamStep> {
    ...
    for (const [index, config] of candidates.entries()) {
        ...
        const minimaxLog =
            config.advancedConfig?.protocol === "minimax-audio" || config.advancedConfig?.protocol === "aliyun-bailian-audio" || config.advancedConfig?.protocol === "tencent-tokenhub-music"
                          ...
                          lifecycle: [
                              {
                                  ...
                                  phase: "running",
                                  message:
                                      config.advancedConfig?.protocol === "aliyun-bailian-audio" ? "已提交阿里云百炼音频请求" : config.advancedConfig?.protocol === "tencent-tokenhub-music" ? "已提交腾讯云 TokenHub 音乐请求" : "已提交 MiniMax 音频请求",
                              },
                          ],
        ...
        try {
            ...
            if (minimaxLog)
                await updateMiniMaxRequestLog(minimaxLog.id, {
                    statusCode: 0,
                    durationMs: Date.now() - new Date(minimaxLog.createdAt).getTime(),
                    phase: "running",
                    requestPreview: JSON.stringify({ model: config.model, mode: config.audioMode || "tts", request: payload }),
                    lifecycle: [
                        {
                            at: new Date().toISOString(),
                            phase: "running",
                            message:
                                config.advancedConfig?.protocol === "tencent-tokenhub-music" ? "开始调用腾讯云 TokenHub 音乐接口" : config.advancedConfig?.protocol === "aliyun-bailian-audio" ? "开始调用阿里云百炼音频接口" : "开始调用 MiniMax 音频接口",
                        },
                    ],
                });
            ...
            if (minimaxLog)
                await updateMiniMaxRequestLog(minimaxLog.id, {
                    ...
                    durationMs: Date.now() - new Date(minimaxLog.createdAt).getTime(),
                    phase: response.ok ? "running" : "failed",
                ...
                if (completed?.status === "success") {
                    ...
                    if (minimaxLog)
                        await updateMiniMaxRequestLog(minimaxLog.id, {
                            ...
                            durationMs: Date.now() - new Date(minimaxLog.createdAt).getTime(),
                            phase: "success",
                            responsePreview: "音频已返回并持久化",
                            lifecycle: [{ at: new Date().toISOString(), phase: "success", message: "音频已生成" }],
                        });
                    ...
                    if (completed?.status === "success") {
                        ...
                        if (minimaxLog)
                            await updateMiniMaxRequestLog(minimaxLog.id, {
                                ...
                                durationMs: Date.now() - new Date(minimaxLog.createdAt).getTime(),
                                phase: "success",
                                responsePreview: "MiniMax 十六进制音频已解码并持久化",
                                lifecycle: [{ at: new Date().toISOString(), phase: "success", message: "MiniMax 音频已完成" }],
                            });
                        return { state: "completed" };
                    }
                    return { state: "failed", status: completed?.status || "cancelled", error: completed?.error || "任务已取消" };
                    ...
                    if (completed?.status === "success") {
                        ...
                        if (minimaxLog)
                            await updateMiniMaxRequestLog(minimaxLog.id, {
                                ...
                                durationMs: Date.now() - new Date(minimaxLog.createdAt).getTime(),
                                phase: "success",
                                responsePreview: "阿里云百炼音频数据已解码并持久化",
                                lifecycle: [{ at: new Date().toISOString(), phase: "success", message: "音频已生成" }],
                            });
                        return { state: "completed" };
                    }
            ...
            if (minimaxLog)
                await updateMiniMaxRequestLog(minimaxLog.id, {
                    statusCode: minimaxResponseStatus || (error instanceof GenerationSubmissionSafeFailure ? error.status || 502 : 0),
                    durationMs: Date.now() - new Date(minimaxLog.createdAt).getTime(),
                    phase: "failed",
                    error: error instanceof Error ? error.message.slice(0, 500) : "MiniMax 音频请求失败",
                    lifecycle: [{ at: new Date().toISOString(), phase: "failed", message: error instanceof Error ? error.message.slice(0, 200) : "MiniMax 音频请求失败" }],
                }).catch(() => undefined);
...
async function completeAudioTask(task: AudioTask, url: string, mimeType: string, settleRequestLog = true) {
    ...
    if ((completed.config.advancedConfig?.protocol === "minimax-audio" && completed.config.model.startsWith("music-")) || (completed.config.advancedConfig?.protocol === "tencent-tokenhub-music" && completed.config.model === "minimax-music-v3.0")) {
        await saveMiniMaxMusicRecord({
            ...
            name: completed.prompt.slice(0, 80) || "MiniMax 音乐",
            model: completed.config.model,
            prompt: completed.config.instructions || completed.prompt,
            lyrics: completed.config.lyrics || "",
            resultUrl: url,
            status: "success",
            metadata: { format: completed.config.format || "mp3", provider: completed.config.advancedConfig?.protocol === "tencent-tokenhub-music" ? "tencent-tokenhub" : "minimax" },
        }).catch((error) => console.error("MiniMax music record failed", error));
    }
    return completed;
}
...
function readBilling(headers: Headers) {
    const raw = headers.get("x-dreamyo-points-cost");
    const value = raw === null ? undefined : Number(raw);
    return { pointsCost: value !== undefined && Number.isFinite(value) && value >= 0 ? value : undefined, pointsRecordId: headers.get("x-dreamyo-points-record-id") || undefined };
}
