...
function buildNodeInfoList(app: RunningHubApp, prompt: string, uploadedNames: string[], preferences: { size?: string; quality?: string }) {
    ...
    return app.fields.map((field) => {
        ...
        if (preferences.quality && preferences.quality !== "auto" && /resolution|quality|画质|分辨率/i.test(`${field.fieldName} ${field.label}`))
            fieldValue = ({ low: "1k", medium: "2k", high: "4k" } as Record<string, string>)[preferences.quality] || preferences.quality;
