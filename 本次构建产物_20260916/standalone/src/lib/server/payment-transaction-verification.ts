...
async function queryStripePayment(order: BillingOrderRecord, payment: VerifiedPaymentTransaction, config: PaymentRuntimeConfig): Promise<VerifiedPaymentTransaction | null> {
    const secret = requiredConfig(config, "DREAMYO_STRIPE_SECRET_KEY", "STRIPE_SECRET_KEY");
    ...
    return {
        status,
        orderId: clean(readPath(payload, "metadata.orderId") || readPath(payload, "metadata.dreamyoProOrderId") || readPath(object, "metadata.orderId") || readPath(object, "metadata.dreamyoProOrderId") || payload.client_reference_id, 120),
        orderNo: clean(readPath(payload, "metadata.orderNo") || readPath(payload, "metadata.dreamyoProOrderNo") || readPath(object, "metadata.orderNo") || readPath(object, "metadata.dreamyoProOrderNo"), 120),
...
async function queryAlipayPayment(order: BillingOrderRecord, payment: VerifiedPaymentTransaction, config: PaymentRuntimeConfig): Promise<VerifiedPaymentTransaction> {
    const appId = requiredConfig(config, "DREAMYO_ALIPAY_APP_ID");
    const privateKey = loadPrivateKey(config, "DREAMYO_ALIPAY_PRIVATE_KEY", "DREAMYO_ALIPAY_PRIVATE_KEY_PATH");
    const gateway = getPaymentRuntimeEnv(config, "DREAMYO_ALIPAY_GATEWAY_URL") || "https://openapi.alipay.com/gateway.do";
    ...
    if (!sign || !signContent || !verifyRsaSha256(signContent, sign, loadPaymentPublicKey(config, "DREAMYO_ALIPAY_PUBLIC_KEY", "DREAMYO_ALIPAY_PUBLIC_KEY_PATH"))) throw new BillingInputError("支付宝交易查询响应验签失败", 502);
...
async function queryPayplyPayment(order: BillingOrderRecord, payment: VerifiedPaymentTransaction, config: PaymentRuntimeConfig): Promise<VerifiedPaymentTransaction | null> {
    const template = getPaymentRuntimeValue(config, "DREAMYO_PAYPLY_QUERY_URL", "PAYPLY_QUERY_URL");
    ...
    const status = String(readConfiguredPath(config, payload, "DREAMYO_PAYPLY_QUERY_STATUS_FIELD", ["status", "tradeStatus", "paymentStatus", "data.status", "data.tradeStatus"]) || "").toLowerCase();
    return {
        ...
        orderId: clean(readConfiguredPath(config, payload, "DREAMYO_PAYPLY_QUERY_ORDER_ID_FIELD", ["orderId", "data.orderId", "metadata.orderId"]), 120),
        orderNo: clean(readConfiguredPath(config, payload, "DREAMYO_PAYPLY_QUERY_ORDER_NO_FIELD", ["orderNo", "outTradeNo", "out_trade_no", "data.orderNo"]), 120),
        providerTradeId: clean(readConfiguredPath(config, payload, "DREAMYO_PAYPLY_QUERY_TRADE_ID_FIELD", ["providerTradeId", "tradeId", "trade_no", "transactionId", "data.tradeId"]), 160),
        providerPaymentId: clean(readConfiguredPath(config, payload, "DREAMYO_PAYPLY_QUERY_PAYMENT_ID_FIELD", ["providerPaymentId", "paymentId", "payment_id", "transactionId", "data.paymentId"]), 160),
        amountCents:
            optionalInteger(readConfiguredPath(config, payload, "DREAMYO_PAYPLY_QUERY_AMOUNT_CENTS_FIELD", ["amountCents", "data.amountCents"])) ??
            decimalToCents(readConfiguredPath(config, payload, "DREAMYO_PAYPLY_QUERY_AMOUNT_FIELD", ["amount", "totalAmount", "data.amount"])),
        currency: currency(readConfiguredPath(config, payload, "DREAMYO_PAYPLY_QUERY_CURRENCY_FIELD", ["currency", "data.currency"])),
        paidAt: optionalIso(readConfiguredPath(config, payload, "DREAMYO_PAYPLY_QUERY_PAID_AT_FIELD", ["paidAt", "successTime", "data.paidAt"])),
        rawPayload: sanitizeJson(payload),
    };
}
...
function verifyWechatResponse(raw: string, headers: Headers, config: PaymentRuntimeConfig) {
    ...
    return verifyRsaSha256(`${timestamp}\n${nonce}\n${raw}\n`, signature, loadPaymentPublicKey(config, "DREAMYO_WECHAT_PAY_PLATFORM_PUBLIC_KEY", "DREAMYO_WECHAT_PAY_PLATFORM_PUBLIC_KEY_PATH"));
}
