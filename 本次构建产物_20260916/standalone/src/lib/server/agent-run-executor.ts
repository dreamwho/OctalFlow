...
function applyRequestedMediaModels(plan: AgentPlan, selectedModels: Array<ReturnType<typeof agentModelOptions>[number]>, requestPrompt: string, referencedAssetIds: string[]): AgentPlan {
    ...
    return {
        ...
        intent: "generation",
        deliverables: [
            ...deliverables.map((deliverable) => (deliverable.type === "video" && prerequisiteImageIds.length ? { ...deliverable, dependencies: Array.from(new Set([...(deliverable.dependencies || []), ...prerequisiteImageIds])) } : deliverable)),
