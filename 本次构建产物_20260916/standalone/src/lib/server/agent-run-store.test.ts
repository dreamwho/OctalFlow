...
describe("createAgentRun Canvas snapshot", () => {
    ...
    it("persists the explicitly selected Skill ids on the public user message", async () => {
        ...
        expect(mocks.createCreativeRunBundle).toHaveBeenCalledWith("user", expect.objectContaining({ userMetadata: { selectedSkillIds: ["skill-character-casting"] } }));
    });
    ...
    it("settles active Canvas tasks and emits terminal node operations when cancelled", async () => {
        ...
        expect(mutation).toMatchObject({
            event: {
                type: "run.cancelled",
                data: {
                    ops: [{ type: "update_node", id: "output-run-0-0", metadata: { status: "cancelled", agentTaskStatus: "cancelled", errorDetails: "任务已取消" } }],
                },
            },
        });
    });
    ...
    it("marks only the failed child output and keeps successful sibling assets", async () => {
        ...
        expect(events[1]).toMatchObject({
            data: {
                ...
                ops: [{ type: "update_node", id: "output-run-0-1", metadata: { status: "error", errorDetails: "上游拒绝" } }],
            },
        });
    });
