...
export class RunningHubRepository {
    ...
    async updateSettings(settings: StoredRunningHubSettings) {
        const result = await this.db.query("UPDATE runninghub_settings SET enabled=$1,api_base_url=$2,api_key_ciphertext=$3,instance_type=$4 WHERE id='default' RETURNING *", [
            settings.enabled,
            settings.apiBaseUrl,
    ...
    async upsertTask(task: RunningHubTask) {
        const result = await this.db.query(
            ...
            [
                ...
                task.status,
                task.error || null,
                JSON.stringify(task.resultUrls),
