...
describe("checkRateLimit", () => {
    ...
    it("only trusts the configured number of proxy hops", () => {
        const previous = process.env.DREAMYO_TRUSTED_PROXY_HOPS;
        ...
        process.env.DREAMYO_TRUSTED_PROXY_HOPS = "2";
        ...
        if (previous === undefined) delete process.env.DREAMYO_TRUSTED_PROXY_HOPS;
        else process.env.DREAMYO_TRUSTED_PROXY_HOPS = previous;
    });
    ...
    it("allows private upstreams only through an explicit exact host allowlist", async () => {
        vi.stubEnv("DREAMYO_ALLOW_PRIVATE_UPSTREAMS", "");
        vi.stubEnv("DREAMYO_PRIVATE_UPSTREAM_HOSTS", "");
        ...
        vi.stubEnv("DREAMYO_PRIVATE_UPSTREAM_HOSTS", "127.0.0.1, provider.internal");
