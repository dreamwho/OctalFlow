...
const previousKey = process.env.DREAMYO_REFERENCE_ASSET_SIGNING_KEY;

afterEach(() => {
    if (previousKey === undefined) delete process.env.DREAMYO_REFERENCE_ASSET_SIGNING_KEY;
    else process.env.DREAMYO_REFERENCE_ASSET_SIGNING_KEY = previousKey;
});

describe("reference asset access", () => {
    it("creates and verifies a bounded signed server URL", () => {
        process.env.DREAMYO_REFERENCE_ASSET_SIGNING_KEY = "test-signing-key";
        const now = Date.UTC(2026, 6, 19);
        const url = new URL(createSignedReferenceAssetUrl("temporary/2026/07/19/images/file.png", "https://dreamyo.example", now));
        const purpose = url.searchParams.get("purpose");

        expect(url.origin).toBe("https://dreamyo.example");
    ...
    it("only signs local reference asset paths", () => {
        process.env.DREAMYO_REFERENCE_ASSET_SIGNING_KEY = "test-signing-key";
        expect(signReferenceAssetInputUrl("https://cdn.example/image.png", "https://dreamyo.example")).toBe("https://cdn.example/image.png");
        expect(signReferenceAssetInputUrl("/api/reference-assets/permanent/2026/07/19/images/file.png", "https://dreamyo.example")).toContain("purpose=provider-read");
    });
});
