                ...
                try {
                    ...
                    const call = await requestStructuredText({
                        ...
                        messages: [
                            ...
                            {
                                role: "user",
                                content: [{ type: "text", text: JSON.stringify(videoEvidenceContext(probe, evidence)) }, ...evidence.media],
                            },
                        ],
...
function analysisInstruction(evidence: CanvasVideoEvidence) {
    const audioInstruction = evidence.audioEvidence === "none" ? "本轮未传入可听音频证据；音频字段必须明确写未传递或无法判断，不得编造对白、歌词、音乐类型或声音事件。" : "音频描述只能写实际可辨认的内容，听不清时明确标注不确定。";
...
function requiredTextArray(value: unknown) {
    const values = array(value)
        .map((item) => (typeof item === "string" ? publicText(item) : ""))
        .filter(Boolean);
