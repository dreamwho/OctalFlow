...
export async function listRunningHubApps(input: { binding?: string; enabledOnly?: boolean } = {}) {
    ...
    return db.apps.filter((app) => (!input.enabledOnly || app.enabled) && (!input.binding || app.featureBindings.includes(input.binding))).sort((left, right) => left.sortOrder - right.sortOrder || right.updatedAt.localeCompare(left.updatedAt));
}
        ...
        return [
            {
                ...
                label: String(field.label || fieldName)
                    ...
                    .slice(0, 160),
                type,
                defaultValue: String(field.defaultValue ?? "").slice(0, 5000),
                options: Array.isArray(field.options) ? field.options.map(String).slice(0, 100) : [],
            },
        ];
    });
}
