...
export function pointsResponseHeaders(value: unknown) {
    ...
    setNumberHeader(headers, "x-dreamyo-points-remaining", points.pointsBalance);
    setNumberHeader(headers, "x-dreamyo-points-permanent", points.permanentPointsBalance);
    setNumberHeader(headers, "x-dreamyo-points-daily", points.dailyPointsBalance);
    if (typeof points.dailyPointsExpiresAt === "string" && points.dailyPointsExpiresAt) headers.set("x-dreamyo-points-daily-expires-at", points.dailyPointsExpiresAt);
    return headers;
}
