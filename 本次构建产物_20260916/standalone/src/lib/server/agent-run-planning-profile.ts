...
export function resolveAgentPlanningProfile(run: PlanningRun): AgentPlanningProfile {
    ...
    const capabilities = run.generationPreferences?.mode ? new Set(["text", run.generationPreferences.mode, ...(run.generationPreferences.mode === "video" ? ["image"] : [])]) : inferCapabilities(run.surface, prompt, selectedTypes);
