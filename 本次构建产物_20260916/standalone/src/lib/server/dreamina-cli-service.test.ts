...
describe("Dreamina CLI service", () => {
    ...
    it("marks a confirmed account submit lease collision as safely deferred", async () => {
        ...
        await expect(submitDreaminaCliTaskWithCreditObservation({ command: "text2image", modelId: "dreamina-seedream-5-0", prompt: "测试", resolutionType: "2k" }, { taskId: "waiting-task", runner: vi.fn() })).rejects.toMatchObject({
            status: 409,
