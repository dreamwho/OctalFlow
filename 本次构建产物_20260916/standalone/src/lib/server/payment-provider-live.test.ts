...
describe("payment providers over a live compatible HTTP fixture", () => {
    it("creates Stripe, Alipay, WeChat and PayPly checkouts", async () => {
        const stripe = await createProviderCheckout("stripe", order, { origin: "https://app.test" }, config({ DREAMYO_STRIPE_SECRET_KEY: "stripe-secret", DREAMYO_STRIPE_API_BASE: `${origin}/stripe` }));
        ...
        const payply = await createProviderCheckout("payply", { ...order, provider: "payply" }, { origin: "https://app.test" }, config({ DREAMYO_PAYPLY_API_KEY: "payply-secret", DREAMYO_PAYPLY_CHECKOUT_URL: `${origin}/payply/checkout` }));
        ...
        expect(fixture.requests[0]?.headers["idempotency-key"]).toBe("dreamyo-checkout-order-live");
    ...
    it("creates Stripe, Alipay, WeChat and PayPly refunds", async () => {
        setRuntimeConfig({ DREAMYO_STRIPE_SECRET_KEY: "stripe-secret", DREAMYO_STRIPE_API_BASE: `${origin}/stripe` });
        ...
        setRuntimeConfig({ DREAMYO_PAYPLY_API_KEY: "payply-secret", DREAMYO_PAYPLY_REFUND_URL: `${origin}/payply/refund` });
        ...
        expect(alipay).toMatchObject({ status: "succeeded", providerRefundId: "dreamyo-refund-order-live" });
        expect(wechat).toMatchObject({ status: "succeeded", providerRefundId: "wx_refund_fixture" });
        expect(payply).toMatchObject({ status: "succeeded" });
        expect(payply.providerRefundId).toMatch(/^payply_refund_fixture_/);
        ...
        expect(fixture.requests[0]?.headers["idempotency-key"]).toBe("dreamyo-refund-order-live");
        expect(fixture.requests[2]?.headers.authorization).toContain('mchid="1900000001"');
        expect(fixture.requests[3]?.headers["idempotency-key"]).toBe("dreamyo-refund-order-live");
    });
});
...
function alipayConfig(): PaymentRuntimeConfig {
    return config({
        ...
        DREAMYO_ALIPAY_GATEWAY_URL: `${origin}/alipay/gateway.do`,
    });
}

function wechatConfig(): PaymentRuntimeConfig {
    return config({
        DREAMYO_WECHAT_PAY_APP_ID: "wx-fixture-app",
        DREAMYO_WECHAT_PAY_MCH_ID: "1900000001",
        DREAMYO_WECHAT_PAY_CERT_SERIAL_NO: "fixture-serial",
        DREAMYO_WECHAT_PAY_PRIVATE_KEY: wechatPrivateKey,
        DREAMYO_WECHAT_PAY_PLATFORM_PUBLIC_KEY: wechatPlatformPublicKey,
        DREAMYO_WECHAT_PAY_API_BASE: `${origin}/wechat`,
    });
}
