...
const runtime = globalThis as typeof globalThis & { __dreamyoProGenerationWorkerHeartbeats?: Map<string, number> };
const fileHeartbeats = (runtime.__dreamyoProGenerationWorkerHeartbeats ??= new Map<string, number>());
...
export async function getGenerationWorkerHealth(now = Date.now()) {
    const staleAfterMs = normalizeGenerationWorkerStaleMs(process.env.DREAMYO_GENERATION_WORKER_STALE_MS);
