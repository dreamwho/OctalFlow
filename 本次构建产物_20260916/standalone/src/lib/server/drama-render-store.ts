...
import { createStoredGenerationTask, getStoredGenerationTask, queryStoredGenerationTasks, touchStoredGenerationTask, transitionStoredGenerationTask } from "./generation-task-store";
...
export async function createDramaRenderTask(input: Pick<DramaRenderTask, "userId" | "projectId" | "conversationId" | "title" | "episodeId">) {
    const now = Date.now();
...
export async function listDramaRenderTasksForProject(userId: string, projectId: string) {
    return queryStoredGenerationTasks<DramaRenderTask>("render", { userId, projectId, surface: "drama", limit: 100 });
}
