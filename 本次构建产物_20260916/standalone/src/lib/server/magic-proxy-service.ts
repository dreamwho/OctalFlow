...
async function requestGroupDelays(runtime: MihomoRuntimeConfig, targetUrl: string, timeoutMs: number): Promise<Map<string, number>> {
    const response = await controllerRequest(runtime, `/group/${encodeURIComponent(GROUP_NAMES.geminiai)}/delay?url=${encodeURIComponent(targetUrl)}&timeout=${timeoutMs}`, { method: "GET", signal: AbortSignal.timeout(timeoutMs + 5000) });
