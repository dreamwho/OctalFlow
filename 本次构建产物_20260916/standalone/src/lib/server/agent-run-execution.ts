...
import {
    agentRunTasksCompleted,
    agentRunTasksTerminal,
    agentTaskHasPendingChildren,
    getAgentRun,
    updateAgentRunById,
    updateAgentRunTaskById,
    type AgentRun,
    type AgentRunChildTask,
    type AgentRunReference,
    type AgentRunTask,
} from "@/lib/server/agent-run-store";
...
export function assertAgentSkillModelConstraints(skills: Awaited<ReturnType<typeof getAuthSettings>>["agentSkills"], selectedSkillIds: readonly string[] | undefined, capability: LogicalModelCapability, protocol: unknown) {
    const required = skills.filter((skill) => (selectedSkillIds || []).includes(skill.id) && skill.modelConstraints?.capability === capability && skill.modelConstraints.requiredModelFamilies?.includes("minimax-h3"));
...
export function readFunctionCallResult(argumentsText: string, headers: Headers, protocol?: AgentFunctionCallResult["protocol"], elapsedMs?: number): AgentFunctionCallResult {
    const pointsRemaining = Number(headers.get("x-dreamyo-points-remaining"));
    return {
