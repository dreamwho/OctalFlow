...
async function recoverAgentRunChildren(stored: AgentRun) {
    ...
    const recovered = await mutateCreativeRun<AgentRun>(
        ...
        (current) => {
            ...
            const tasks = current.tasks.map((task) => (agentTaskHasPendingChildren(task) && task.status !== "running" ? { ...task, status: "running" as const, completedAt: undefined, ...(task.status === "completed" ? { error: undefined } : {}) } : task));
            return {
...
export async function updateAgentRunById(
    ...
    return mutateCreativeRun<AgentRun>(
        ...
        (current) => {
            ...
            const terminalCanvasEvent =
                current.surface === "canvas" && event && ["run.completed", "run.failed", "run.cancelled"].includes(event.type)
                    ? { ...event, data: { ...(event.data && typeof event.data === "object" ? (event.data as Record<string, unknown>) : {}), recoveryOps: agentRunRecoveryOps(next) } }
                    : event;
