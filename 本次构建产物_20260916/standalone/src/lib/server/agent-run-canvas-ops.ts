...
export function layoutCanvasAgentTasks(tasks: AgentRunTask[], snapshot: unknown): AgentRunTask[] {
    ...
    const referencedNodes = Array.from(referencedNodeIds)
        .map((id) => nodes.get(id))
        ...
        ops: [
            {
                ...
                metadata: cancelled
                    ? {
                          ...outputMetadata(runId, task, "error"),
                          status: "cancelled",
                          agentTaskStatus: "cancelled",
                          errorDetails: child?.error || "任务已取消",
                          agentTaskSubmittedParameters: task.submittedParameters,
                          agentGenerationTaskIds: child?.id ? [child.id] : [],
                      }
                    : failed
