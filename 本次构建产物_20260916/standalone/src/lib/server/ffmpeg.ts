...
export async function ffmpegFilterAvailable(filterName: string) {
    const normalized = filterName.trim();
    if (!normalized) return false;
    try {
        const result = await runFfmpeg(["-hide_banner", "-filters"], { timeoutMs: 10_000 });
        return `${result.stdout}\n${result.stderr}`.split(/\r?\n/).some((line) => line.trim().split(/\s+/)[1] === normalized);
    } catch {
        return false;
    }
}
