...
const globalForInstallStatus = globalThis as typeof globalThis & {
    __dreamyoProInstallStatusCache?: {
        key: string;
        value?: InstallStatus;
        expiresAt: number;
...
export async function getInstallStatus(): Promise<InstallStatus> {
    ...
    const now = Date.now();
    const cached = globalForInstallStatus.__dreamyoProInstallStatusCache;
    if (cached?.key === key) {
    ...
    globalForInstallStatus.__dreamyoProInstallStatusCache = { key, expiresAt: 0, pending };
    try {
        const value = await pending;
        const ttl = value.firstAdminRequired ? 0 : value.ready ? READY_CACHE_TTL_MS : UNHEALTHY_CACHE_TTL_MS;
        globalForInstallStatus.__dreamyoProInstallStatusCache = { key, value: ttl ? value : undefined, expiresAt: ttl ? Date.now() + ttl : 0 };
        return value;
    } catch (error) {
        globalForInstallStatus.__dreamyoProInstallStatusCache = undefined;
        throw error;
    }
}
    ...
    if (!getPostgresConnectionString()) {
        return buildStatus({
            ...
            database: {
                ...
                detail: "本机部署通常是 postgres://用户名:密码@localhost:5432/dreamyo；Docker Compose 通常是 postgres://用户名:密码@postgres:5432/dreamyo。",
            },
        });
    }
...
export async function initializeInstallDatabase(installToken: unknown) {
    ...
    if (!getEncryptionKeyStatus().ready) throw new InstallInitializationError("请先配置有效的 DREAMYO_ENCRYPTION_KEY", 400);
    try {
