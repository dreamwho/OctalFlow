...
export async function resolveSafeOutboundTarget(value: string | URL, options?: SafeOutboundOptions): Promise<SafeOutboundTarget | null> {
    ...
    const privateAllowed = !options?.publicOnly && privateUpstreamHostAllowed(hostname);
...
function privateUpstreamHostAllowed(hostname: string) {
    if (process.env.DREAMYO_ALLOW_PRIVATE_UPSTREAMS !== "1") return false;
    return (process.env.DREAMYO_PRIVATE_UPSTREAM_HOSTS || "").split(",").map(normalizeHostname).filter(Boolean).includes(hostname);
}
