...
export async function createUserDataExportStream(userId: string) {
    ...
    return new ReadableStream<Uint8Array>({
        async start(controller) {
            try {
                controller.enqueue(encoder.encode(`{"format":"dreamyo-personal-data","version":1,"exportedAt":${JSON.stringify(exportedAt)},"account":${JSON.stringify(account)}`));
