...
import { AGENT_SKILL_ARCHIVE_MAX_BYTES, AGENT_SKILL_EXTRACTION_SOURCE_LENGTH, type AgentSkillImportCandidate, type AgentSkillImportResult, type ImportedAgentSkill } from "@/lib/agent-skill-import-types";
...
function parseLocalSkillMarkdown(markdown: string, source: { fileName: string; sourcePath: string; sourceBundle?: string; archiveEntries?: Map<string, string> }): ImportedAgentSkill {
    ...
    const sourceContentHash = createHash("sha256")
        .update(source.sourceBundle || markdown, "utf8")
        .digest("hex");
