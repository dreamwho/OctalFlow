...
describe("Drama project Agent references", () => {
    ...
    it("offers stage-aware actions and keeps the project snapshot semantic without arbitrary array slicing", async () => {
        ...
        expect(source).toContain('rootStyle={{ width: "100vw", maxWidth: "100vw", overflowX: "clip" }}');
        expect(source).toContain('styles={{ wrapper: { width: "min(360px, calc(100vw - 8px))", maxWidth: "calc(100vw - 8px)" }, body: { padding: 0 } }}');
