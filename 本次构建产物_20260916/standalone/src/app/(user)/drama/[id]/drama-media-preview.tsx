...
export function DramaMediaThumbnail({ media, onOpen }: { media: DramaPreviewMedia; onOpen: (media: DramaPreviewMedia) => void }) {
    return (
        ...
        >
            ...
            <span className="absolute bottom-1.5 left-1.5 grid size-6 place-items-center rounded bg-black/60 text-white">{media.type === "image" ? <DreamyoIcon name="image" size={16} /> : <DreamyoIcon name="video" size={16} />}</span>
        </button>
    );
}
