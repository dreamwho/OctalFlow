...
export function buildCanvasNodeConfig(globalConfig: AiConfig, node: CanvasNodeData, mode: CanvasGenerationMode, model: string): AiConfig {
    ...
    return {
        ...
        audioMode,
        audioVoice:
            isQwenAudioModel(upstreamModel) && audioVoiceOptions.some((option) => option.value === configuredAudioVoice)
                ? qwenDefaultAudioVoice(upstreamModel)
                : isMiniMaxAudioModel && audioVoiceOptions.some((option) => option.value === configuredAudioVoice)
                  ? ""
                  : configuredAudioVoice,
