...
import {
    ...
    hydrateCanvasImages,
    isDreaminaUpscaleImageNode,
    normalizeCanvasConfigNodeLayout,
    normalizeConnection,
    normalizeCreatedNodeConnection,
    replaceCanvasNodeMediaMetadata,
    resolveDreaminaUpscaleSourceNode,
...
describe("Canvas config node layout", () => {
    it("keeps the persisted details state and node hit box in sync", () => {
        ...
        expect(collapsed).toMatchObject({ width: 480, height: CANVAS_CONFIG_NODE_HEIGHT.collapsed, metadata: { configDetailsOpen: false } });
