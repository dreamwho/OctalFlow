        ...
        <Modal
            ...
            footer={
                <div className="flex justify-end gap-2">
                    ...
                    <Button type="primary" disabled={!file} loading={saving} onClick={() => void submit()}>
                        上传并替换
                    </Button>
                </div>
            }
        >
            <div
                ...
                onDrop={(event) => {
                    if (!preventFileDragEvent(event)) return;
                    setDragging(false);
                    choose(droppedFiles(event, isAudioFile)[0]);
                }}
            >
                ...
                <div className="mt-4 flex justify-center gap-2">
                    <Button icon={<DreamyoIcon name="audio" size={18} />} onClick={() => inputRef.current?.click()}>
                        选择文件
                    </Button>
                    <Button
                            ...
                            void navigator.clipboard
                                ...
                                .then(async (items) => {
                                    for (const item of items) {
                                        const type = item.types.find((candidate) => candidate.startsWith("audio/"));
                                        if (type) {
                                            choose(new File([await (await item.getType(type)).arrayBuffer()], `pasted-audio.${type.split("/")[1] || "wav"}`, { type }));
                                            break;
                                        }
                                    }
                                })
                ...
                {file ? (
                    <div className="mx-auto mt-5 flex max-w-sm items-center justify-between rounded-lg border border-indigo-200 bg-white px-3 py-2 text-left text-sm dark:border-indigo-900 dark:bg-zinc-950">
                        <span className="truncate">{file.name}</span>
                        <span className="ml-3 shrink-0 text-xs text-zinc-500">{Math.ceil(file.size / 1024)} KB</span>
                    </div>
                ) : null}
            </div>
        </Modal>
