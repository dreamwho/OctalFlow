...
describe("Canvas Agent 事件流", () => {
    ...
    it("streams task start and completion timing for the multi-task progress card", async () => {
        ...
        expect(progress).toEqual([
            ...
            },
            {
                startedAt: 100,
                tasks: [
                    { id: "one", title: "分镜一", status: "running", startedAt: 200 },
                    { id: "two", title: "分镜二", status: "ready" },
                ],
            },
            {
                startedAt: 100,
                tasks: [
                    { id: "one", title: "分镜一", status: "completed", startedAt: 200, completedAt: 500 },
                    { id: "two", title: "分镜二", status: "ready" },
                ],
            },
        ]);
    ...
    it("ignores a stale terminal event when persisted pending children revive the Run", async () => {
        ...
        mocks.getCreativeAgentRun.mockResolvedValue({
            ...
            status: "running",
            assetIds: [],
            tasks: [{ id: "video", title: "视频", status: "running", childTasks: [{ id: "child-1", status: "pending", attempt: 1 }] }],
        });
        const terminal: string[] = [];
        const stages: CanvasAgentRunStage[] = [];
        const promise = watchCanvasAgentRun("run", {
        ...
        mocks.getCreativeAgentRun.mockResolvedValue({
            ...
            status: "completed",
            assetIds: [],
            tasks: [{ id: "video", title: "视频", status: "completed", childTasks: [{ id: "child-1", status: "completed", attempt: 1 }] }],
        });
        FakeEventSource.instance.emit("run.completed", { data: { reply: "完成" } });
        await promise;
        expect(terminal).toEqual(["completed"]);
