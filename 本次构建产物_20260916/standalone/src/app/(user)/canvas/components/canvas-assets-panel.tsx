...
import { ChevronDown, FolderOpen, LayoutGrid, LibraryBig, LocateFixed, Plus, RefreshCw, Search, SlidersHorizontal, Sparkles, X } from "lucide-react";
...
function CurrentCanvasAssets({ items, onLocate, onPreview }: { items: CurrentMedia[]; onLocate: (id: string) => void; onPreview: (preview: MediaPreview) => void }) {
    ...
    if (!items.length) return <PanelEmpty icon={<DreamyoIcon name="image" size={24} />} text="当前画布还没有图片或视频" />;
...
function LibraryAssetThumbnail({ asset }: { asset: Asset }) {
    ...
    if (asset.kind === "audio") return <FallbackThumbnail icon={<DreamyoIcon name="audio" size={22} />} />;
    return <FallbackThumbnail icon={<DreamyoIcon name="document" size={22} />} />;
}
