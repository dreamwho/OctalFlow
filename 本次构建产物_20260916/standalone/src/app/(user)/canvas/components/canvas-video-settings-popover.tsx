...
export function CanvasVideoSettingsPopover({ config, metadata, references, onConfigChange, onMetadataChange, buttonClassName, placement = "topLeft" }: CanvasVideoSettingsPopoverProps) {
    ...
    const canvasPanelSummary = `${referenceLabel} · ${fixedSizeLabel || (preferences.video?.size === "auto" ? "智能" : String(preferences.video?.size || "").replace("x", "×"))} · ${preferences.video?.quality === "auto" ? "智能" : `${String(preferences.video?.quality || "").replace(/p$/i, "")}P`} · ${preferences.video?.seconds || 5}秒`;
    ...
    return (
        <CreativeGenerationPreferences
            ...
            canvasPanel={{ title: "视频生成参数", description: "设置参考方式、比例、清晰度、时长与声音选项", summary: canvasPanelSummary }}
