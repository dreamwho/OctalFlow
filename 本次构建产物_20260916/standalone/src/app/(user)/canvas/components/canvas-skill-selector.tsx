...
export function CanvasSkillSelector({ skills, loading = false, selectedSkillIds = [], onSelect, trigger }: Props) {
    ...
    const dialogContent = (
        <div className="flex max-h-[min(600px,calc(100dvh-48px))] min-h-0 select-none flex-col overflow-hidden" onClick={(e) => e.stopPropagation()} data-canvas-skill-dialog>
            <div data-canvas-skill-toolbar className="flex shrink-0 items-center gap-3 border-b px-4 py-3 pr-14 max-[720px]:flex-col max-[720px]:items-stretch max-[720px]:gap-2" style={{ borderColor: theme.toolbar.border }}>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="mb-1 flex items-center justify-between gap-1">
                                    ...
                                    {selected ? (
                                        <Check className="size-4 shrink-0 text-cyan-600 dark:text-cyan-300" />
                                    ) : (
                                        <span className="shrink-0 rounded-full bg-black/5 px-1.5 py-0.5 text-[10px] opacity-55 dark:bg-white/10">{skill.promptMode === "optional" ? "提示词可选" : "需补充提示词"}</span>
                                    )}
                                </div>
