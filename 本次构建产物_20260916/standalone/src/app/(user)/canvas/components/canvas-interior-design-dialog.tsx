...
import { Aperture, Maximize2, Minimize2, Search, SlidersHorizontal, Sparkles, X } from "lucide-react";
...
export function CanvasInteriorDesignDialog({ open, initialSettings, initialRunningHubAppId, initialRunningHubAppName, onCancel, onConfirm }: CanvasInteriorDesignDialogProps) {
    ...
    useEffect(() => {
        ...
        void listRunningHubCatalogApps("interior-design")
            .then(setCatalogApps)
        ...
        <Modal
            ...
            styles={{
                container: {
                    ...
                    boxShadow: colorTheme === "dark" ? "0 0 0 1px rgba(103,232,249,.16), 0 24px 76px rgba(34,211,238,.14), 0 22px 64px rgba(0,0,0,.5)" : "0 0 0 1px rgba(8,145,178,.09), 0 24px 72px rgba(8,145,178,.16), 0 18px 54px rgba(15,23,42,.2)",
                },
                                ...
                                <div className="grid grid-cols-2 content-start gap-4 sm:grid-cols-[repeat(auto-fill,minmax(160px,184px))]">
                                            ...
                                            <span className="relative block aspect-square overflow-hidden rounded-xl border" style={{ background: theme.node.fill, borderColor: theme.toolbar.border }}>
                                                {previewFailed ? (
                                                    <span className="grid size-full place-items-center gap-2 p-4 text-center text-xs" style={{ color: theme.node.muted }}>
                                                        <DreamyoIcon name="image" size={32} />
                                                        预览图暂不可用
                                                    </span>
                                                ) : (
                                    ...
                                    {visibleApps.map((app) => (
                                        ...
                                        >
                                            <span className="relative block aspect-square overflow-hidden rounded-xl border" style={{ background: theme.node.fill, borderColor: theme.toolbar.border }}>
                                                {app.thumbnailUrl ? (
                                                    <img src={browserReadableMediaUrl(imagePreviewUrl(app.thumbnailUrl, 720))} alt={`${app.name}预览`} className="size-full object-cover transition duration-200 group-hover:scale-[1.02]" />
                                                ) : (
                                                    <span className="grid size-full place-items-center gap-2 p-4 text-center text-xs" style={{ color: theme.node.muted }}>
                                                        <DreamyoIcon name="image" size={32} />
                                                        暂无预览图
                                            ...
                                            <span className="mt-1 flex items-center justify-between gap-2 text-[11px]" style={{ color: theme.node.muted }}>
                                                <span className="flex min-w-0 items-center gap-1 truncate">
                                                    ...
                                                    {app.kind === "ai-app" ? "AI 应用" : "工作流"}
                                                </span>
                                                <span className="shrink-0">{app.fieldCount} 参数</span>
                                            </span>
                                        </button>
                                    ))}
                                </div>
