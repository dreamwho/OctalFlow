...
export function CanvasImageSettingsPopover({ config, onConfigChange, onOpenChange, buttonClassName, placement = "topLeft", fixedSizeLabel, compactTriggerLabel, showTriggerChevron = true }: CanvasImageSettingsPopoverProps) {
    ...
    const canvasPanelSummary = `${fixedSizeLabel || compactSizeLabel(preferences.image?.size, geminiAi)} · ${imageQualityLabel(preferences.image?.quality, geminiAi, dreamina?.qualities)} · ${preferences.image?.count || 1}份`;
    ...
    return (
        <CreativeGenerationPreferences
            ...
            canvasPanel={{ title: "图片生成参数", description: "设置生成图片的比例、画质与数量", summary: canvasPanelSummary }}
