        ...
        <div data-canvas-interior-design-node className="flex h-full w-full cursor-move flex-col px-3 py-3 text-sm" style={{ color: theme.node.text }}>
                ...
                <div className="min-w-0" onMouseDown={(event) => event.stopPropagation()}>
                    {runningHubAppId ? (
                        <button
                            type="button"
                            data-canvas-no-drag
                            className="canvas-compact-control flex h-9 w-full min-w-0 items-center gap-2 rounded-xl border px-3 text-left text-[11px] font-medium"
                            style={{ background: theme.toolbar.itemHover, borderColor: theme.node.stroke, color: theme.node.muted }}
                            onClick={() => onEdit(node.id)}
                        >
                            <Workflow className="size-3.5 shrink-0" />
                            <span className="min-w-0 flex-1 truncate">RunningHub · {node.metadata?.runningHubAppName || "已绑定应用"}</span>
                        </button>
                    ) : (
                        <ModelPicker
            ...
            <div data-canvas-interior-actions className="mt-2.5">
                ...
                <GenerationActionButton
                    ...
                    running={isRunning}
                    cancellable
                    className="!h-12 !w-full !rounded-2xl !px-3 !text-[15px] !font-semibold"
