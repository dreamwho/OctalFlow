...
export function CanvasAudioSettingsPopover({ config, onConfigChange, buttonClassName, placement = "top", sourceAudioUrl }: CanvasAudioSettingsPopoverProps) {
    ...
    const panelSummary = config.audioMode === "music" ? `${audioFormatLabel(config.audioFormat)} · ${audioSpeedLabel(config.audioSpeed)} · 1份` : `${voiceLabel} · ${audioSpeedLabel(config.audioSpeed)} · ${audioFormatLabel(config.audioFormat)} · 1份`;
    return (
        <CanvasSettingsPopoverShell
            ...
            panelWidth={520}
            panelMaxHeight={680}
            canvasPanel={{ title: "音频模型参数设置", description: "调整当前模型支持的音色、节奏与输出参数", summary: panelSummary }}
        >
