...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    const rememberNodePointer = (event: React.MouseEvent | React.PointerEvent) => {
        if (event.ctrlKey || event.metaKey || event.shiftKey) suppressContextMenuRef.current = true;
        else suppressContextMenuRef.current = false;
        const target = event.target instanceof Element ? event.target : null;
    ...
    return (
        <div
            ...
            onContextMenu={(event) => {
                // On macOS a Control-click is delivered as a context-menu
                // event. It is also the additive-selection gesture used by
                // the canvas, so do not leave a node menu open after that
                // selection path.
                if (suppressContextMenuRef.current || event.ctrlKey || event.metaKey || event.shiftKey) {
        ...
        >
            <div
                className={`relative h-full w-full overflow-visible ${isConfig ? "canvas-config-node-shell rounded-2xl border-2" : "rounded-md border"}`}
                style={{
            ...
            >
                {isSelected ? (
                    <div
                        data-canvas-node-selection-flow
                        aria-hidden="true"
                        className="canvas-node-selection-flow pointer-events-none absolute -inset-px z-20 rounded-[inherit] border-2 border-transparent"
                        style={
                            {
                                "--canvas-selection-start": canvasSelectionFlowColors.start,
                                "--canvas-selection-middle": canvasSelectionFlowColors.middle,
                                "--canvas-selection-end": canvasSelectionFlowColors.end,
                ...
                {data.type === CanvasNodeType.Audio && onUpload ? (
                    <button
                        type="button"
                        data-canvas-no-drag
                        className="absolute left-1/2 top-[-27px] z-40 inline-flex -translate-x-1/2 items-center gap-1 rounded-full border px-2 py-1 text-[11px] shadow-sm transition hover:brightness-110"
                        style={{ borderColor: theme.node.subtleBorder, background: theme.toolbar.panel, color: theme.node.text }}
                        onClick={(event) => {
                            event.stopPropagation();
                            onUpload(data);
                        }}
