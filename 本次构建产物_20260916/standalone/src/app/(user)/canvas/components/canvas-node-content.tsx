...
export function TextContent({ node, theme, isEditingContent, textareaRef, mentionReferences, onContentChange, onStartEditing, onStopEditing, onGenerateImage }: NodeContentRendererProps) {
    ...
    return (
        <div className="flex h-full min-h-0 min-w-0 w-full flex-col overflow-hidden pt-8">
            <div
                data-canvas-no-drag
                className="absolute right-3 top-3 z-20 flex h-8 items-center overflow-hidden rounded-lg border backdrop-blur-md"
            ...
            >
                ...
                <button
                    type="button"
                    disabled={!content}
                    className="grid h-full w-8 place-items-center border-l transition hover:bg-black/5 disabled:cursor-not-allowed disabled:opacity-35 dark:hover:bg-white/10"
                ...
                <button
                    type="button"
                    className="grid h-full w-8 place-items-center border-l transition hover:bg-black/5 dark:hover:bg-white/10"
                    style={{ borderColor: theme.node.stroke }}
            ...
            <div className="contents" onClick={stopCanvasInteraction} onDoubleClick={stopCanvasInteraction} onMouseDown={stopCanvasInteraction} onPointerDown={stopCanvasInteraction} onWheel={stopCanvasInteraction} onContextMenu={stopCanvasInteraction}>
                <Modal
                    ...
                    footer={null}
                    styles={{
                        wrapper: { width: "100vw", maxWidth: "100vw" },
                ...
                >
                    <div className="min-w-0 overflow-hidden rounded-xl border" style={{ borderColor: theme.node.stroke }}>
                        <CanvasResourceMentionTextarea
                            ...
                            onChange={(value) => onContentChange(node.id, value)}
                            aria-label="展开文本编辑器"
                            className="thin-scrollbar h-[min(58vh,30rem)] min-h-64 w-full resize-none border-0 px-4 py-3 font-mono outline-none"
                            style={{ ...textStyle, background: theme.node.fill }}
                        />
                    </div>
                    <div className="mt-3 flex items-center justify-between gap-2">
                        <Button icon={<Copy className="size-4" />} disabled={!content} onClick={copyContent}>
                            复制全文
                        </Button>
                        <Button icon={<Minimize2 className="size-4" />} onClick={() => setExpanded(false)}>
                            收起
                        </Button>
                    </div>
                </Modal>
            ...
            <div className="flex h-full w-full flex-col items-center justify-center gap-2 px-4" style={{ color: theme.node.placeholder }}>
                ...
                {onUpload ? (
                    <button
                        type="button"
                        data-canvas-no-drag
                        className="rounded-full border px-2.5 py-1 text-xs transition hover:brightness-110"
                        style={{ borderColor: theme.node.subtleBorder, background: theme.node.subtleSurface, color: theme.node.text }}
        ...
        <div className="flex h-full min-w-0 w-full flex-col items-center justify-center gap-3 overflow-hidden px-4" style={{ background: theme.node.fill, color: theme.node.text }}>
            ...
            {onUpload ? (
                <button
                    type="button"
                    data-canvas-no-drag
                    className="self-center rounded-full border px-2.5 py-1 text-xs transition hover:brightness-110"
                    style={{ borderColor: theme.node.subtleBorder, background: theme.node.subtleSurface, color: theme.node.text }}
                    onClick={() => onUpload(node)}
                >
                    替换音频
