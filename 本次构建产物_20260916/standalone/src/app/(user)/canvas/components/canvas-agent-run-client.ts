...
export function watchCanvasAgentRun(runId: string, handlers: RunHandlers, options: { signal?: AbortSignal } = {}) {
    ...
    return new Promise<void>((resolve, reject) => {
        ...
        const settleTerminal = (
            fallbackStatus: Extract<CreativeAgentRun["status"], "completed" | "failed" | "cancelled">,
            onSettled: (status: Extract<CreativeAgentRun["status"], "completed" | "failed" | "cancelled">, run?: CanvasAgentRunWithRecovery) => void,
        ) => {
        ...
        listen("run.failed", (event) => {
            ...
            settleTerminal("failed", (status, run) => {
                if (status === "failed" && !latestFailedTask) {
                    const failed = run?.tasks.find((task) => task.status === "failed");
                    handlers.onAssistant(failed ? `「${failed.title || "创作任务"}」执行失败：${failed.error || "生成服务暂时不可用"}` : run?.error || payload.data?.message || "Agent 执行失败", {
                        runId,
                        title: failed?.title || "Agent 执行失败",
                        ...(failed?.id ? { taskId: failed.id } : {}),
                    });
                }
            });
        });
