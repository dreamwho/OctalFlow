...
export function AdminRunningHubSection() {
    ...
    const tabs = [
        {
            key: "account",
            label: <RunningHubTabLabel icon={CircleDollarSign}>账户统计</RunningHubTabLabel>,
            children: <AccountTab overview={overview} loading={loading} settingsForm={settingsForm} saving={saving} onSave={saveSettings} onRefresh={() => void load(true)} />,
        },
        {
            key: "apps",
            label: <RunningHubTabLabel icon={Workflow}>应用管理</RunningHubTabLabel>,
            children: <AppsTab apps={overview?.apps || []} loading={loading} onAdd={() => openAppEditor()} onEdit={openAppEditor} onSync={(app) => void syncApp(app)} onDelete={removeApp} />,
        },
        {
            key: "tasks",
            label: <RunningHubTabLabel icon={SquareActivity}>任务监控</RunningHubTabLabel>,
            children: (
                <TasksTab
                    ...
                    onRefresh={() => void load()}
                    onCancel={async (task) => {
                        await updateRunningHubAdmin({ action: "cancel-task", id: task.id });
        ...
        {
            key: "logs",
            label: <RunningHubTabLabel icon={ScrollText}>请求日志</RunningHubTabLabel>,
            children: (
                <LogsTab
                    logs={overview?.logs || []}
                    loading={loading}
                    ...
                    onClear={() =>
                        modal.confirm({
                            title: "清空 RunningHub 请求日志？",
                            content: "此操作只清空接口请求日志，不会删除任务和应用。",
                            okText: "清空",
                            okButtonProps: { danger: true },
                            cancelText: "取消",
                            onOk: async () => {
                                await updateRunningHubAdmin({ action: "clear-logs" });
                ...
                <Form form={appForm} layout="vertical" className="pt-2" onFinish={(values) => void saveApp(values)}>
                    ...
                    <div className="grid gap-x-3 sm:grid-cols-2">
                        <Form.Item name="name" label="应用名称" rules={[{ required: true, message: "请输入应用名称" }]}>
                            <Input placeholder="例如：SU直出摄影级照片" />
                        </Form.Item>
                        <Form.Item name="kind" label="接入类型" rules={[{ required: true }]}>
                            <Select
                                options={[
                                    { label: "AI 应用（WebApp）", value: "ai-app" },
                                    { label: "ComfyUI 工作流", value: "workflow" },
                                ]}
                            />
                        </Form.Item>
                        <Form.Item name="remoteId" label="RunningHub ID" rules={[{ required: true, message: "请输入应用或工作流 ID" }]}>
                            <Input placeholder="RunningHub 后台中的 ID" />
                        </Form.Item>
                    ...
                    <Form.Item name="description" label="功能说明">
                        <Input.TextArea rows={3} placeholder="向用户说明这个应用适合什么场景" />
                    </Form.Item>
                    <Form.Item name="featureBindings" label="功能绑定">
                        <Select mode="multiple" options={[{ label: "室内设计", value: "interior-design" }]} placeholder="选择要展示此应用的功能面板" />
                    </Form.Item>
                    <Form.Item name="enabled" label="启用" valuePropName="checked">
                        <Switch />
                    </Form.Item>
                    <div className="flex justify-end gap-2">
                        <Button onClick={() => setAppModalOpen(false)}>取消</Button>
...
function AccountTab({
    ...
    overview?: RunningHubAdminOverview;
    loading: boolean;
    settingsForm: ReturnType<typeof Form.useForm<SettingsForm>>[0];
    saving: boolean;
        ...
        <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)]">
            <div className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
                <div className="mb-4 flex items-center gap-2">
                    <CloudCog className="size-5 text-cyan-500" />
                    <div>
                        <h3 className="font-semibold text-zinc-950 dark:text-zinc-100">接口配置</h3>
                        <p className="mt-0.5 text-xs text-zinc-500">统一用于上传、任务提交、查询、取消和字段同步。</p>
                    </div>
                </div>
                <Form form={settingsForm} layout="vertical" disabled={loading} onFinish={(values) => void onSave(values)}>
                    <div className="grid gap-x-3 sm:grid-cols-2">
                        <Form.Item name="apiBaseUrl" label="API 地址" rules={[{ required: true }]}>
                            <Input placeholder="https://www.runninghub.ai" />
                        </Form.Item>
                        <Form.Item name="instanceType" label="算力实例">
                            <Select
                                options={[
                                    { label: "标准实例", value: "standard" },
                                    { label: "Plus 实例", value: "plus" },
                                ]}
                            />
                        </Form.Item>
                    </div>
                    <Form.Item
                        ...
                        label={
                            <span className="inline-flex items-center gap-1.5">
                                ...
                                API Key{overview?.settings.hasApiKey ? <Tag color="green">已保存</Tag> : null}
                            </span>
                        }
                    >
                        <Input.Password placeholder={overview?.settings.hasApiKey ? "留空则保持现有密钥" : "请输入 RunningHub API Key"} autoComplete="new-password" />
                    </Form.Item>
                    <Form.Item name="enabled" label="启用 RunningHub" valuePropName="checked">
                        <Switch />
                    </Form.Item>
            ...
            <div className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
                <div className="flex items-center justify-between gap-3">
                    <div>
                        <h3 className="font-semibold text-zinc-950 dark:text-zinc-100">账户状态</h3>
                        <p className="mt-0.5 text-xs text-zinc-500">实时读取 RunningHub 官方账户接口。</p>
                    </div>
                    <Button aria-label="刷新账户状态" icon={<RefreshCw className="size-4" />} loading={loading} onClick={onRefresh} />
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                    {accountMetrics.length ? (
                        accountMetrics.map((metric) => (
                            <div key={metric.label} className="rounded-lg bg-zinc-50 p-3 dark:bg-zinc-900">
                                <div className="text-xs text-zinc-500">{metric.label}</div>
                                <div className="mt-1 truncate text-lg font-semibold text-zinc-950 dark:text-zinc-100" title={metric.value}>
                                    {metric.value}
                                </div>
                            </div>
                        ))
                        ...
                        <div className="col-span-2 py-8">
                            <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="保存并启用后读取账户数据" />
                        </div>
                    )}
                </div>
            </div>
...
function AppsTab({
    ...
    apps: RunningHubAdminApp[];
    loading: boolean;
    onAdd: () => void;
    onEdit: (app: RunningHubAdminApp) => void;
    onSync: (app: RunningHubAdminApp) => void;
    onDelete: (app: RunningHubAdminApp) => void;
}) {
    return (
        <div>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h3 className="font-semibold text-zinc-950 dark:text-zinc-100">应用与工作流</h3>
                    <p className="mt-1 text-xs text-zinc-500">绑定“室内设计”后，名称与缩略图会自动出现在画布功能弹层。</p>
                </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {apps.map((app) => (
                    <article key={app.id} className="overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50/60 dark:border-zinc-800 dark:bg-zinc-900/40">
                        <div className="aspect-[16/8] bg-zinc-100 dark:bg-zinc-900">
                            {app.thumbnailUrl ? (
                                <img className="h-full w-full object-cover" src={browserReadableMediaUrl(imagePreviewUrl(app.thumbnailUrl, 640))} alt={app.name} />
                            ) : (
                        ...
                        <div className="p-3.5">
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                    <h4 className="truncate font-medium text-zinc-950 dark:text-zinc-100">{app.name}</h4>
                                    <p className="mt-1 truncate text-xs text-zinc-500">
                                        {app.kind === "ai-app" ? "AI 应用" : "工作流"} · {app.remoteId}
                                    </p>
                                </div>
                                <Tag color={app.enabled ? "green" : "default"}>{app.enabled ? "已启用" : "已停用"}</Tag>
                            </div>
                            <p className="mt-3 line-clamp-2 min-h-10 text-xs leading-5 text-zinc-500">{app.description || "暂无功能说明"}</p>
                            <div className="mt-3 flex flex-wrap gap-1.5">
                                {app.featureBindings.includes("interior-design") ? <Tag color="cyan">室内设计</Tag> : <Tag>未绑定功能</Tag>}
                                <Tag>{app.fields.length} 个字段</Tag>
                            </div>
                            <div className="mt-4 flex justify-end gap-2">
                                <Button size="small" icon={<RotateCw className="size-3.5" />} onClick={() => onSync(app)}>
                                    同步字段
                                </Button>
                                <Button size="small" onClick={() => onEdit(app)}>
                                    编辑
                                </Button>
                                <Button size="small" danger aria-label={`删除 ${app.name}`} icon={<Trash2 className="size-3.5" />} onClick={() => onDelete(app)} />
                            </div>
                        </div>
                    </article>
                ))}
                {!apps.length && !loading ? (
                    <div className="col-span-full py-14">
                        <Empty description="还没有 RunningHub 应用" />
                    </div>
                ) : null}
            </div>
        </div>
            ...
            <Table
                ...
                columns={[
                    ...
                    { title: "错误", dataIndex: "error", ellipsis: true, render: (value: string) => value || "—" },
                    {
                        ...
                        fixed: "right",
                        width: 90,
                        render: (_: unknown, task: RunningHubAdminTask) =>
                            ["queued", "running"].includes(task.status) ? (
                                <Button size="small" danger onClick={() => void onCancel(task)}>
                                    取消
                                </Button>
                            ) : null,
                    },
    ...
    return (
        <>
            <div className="mb-3 flex justify-end gap-2">
                ...
                <Button danger icon={<Trash2 className="size-4" />} onClick={onClear}>
                    清空日志
                </Button>
            </div>
            <Table
...
function extractAccountMetrics(value: unknown) {
    ...
    return Object.entries(data)
        .filter(([key, item]) => labels[key] && ["string", "number", "boolean"].includes(typeof item))
        .slice(0, 6)
        .map(([key, item]) => ({ label: labels[key], value: typeof item === "boolean" ? (item ? "正常" : "不可用") : String(item) }));
}
