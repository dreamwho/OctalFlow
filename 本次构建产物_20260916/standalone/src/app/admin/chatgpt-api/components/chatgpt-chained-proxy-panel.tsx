...
export function ChatGptChainedProxyPanel({ proxyRuntime }: { proxyRuntime: ChatGptProxyRuntimeController }) {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const [nextMagic, genericData] = await Promise.all([getMagicProxy().catch(() => null), getChatGptProxies().catch(() => genericProxyRequest<ChatGptProxyView>("proxies").catch(() => null))]);
            ...
            <div className="space-y-6 p-4 sm:p-6">
                ...
                <div className="rounded-xl border border-blue-100 bg-gradient-to-r from-blue-50/60 via-indigo-50/40 to-slate-50/60 p-4 dark:border-blue-900/40 dark:from-blue-950/20 dark:via-indigo-950/20 dark:to-zinc-900/30">
                    <div className="mb-3 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold tracking-wide text-blue-900 dark:text-blue-200">链式级联数据流向 (dialer-proxy 拓扑)</span>
                        </div>
                    ...
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between text-xs text-zinc-700 dark:text-zinc-300">
                        ...
                        <div className="flex items-center gap-2 rounded-lg bg-white/80 px-3 py-2 shadow-xs dark:bg-zinc-800/80">
                            ...
                            <span>跳板：{hopNode ? <strong className="text-blue-600 dark:text-blue-400">{hopNode}</strong> : <span className="text-zinc-400">未选跳板</span>}</span>
                        </div>
                        <ArrowRight className="hidden h-4 w-4 text-zinc-400 md:block" />
                        <div className="flex items-center gap-2 rounded-lg bg-white/80 px-3 py-2 shadow-xs dark:bg-zinc-800/80">
                            ...
                            <div className="space-y-3">
                                <div>
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Button type="text" size="small" icon={<RefreshCw className="h-3 w-3" />} onClick={() => void load()}>
                                            刷新
                                        </Button>
                                    </div>
                                    <p className="mt-1 text-xs text-zinc-500">由 Clash 订阅中的海外专线（如香港、日本）作为跳板，负责在本地服务器与海外之间建立稳定加密通道。</p>
                                </div>
                                <Select
                                ...
                                />
                                {!magicState?.nodes.length ? <Alert type="warning" showIcon className="text-xs" title="暂无可用魔法节点，请先在「上游配置 ➔ 魔法代理」导入 Clash 订阅。" /> : null}
                            </div>
                        </Card>
                        ...
                        <Card size="small" className="border-zinc-200 dark:border-zinc-800">
                            <div className="space-y-3">
                                <div>
                                    <div className="flex items-center justify-between">
                                        <Text strong className="text-sm">
                                            第二步：选择后置落地出口 (通用代理/IPWO)
                                        </Text>
                                        <Button type="text" size="small" icon={<RefreshCw className="h-3 w-3" />} onClick={() => void load()}>
                                            刷新
                                        </Button>
                                    </div>
                                    <p className="mt-1 text-xs text-zinc-500">选择在「通用代理」中添加的 IPWO 动态住宅节点或其他代理节点作为真实对外出口。</p>
                                </div>
                                <Select
                                    ...
                                    filterOption={(input, option) => (option?.label ?? "").toLowerCase().includes(input.toLowerCase())}
                                />
                                {!landingOptions.length ? <Alert type="warning" showIcon className="text-xs" title="暂无可用通用代理节点，请先在「上游配置 ➔ 通用代理」添加 IPWO 或 HTTP/SOCKS5 节点。" /> : null}
                            </div>
                        </Card>
                    </div>
                )}
                ...
                {testResult ? (
                    <div className="rounded-lg border border-zinc-200 p-4 text-xs dark:border-zinc-800">
                        ...
                        <div className="mt-2 space-y-1 text-zinc-600 dark:text-zinc-400">
                            {testResult.ok ? (
                                <div>
                                    耗时延迟：<strong>{testResult.delay} ms</strong>
                                </div>
                            ) : null}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-zinc-100 pt-4 dark:border-zinc-800">
                    <div className="text-xs text-zinc-500">
                        当前状态：
                        {proxyRuntime.runtime?.mode === "chained" && proxyRuntime.runtime.enabled ? (
