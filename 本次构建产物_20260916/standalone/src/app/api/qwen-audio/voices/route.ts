...
export async function POST(request: Request) {
    ...
    const log = await appendMiniMaxRequestLog({
        ...
        durationMs: 0,
        phase: "queued",
        requestPreview: JSON.stringify({ mode: "voice-design", model }),
        lifecycle: [{ at: new Date().toISOString(), phase: "queued", message: "阿里云百炼音色设计请求已提交" }],
    }).catch(() => undefined);
    ...
    try {
        ...
        const payload =
            operation === "qwen"
                ? {
                      ...
                      input: { action: "create", target_model: model, preferred_name: safeName(body.name), voice_prompt: prompt.slice(0, 2048), preview_text: previewText.slice(0, 1024) },
                      parameters: { sample_rate: 24000, response_format: "wav" },
                  }
                : {
                      model: "voice-enrollment",
                      input: { action: "create_voice", target_model: model, prefix: safePrefix(body.name), voice_prompt: prompt.slice(0, 500), preview_text: previewText.slice(0, 200), language_hints: ["zh"] },
                      parameters: { sample_rate: 24000, response_format: "wav" },
                  };
        ...
        const voice = await saveMiniMaxVoice({
            provider: "aliyun-bailian",
            userId: user.id,
            ...
            voiceName: name,
            description,
            providerCreatedTime: qwenVoiceCreatedAt(result),
            scene: "个人创建",
            feature: classifyMiniMaxVoice(name, description),
            voiceType: "voice_generation",
            visible: true,
            category: classifyMiniMaxVoice(name, description),
        });
        if (log)
            await updateMiniMaxRequestLog(log.id, {
                ...
                phase: "success",
                responsePreview: "阿里云百炼设计音色已创建并保存",
        ...
        if (log)
            await updateMiniMaxRequestLog(log.id, {
                ...
                lifecycle: [{ at: new Date().toISOString(), phase: "failed", message: message.slice(0, 200) }],
            }).catch(() => undefined);
