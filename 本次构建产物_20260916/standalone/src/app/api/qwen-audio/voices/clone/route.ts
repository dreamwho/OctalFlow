...
export async function POST(request: Request) {
    ...
    try {
        ...
        const log = await appendMiniMaxRequestLog({
            ...
            durationMs: 0,
            phase: "queued",
            requestPreview: JSON.stringify({ mode: "voice-cloning", model, audio: { name: file.name, bytes: file.size, contentType: file.type || "audio/mpeg" } }),
            lifecycle: [{ at: new Date().toISOString(), phase: "queued", message: "阿里云百炼音色复刻请求已提交" }],
        }).catch(() => undefined);
...
async function persistPublicAudioUrl(bytes: Buffer, contentType: string, userId: string, originalName: string, request: Request, onStage?: (message: string) => Promise<void> | void) {
    ...
    try {
        ...
        return {
            ...
            storage: "site" as const,
            cleanup: async () => {
                await deleteLocalMediaAssetsByStorageKeys([asset.token], "reference");
            },
        };
    } catch (error) {
