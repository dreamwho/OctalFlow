...
vi.mock("@/lib/server/chatgpt-api-service", async (original) => ({
    ...
    chatGptRuntimeJson: state.runtime,
    chatGptRuntimeRequest: state.runtimeRequest,
    syncChatGptMagicProxy: vi.fn(),
    updateChatGptProxySelection: state.selection,
}));
...
describe("ChatGPT management authorization", () => {
    ...
    it("uses the explicit proxy-selection bridge and never treats IPWO as an arbitrary route", async () => {
        ...
        const patch = await PATCH(
            new Request("http://canvas.test/api/admin/chatgpt-api/proxy-selection", {
                method: "PATCH",
