...
async function handle(request: Request, context: Context) {
    ...
    const route =
        routes[path] ||
        (/^proxies\/groups\/[\p{L}\p{N}_-]+$/u.test(path) ? { path: `/api/proxy/groups/${segments[2]}`, methods: ["DELETE"] } : /^logs\/(?!g[ta]-)[\p{L}\p{N}_-]+$/u.test(path) ? { path: `/api/logs/${segments[1]}`, methods: ["GET"] } : null);
