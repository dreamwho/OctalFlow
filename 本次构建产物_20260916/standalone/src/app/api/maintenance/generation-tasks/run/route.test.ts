...
describe("POST /api/maintenance/generation-tasks/run", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        vi.stubEnv("NEXT_PUBLIC_SITE_URL", "https://dreamyo.example");
    ...
    it("passes the stable worker identity and server-only origins to the recovery batch", async () => {
        ...
        expect(mocks.recover).toHaveBeenCalledWith({ origin: "http://internal:3000", publicOrigin: "https://dreamyo.example", limit: 50, workerId: "worker-one" });
