...
export async function POST(request: Request, { params }: { params: Promise<{ channelId: string }> }) {
    try {
        ...
        const eventId = request.headers.get("x-dreamyo-event-id")?.trim() || readProviderString(payload, "event.id", EVENT_KEYS);
        const verification = verifyGenerationWebhookSignature({
            channelId,
            eventId,
            timestamp: request.headers.get("x-dreamyo-timestamp") || request.headers.get("x-timestamp") || "",
            rawBody,
            signature: request.headers.get("x-dreamyo-signature") || request.headers.get("x-signature") || "",
