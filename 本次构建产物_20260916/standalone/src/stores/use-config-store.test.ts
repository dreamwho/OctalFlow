...
describe("applyPublicSystemSettings", () => {
    ...
    it("does not submit a stale channel-scoped model identity", () => {
        const config = applyPublicSystemSettings(defaultConfig, {
            ...audioSettings,
            systemChannels: [...audioSettings.systemChannels!, { id: "legacy-audio", name: "旧渠道", baseUrl: "https://legacy.example.com", apiKey: "", apiFormat: "openai", models: ["speech-v1"], enabled: true, hasApiKey: true }],
            logicalModels: [
                {
                    ...audioSettings.logicalModels![0],
                    bindings: [
                        { ...audioSettings.logicalModels![0].bindings[0], channelId: "audio-channel", priority: 1 },
                        { id: "legacy-binding", channelId: "legacy-audio", upstreamModel: "speech-v1", enabled: true, priority: 2 },
                    ],
                },
            ],
        });
