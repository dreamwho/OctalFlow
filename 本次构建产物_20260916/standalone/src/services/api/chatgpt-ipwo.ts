...
export async function testIpwoConnection(onEvent: (event: IpwoTestEvent) => void, signal: AbortSignal) {
    ...
    const consume = (line: string) => {
        ...
        if (completed || !["running", "success", "error"].includes(event.status) || (event.done === true && event.status === "running") || typeof event.stage !== "string" || typeof event.message !== "string" || !Number.isFinite(event.elapsed_ms))
            throw new Error("测试日志格式无效");
        onEvent(event);
