...
import { ChevronDown, Lightbulb, Maximize2, Sparkles, X } from "lucide-react";
...
export function CreativeGenerationPreferences({
    ...
    const maximumPanelHeight = canvasPanel ? 680 : compact ? 620 : 520;
    const closePanel = useCallback(() => {
        setOpen(false);
        onOpenChange?.(false);
    }, [onOpenChange]);
    const applyCanvasPanel = useCallback(() => {
        canvasPanel?.onApply?.();
        closePanel();
    }, [canvasPanel, closePanel]);
        ...
        <Popover
            ...
            styles={{
                container: {
                    ...
                    padding: canvasPanel ? 0 : compact ? 6 : 8,
                    borderRadius: canvasPanel ? 20 : compact ? 14 : 16,
                    transform: canvasPanel && (adaptivePlacement === "top" || adaptivePlacement === "bottom") ? "translateX(12px)" : undefined,
                },
            }}
            content={
                <div
                    ...
                    className={cn(
                        "hide-scrollbar min-w-0 max-w-[calc(100vw-24px)] overflow-x-hidden overflow-y-auto overscroll-contain bg-white/[.96] text-[#1f2b46] dark:bg-[#020813]/[.98] dark:text-[#f7fbff]",
                        canvasPanel ? "w-[520px]" : compact ? "w-[316px]" : "w-[360px]",
                        panelClassName,
                    )}
                    style={{ width: canvasPanel ? "min(520px, calc(100vw - 48px))" : undefined, maxHeight: panelMaxHeight === undefined ? `min(${maximumPanelHeight}px, calc(100dvh - 96px))` : panelMaxHeight }}
                ...
                >
                        ...
                        <div className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b px-4 py-3" style={{ background: theme.toolbar.panel, borderColor: theme.node.stroke }}>
                            <div className="flex min-w-0 items-start gap-2.5">
                                <span className="mt-0.5 grid size-7 shrink-0 place-items-center rounded-lg" style={{ background: theme.node.infoSurface, color: theme.node.infoText }}>
                                    {canvasPanel.icon || <CreativeModeIcon mode={activeCapability} />}
                                </span>
                            ...
                            <button type="button" className="grid size-8 shrink-0 place-items-center rounded-xl transition hover:bg-black/[.05] dark:hover:bg-white/[.08]" style={{ color: theme.node.muted }} aria-label="关闭参数设置" onClick={closePanel}>
                                <X className="size-4" />
                            </button>
                        </div>
                    ) : (
                    ...
                    {showPreferenceFields ? (
                        <div className={canvasPanel ? "p-4" : undefined}>
                        <PreferencePanel
