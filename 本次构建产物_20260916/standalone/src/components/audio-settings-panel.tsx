...
type AudioSettingKey = "audioMode" | "audioVoice" | "audioFormat" | "audioSpeed" | "audioVolume" | "audioPitch" | "audioEmotion" | "audioLanguageBoost" | "audioSampleRate" | "audioBitrate" | "audioChannel" | "audioLyrics" | "audioInstructions";
...
export function AudioSettingsPanel({
    ...
    const qwenVoiceValues = useMemo(() => {
        ...
        return qwenAudioSystemVoices(qwenModel).map((item) => ({
            value: item.voiceParam,
    ...
    const qwenVoiceOptions: NonNullable<SelectProps["options"]> = [
        ...
        ...groupVoices(
            miniMaxVoices.personal.filter((item) => !audioVoiceOptions.some((option) => option.value === item.value) && (!item.model || item.model === qwenModel)),
            "个人创建",
        ),
                ...
                if (!active || !payload) return;
                    ...
                    const description = item.description || "";
                                    return [
                                        item.remoteVoiceId,
                                        {
                                            value: item.remoteVoiceId,
                                            label: voiceName,
                                            voiceName,
                                            description,
                                            category: item.category || item.feature || classifyMiniMaxVoice(voiceName, description),
                                            model: item.model,
                                            scene: item.scene,
                                            feature: item.feature,
                                            age: item.age,
    ...
    const createVoice = async () => {
        ...
        try {
            ...
            const response = clone
                ? await fetch(
                      isQwen ? "/api/qwen-audio/voices/clone" : "/api/minimax/voices/clone",
                      (() => {
                ...
                : await fetch(isQwen ? "/api/qwen-audio/voices" : "/api/minimax/voices", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ kind: "design", model: requestConfig.model, name: voiceName.trim() || "我的设计音色", prompt: voicePrompt, description: voiceDescription.trim() }),
                  });
    ...
    return (
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                {showModeSelector ? (
                    <SettingGroup title="功能" color={theme.node.muted}>
                    <AudioSelect
                ...
                {config.audioMode === "music" ? (
                    <>
                        ...
                        <div className="grid grid-cols-2 gap-2.5">
                            <SettingGroup title="歌词" color={theme.node.muted}>
                                <textarea
                                    value={config.audioLyrics || ""}
                                    placeholder="可选，输入歌词"
                                    className="h-16 w-full resize-none rounded-xl border bg-transparent px-3 py-2 text-sm outline-none"
                                    style={{ borderColor: theme.node.stroke, color: theme.node.text }}
                                    onChange={(event) => onConfigChange("audioLyrics", event.target.value)}
                                    onMouseDown={(event) => event.stopPropagation()}
                                />
                            </SettingGroup>
                        </div>
                    ...
                    <div className="space-y-2 rounded-xl border px-3 py-2.5 text-xs" style={{ borderColor: theme.node.stroke, color: theme.node.muted }}>
                        <div>{config.audioMode === "voice-clone" ? "上传参考音频创建复刻音色，然后在提示词面板填写要输出的文字并生成" : "点击下方按钮创建音色，名称、介绍和音色提示词在弹出层中填写"}</div>
                            {isVoiceProvider && (isQwen || (config.audioMode === "voice-clone" ? requestConfig.advancedConfig?.minimaxVoiceCloneEnabled !== false : requestConfig.advancedConfig?.minimaxVoiceDesignEnabled !== false)) ? (
                                <Button size="small" type="link" className="!px-0" onClick={openVoiceModal}>
                                    {config.audioMode === "voice-clone" ? "复刻新音色" : "设计新音色"}
                                </Button>
                            ) : null}
                    </div>
                    )
                    ...
                    <div className="space-y-3">
                        ...
                        <SettingGroup title="情感" color={theme.node.muted}>
                            <div className="grid grid-cols-4 gap-1 rounded-xl p-1" style={{ background: theme.node.subtleSurface }}>
                                {miniMaxEmotionOptions.map((option) => (
                                    <button
                                        key={option.value || "auto"}
                                        type="button"
                                        className={cn(
                                            "rounded-lg border border-transparent px-1.5 py-1.5 text-xs outline-none transition focus-visible:ring-2 focus-visible:ring-[var(--dreamyo-selection-accent)]/35",
                                            config.audioEmotion === option.value ? "dreamyo-selection-surface font-semibold" : "bg-white/55 hover:bg-white dark:bg-white/[.04] dark:hover:bg-white/[.08]",
                                        )}
                                        style={config.audioEmotion === option.value ? undefined : { color: theme.node.muted }}
                                        onClick={() => onConfigChange("audioEmotion", option.value)}
                                    >
                                        {option.label}
                                    </button>
                                ))}
                    ...
                    <div className="space-y-3">
                        <SettingGroup title="音色" color={theme.node.muted}>
                            ...
                            />
                            {isQwen3Snapshot && !qwenVoice ? (
                                <div className="text-xs" style={{ color: theme.node.muted }}>
                                    Qwen-TTS 复刻/设计模型必须先创建并选择与当前模型绑定的个人音色。
                                </div>
                            ) : null}
                        </SettingGroup>
                        {!isQwen3Model ? (
                            ...
                            <div className="grid grid-cols-2 gap-x-3 gap-y-2.5">
                                <RangeSetting title="语速" value={Number(config.audioSpeed || 1)} min={0.5} max={2} step={0.05} theme={theme} onChange={(value) => onConfigChange("audioSpeed", String(value))} />
                                    <RangeSetting
                                        title="音量"
                                        value={config.audioVolume === defaultConfig.audioVolume ? 50 : Number(config.audioVolume || 50)}
                                        min={0}
                                        max={100}
                                        step={1}
                                        theme={theme}
                                        onChange={(value) => onConfigChange("audioVolume", String(value))}
                                    />
                                    <RangeSetting
                                        title="音高"
                                        value={config.audioPitch === defaultConfig.audioPitch ? 1 : Number(config.audioPitch || 1)}
                                        min={0.5}
                                        max={2}
                                        step={0.05}
                                        theme={theme}
                                        onChange={(value) => onConfigChange("audioPitch", String(value))}
                                    />
                            </div>
                            <div className="grid grid-cols-2 gap-2.5">
                                    <SettingGroup title="格式" color={theme.node.muted}>
                                        <AudioSelect value={format === "pcm" ? "pcm" : format === "wav" ? "wav" : format === "opus" ? "opus" : "mp3"} options={qwenFormatOptions} theme={theme} onChange={(value) => onConfigChange("audioFormat", value)} />
                                    </SettingGroup>
                                    <SettingGroup title="采样率" color={theme.node.muted}>
                                        <AudioSelect
                                            value={qwenSampleRateOptions.some((item) => item.value === config.audioSampleRate) ? config.audioSampleRate : "24000"}
                                            options={qwenSampleRateOptions}
                                            theme={theme}
                                            onChange={(value) => onConfigChange("audioSampleRate", value)}
                                        />
                                    </SettingGroup>
                            </div>
                                {/^(?:cosyvoice-)/i.test(qwenModel) ? (
                                    <SettingGroup title="声音指令" color={theme.node.muted}>
                                        <textarea
                                            value={config.audioInstructions || ""}
                                            placeholder="例如：请用河南话表达。"
                                            className="h-20 w-full resize-none rounded-xl border bg-transparent px-3 py-2 text-sm leading-5 outline-none"
                                            style={{ borderColor: theme.node.stroke, color: theme.node.text }}
                                            onChange={(event) => onConfigChange("audioInstructions", event.target.value)}
                                            onMouseDown={(event) => event.stopPropagation()}
                                        />
                ...
                ) : config.audioMode !== "voice-clone" && config.audioMode !== "voice-design" && !isMiniMax && !isQwen ? (
                    <div className="grid grid-cols-2 gap-2.5">
                    <SettingGroup title="格式" color={theme.node.muted}>
                        <AudioSelect value={format} options={audioFormatOptions} theme={theme} onChange={(value) => onConfigChange("audioFormat", value)} />
                    </SettingGroup>
            ...
            <Modal
                ...
                footer={[
                    <Button key="cancel" onClick={closeVoiceModal} disabled={voiceSaving}>
                        取消
                    </Button>,
                    <Button
                        key="submit"
                        type="primary"
                        loading={voiceSaving}
                        style={{ background: theme.node.action, borderColor: theme.node.action, color: theme.node.actionText, opacity: 1 }}
                        onMouseDown={(event) => event.stopPropagation()}
                ...
                modalRender={(node) => (
                    <div onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()} onPointerUp={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
                        {node}
                    </div>
                )}
            >
                <div className="flex flex-col gap-4 pt-1">
                    ...
                    {config.audioMode === "voice-clone" ? (
                        <div className="flex flex-col gap-4">
                            <Input.TextArea className="!min-h-20" value={voiceDescription} maxLength={500} showCount placeholder="音色介绍（仅保存到当前项目后台，不会发送到上游）" onChange={(event) => setVoiceDescription(event.target.value)} />
                            <div
                                className="flex cursor-pointer flex-col gap-3 rounded-xl border border-dashed p-4"
                                onClick={() => voiceFileInputRef.current?.click()}
                                onDragOver={(event) => event.preventDefault()}
                                onDrop={handleVoiceDrop}
                                onPaste={handleVoicePaste}
                                onKeyDown={(event) => {
                                    if (event.key === "Enter" || event.key === " ") voiceFileInputRef.current?.click();
                                }}
                                role="button"
                                tabIndex={0}
                            >
                                <div className="text-sm font-medium">参考音频</div>
                                <div className="text-xs text-stone-500">将音频拖到这里，或点击区域后粘贴，也可以选择文件。支持 MP3、M4A、WAV，最大 20MB。</div>
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <span className="max-w-full truncate text-xs text-stone-600 dark:text-stone-300">{voiceFile?.name || (sourceAudioUrl ? "将使用当前音频节点文件" : "未选择文件")}</span>
                                </div>
                                <input ref={voiceFileInputRef} className="hidden" type="file" accept="audio/*,.mp3,.m4a,.wav" onChange={(event) => setSelectedVoiceFile(event.target.files?.[0])} />
                            </div>
                            <div className="text-xs text-stone-500">上传参考音频创建音色后，回到提示词面板填写要输出的文字文案，点击生成即可用该音色合成语音。</div>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-4">
                            <Input.TextArea
                                ...
                                maxLength={500}
                                showCount
                                placeholder="音色介绍（仅保存到当前项目后台，方便辨识音色类型，不会发送到上游）"
                                onChange={(event) => setVoiceDescription(event.target.value)}
                            />
                            <Input.TextArea className="!min-h-24" value={voicePrompt} placeholder="音色提示词：描述想要的音色，例如温暖、成熟、纪录片旁白；内容越具体，音色效果越稳定" onChange={(event) => setVoicePrompt(event.target.value)} />
                        </div>
                    )}
...
function AudioVoiceSelect({ value, options, theme, onChange }: { value: string; options: NonNullable<SelectProps["options"]>; theme: CanvasTheme; onChange: (value: string) => void }) {
    ...
    const voiceEntries = useMemo<VoiceEntry[]>(
        () =>
        ...
        return items.map((item) => {
            ...
            const categoryName = String(data.category || data.feature || groupLabel || "其他");
                    return {
                        ...
                        voiceName,
                        description: String(data.description || data.feature || "暂无音色介绍"),
                        category: `${groupSource} · ${categoryName}`,
    ...
    const categories = useMemo(
        () => [{ key: "全部", count: voiceEntries.length }, ...Array.from(new Map(voiceEntries.map((entry) => [entry.category, 1])).keys()).map((key) => ({ key, count: voiceEntries.filter((entry) => entry.category === key).length }))],
        [voiceEntries],
    );
            ...
            <div className="border-b px-4 pb-3 pt-3" style={{ borderColor: theme.toolbar.border }}>
                <div className="mb-2 flex items-center justify-between gap-2">
                    ...
                    <span className="text-[11px]" style={{ color: theme.node.muted }}>
                        {selected?.category || `${voiceEntries.length} 个音色`}
                    </span>
                </div>
            ...
            <div className="thin-scrollbar grid max-h-[min(390px,calc(100dvh-250px))] min-h-20 gap-2 overflow-y-auto p-3 sm:grid-cols-2">
                {filteredEntries.length ? (
                    ...
                    const active = entry.value === String(value);
                        return (
                            <div
                                key={`${entry.category}-${entry.value || entry.voiceName}`}
                                role="button"
                                ...
                                style={{
                                    ...
                                    background: active ? theme.node.subtleSurface : theme.node.fill,
                                ...
                                onKeyDown={(event) => {
                                    if ((event.key === "Enter" || event.key === " ") && !entry.disabled && entry.value) {
                                        event.preventDefault();
                                        onChange(entry.value);
                                        setOpen(false);
                                    }
                        ...
                        <div className="flex min-w-0 items-start gap-2">
                                    <span className="mt-0.5 grid size-7 shrink-0 place-items-center rounded-lg" style={{ background: theme.toolbar.itemHover, color: active ? theme.toolbar.activeText : theme.node.muted }}>
                                ...
                                <div className="truncate text-sm font-medium">{entry.voiceName}</div>
                                        ...
                                        {entry.previewUrl ? (
                                            <audio
                                                ...
                                                className="mt-2 h-7 w-full max-w-[220px]"
                                                aria-label={`试听 ${entry.voiceName}`}
                                                onClick={(event) => event.stopPropagation()}
                                                onMouseDown={(event) => event.stopPropagation()}
                                                onPointerDown={(event) => event.stopPropagation()}
                                            />
    ...
    return (
        <div className="block w-full" onMouseDown={(event) => event.stopPropagation()}>
        <Popover
            trigger="click"
        ...
        >
                <button
                    type="button"
                    className="flex h-9 w-full min-w-0 items-center gap-2 rounded-lg border px-3 text-left text-sm transition hover:brightness-110"
            ...
            <Select
                ...
                optionRender={(option) => {
                    ...
                    return data.voiceName ? (
                        <div className="min-w-[420px] py-1" onMouseDown={(event) => event.stopPropagation()}>
                            <div className="font-medium">{data.voiceName}</div>
                            <div className="mt-0.5 truncate text-xs opacity-60">{data.description || data.feature || "暂无音色介绍"}</div>
                            {data.previewUrl ? <audio controls preload="none" src={data.previewUrl} className="mt-1 h-7 w-48" onMouseDown={(event) => event.stopPropagation()} /> : null}
                        </div>
                    ) : (
...
function RangeSetting({ title, value, min, max, step, theme, onChange }: { title: string; value: number; min: number; max: number; step: number; theme: CanvasTheme; onChange: (value: number) => void }) {
    ...
    return (
        <SettingGroup title={title} color={theme.node.muted}>
            <div className="flex items-center gap-2">
                <Slider className="min-w-0 flex-1" min={min} max={max} step={step} value={safeValue} tooltip={{ formatter: null }} onChange={(next) => onChange(Number(next))} />
                <span className="min-w-9 rounded-md border px-1.5 py-0.5 text-center text-xs tabular-nums" style={{ borderColor: theme.node.stroke, color: theme.node.text }}>
                    {safeValue}
                </span>
            </div>
        </SettingGroup>
...
function SegmentedSetting({ title, value, options, theme, onChange }: { title: string; value: string; options: string[]; theme: CanvasTheme; onChange: (value: string) => void }) {
    return (
        <SettingGroup title={title} color={theme.node.muted}>
            <div className="grid grid-cols-5 overflow-hidden rounded-xl border" style={{ borderColor: theme.node.stroke }}>
                {options.map((option) => (
                    <button
                        key={option}
                        type="button"
                        className="min-w-0 px-2 py-2 text-xs transition"
                        style={{ background: value === option ? theme.node.subtleSurface : "transparent", color: value === option ? theme.node.text : theme.node.muted }}
