...
export const AnimatedThemeToggler = ({ children, className, duration = 400, variant, fromCenter = false, theme, targetTheme, onThemeChange, ...props }: AnimatedThemeTogglerProps) => {
    ...
    const toggleTheme = useCallback(() => {
        ...
        // Keep the DOM theme mutation inside the transition callback, but do not
        // force a synchronous React flush here. Chromium can reject a view
        // transition update callback when React is flushed from it; the rejected
        // callback leaves the root in the previous theme and makes the toggle
        // appear inert. The class mutation is synchronous and AppProviders will
        // reconcile the store-driven theme on the following render.
