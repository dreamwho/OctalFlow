                ...
                <PanelHeader
                    ...
                    actions={
                        <Space wrap>
                            ...
                            <Button type="primary" icon={<Save className="size-4" />} loading={settingsLoading} onClick={() => void save()}>
                                保存配置
                            </Button>
                        </Space>
                    }
                ...
                <div className="space-y-4 p-4 sm:p-5">
                    <div className="grid gap-3 sm:grid-cols-3">
                        <label className="space-y-1.5 text-sm">
                            <span>渠道名称</span>
                            <Input value={channel.name} onChange={(event) => updateChannel({ name: event.target.value })} />
                        </label>
                        <label className="space-y-1.5 text-sm">
                            <span>Base URL</span>
                            <Input value={channel.baseUrl} onChange={(event) => updateChannel({ baseUrl: event.target.value })} />
                        </label>
                        <label className="space-y-1.5 text-sm">
                            <span>API Key {channel.hasApiKey ? <Tag color="green">已配置</Tag> : null}</span>
                            <Input.Password placeholder={channel.hasApiKey ? "留空保持已保存密钥" : "请输入 MiniMax API Key"} value={channel.apiKey} onChange={(event) => updateChannel({ apiKey: event.target.value, enabled: true })} />
                        </label>
                    </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                        <div className="flex items-center justify-between rounded-lg border border-zinc-200 px-3 py-2.5 text-sm dark:border-zinc-800">
                            <div>
                                <div className="font-medium">允许音色复刻</div>
                                <div className="text-xs text-stone-500">关闭后 MiniMax 不会提交复刻请求，改用阿里云百炼或其他已启用模型。</div>
                            </div>
                            <Switch checked={channel.advancedConfig?.minimaxVoiceCloneEnabled !== false} onChange={(enabled) => updateVoiceFeature("minimaxVoiceCloneEnabled", enabled)} />
                        </div>
                        <div className="flex items-center justify-between rounded-lg border border-zinc-200 px-3 py-2.5 text-sm dark:border-zinc-800">
                            <div>
                                <div className="font-medium">允许音色设计</div>
                                <div className="text-xs text-stone-500">关闭后 MiniMax 不会提交设计请求，避免产生一次性音色费用。</div>
                            </div>
                            <Switch checked={channel.advancedConfig?.minimaxVoiceDesignEnabled !== false} onChange={(enabled) => updateVoiceFeature("minimaxVoiceDesignEnabled", enabled)} />
                        </div>
                        <div className="flex items-center justify-between rounded-lg border border-zinc-200 px-3 py-2.5 text-sm dark:border-zinc-800">
                            <div>
                                <div className="font-medium">允许音乐设计</div>
                                <div className="text-xs text-stone-500">关闭后前端不显示 MiniMax 音乐设计模型，不会提交音乐生成请求。</div>
                            </div>
                            <Switch checked={channel.advancedConfig?.minimaxMusicEnabled !== false} onChange={(enabled) => updateVoiceFeature("minimaxMusicEnabled", enabled)} />
                        </div>
                    </div>
                    <div className="grid gap-3 lg:grid-cols-2">
                        <ModelPicker
                            ...
                            values={channel.models.filter((model) => MINIMAX_SPEECH_MODELS.includes(model as (typeof MINIMAX_SPEECH_MODELS)[number]))}
                            options={[...MINIMAX_SPEECH_MODELS]}
                            onChange={(models) => updateChannel({ models: [...models, ...channel.models.filter((model) => MINIMAX_MUSIC_MODELS.includes(model as (typeof MINIMAX_MUSIC_MODELS)[number]))] })}
                        />
                        <ModelPicker
                            label="音乐模型"
                            icon={<Music2 className="size-4" />}
                            values={channel.models.filter((model) => MINIMAX_MUSIC_MODELS.includes(model as (typeof MINIMAX_MUSIC_MODELS)[number]))}
                            options={[...MINIMAX_MUSIC_MODELS]}
                            onChange={(models) => updateChannel({ models: [...channel.models.filter((model) => MINIMAX_SPEECH_MODELS.includes(model as (typeof MINIMAX_SPEECH_MODELS)[number])), ...models] })}
                        />
                    </div>
                    <Tabs
                        activeKey={tab}
                        onChange={(key) => setTab(key as MiniMaxTab)}
                        items={[
                    ...
                    <div className="space-y-4 text-sm">
                        <div className="grid gap-3 sm:grid-cols-3">
                            <div>
                                ...
                                <div>
                                    {selectedLog.phase} · HTTP {selectedLog.statusCode || "-"}
                                </div>
                            </div>
                            <div>
                                <div className="text-stone-500">模型</div>
                                <div>{selectedLog.model}</div>
                            </div>
                            <div>
                                <div className="text-stone-500">路径</div>
                        ...
                        <div>
                            <div className="mb-1 text-stone-500">生命周期</div>
                            <div className="max-h-52 overflow-y-auto rounded-lg bg-stone-50 p-3 dark:bg-zinc-900">
                                {selectedLog.lifecycle.map((item, index) => (
                                    <div key={`${item.at}-${index}`} className="border-b border-stone-200 py-2 last:border-0 dark:border-zinc-800">
                                        <span className="mr-2 text-stone-500">{new Date(item.at).toLocaleString()}</span>
                                        {item.message}
                                    </div>
                                ))}
                            </div>
    ...
    return (
        <div className="rounded-lg border border-zinc-200 p-3 dark:border-zinc-800">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-2">
                {options.map((value) => (
                    <Checkbox key={value} checked={values.includes(value)} onChange={(event) => onChange(event.target.checked ? Array.from(new Set([...values, value])) : values.filter((item) => item !== value))}>
                        {value}
                    </Checkbox>
                ))}
            </div>
        ...
        <>
            <div className="space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="text-sm text-stone-500">系统音色与用户创建音色分组管理；用户端个人音色按用户隔离。</div>
                    <Button onClick={onSync}>获取全部音色</Button>
                </div>
                <Tabs
                    activeKey={group}
                    onChange={(key) => setGroup(key as MiniMaxVoiceGroup)}
                    items={[
                        { key: "system", label: `系统音色（${systemVoices.length}）` },
                        { key: "personal", label: `用户创建（${personalVoices.length}）` },
                    ]}
                />
                <Table
                    rowKey="id"
                    ...
                    columns={[
                        {
                            ...
                            render: (value, voice) => (
                                <div>
                                    <div className="font-medium">{value || voice.name}</div>
                                    <div className="text-xs text-stone-500">前端显示：{voice.name}</div>
                                </div>
                            ),
                        },
                            ...
                            render: (_, voice) => (
                                <Space>
                                    <Button
                                        ...
                                        aria-label="编辑音色名称和分类"
                                    />
                                    <Popconfirm title="确认删除该音色？" onConfirm={() => void removeVoice(voice)}>
                                        <Button type="text" danger icon={<Trash2 className="size-4" />} aria-label="删除音色" />
                                    </Popconfirm>
                                </Space>
                            ),
                        },
            ...
            <Modal open={Boolean(editing)} title="编辑音色名称和分类" okText="保存" cancelText="取消" onCancel={() => setEditing(null)} onOk={() => void updateName()}>
                <div className="space-y-3">
                    <Input value={name} onChange={(event) => setName(event.target.value)} maxLength={80} />
                    <Select className="w-full" value={category} options={MINIMAX_VOICE_CATEGORIES.map((item) => ({ value: item, label: item }))} onChange={setCategory} />
                </div>
            </Modal>
        </>
    );
    ...
    return (
        <>
            <Table
                ...
                size="small"
                pagination={false}
                dataSource={state?.music || []}
                columns={[
                    { title: "名称", dataIndex: "name" },
                    { title: "模型", dataIndex: "model" },
                    { title: "风格/提示", dataIndex: "prompt", render: (value) => <span className="line-clamp-2 max-w-md">{value || "-"}</span> },
                    { title: "歌词", dataIndex: "lyrics", render: (value) => <span className="line-clamp-2 max-w-md">{value || "器乐"}</span> },
                        ...
                        render: (_, record) => (
                            <Space>
                                <Button
                                    ...
                                    aria-label="编辑音乐名称"
                                />
                                <Popconfirm title="确认删除该音乐记录？" onConfirm={() => void remove(record)}>
                                    <Button type="text" danger icon={<Trash2 className="size-4" />} aria-label="删除音乐记录" />
                                </Popconfirm>
                            </Space>
                        ),
                    },
                ]}
                locale={{ emptyText: "暂无 MiniMax 音乐记录，完成一次音乐生成后会自动出现" }}
            />
            <Modal open={Boolean(editing)} title="编辑音乐名称" okText="保存" cancelText="取消" onCancel={() => setEditing(null)} onOk={() => void updateName()}>
                <Input value={name} onChange={(event) => setName(event.target.value)} maxLength={80} />
            </Modal>
        </>
    );
...
function RequestLogs({ state, onSelect, onPageChange }: { state: MiniMaxAdminState | null; onSelect: (log: MiniMaxAdminState["logs"]["items"][number]) => void; onPageChange: (page: number) => void }) {
    ...
    return (
        <Table
            rowKey="id"
            size="small"
            pagination={{ current: state?.logs.page || 1, pageSize: state?.logs.pageSize || 20, total: state?.logs.total || 0, showSizeChanger: false, onChange: onPageChange }}
            dataSource={logs}
            onRow={(log) => ({ onClick: () => onSelect(log), className: "cursor-pointer" })}
            columns={[
...
function LogPayload({ title, value }: { title: string; value?: string }) {
    return (
        <div>
            <div className="mb-1 text-stone-500">{title}</div>
            <pre className="max-h-56 overflow-auto whitespace-pre-wrap break-all rounded-lg bg-stone-50 p-3 text-xs dark:bg-zinc-900">{value || "-"}</pre>
        </div>
    );
}
...
function promoteMiniMaxLogicalModels(models: LogicalModel[], channel: SystemModelChannel) {
    ...
    for (const model of models) {
        ...
        const upstream = model.bindings
            .find((binding) => miniMaxModels.has(binding.upstreamModel.trim().toLowerCase()))
            ?.upstreamModel.trim()
            .toLowerCase();
