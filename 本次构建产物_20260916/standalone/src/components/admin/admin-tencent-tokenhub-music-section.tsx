    ...
    return (
        <div className="space-y-4">
            ...
            <Panel>
                <PanelHeader
                    title="TokenHub"
                    description="管理 TokenHub 音频/音乐模型、生成记录和调用日志。API Key 仅通过渠道安全保存。"
                    actions={
                        <Space wrap>
                ...
                <div className="space-y-4 p-4 sm:p-5">
                    <div className="grid gap-3 sm:grid-cols-3">
                        ...
                        <label className="space-y-1.5 text-sm">
                            ...
                            <Input.Password value={channel.apiKey} placeholder={channel.hasApiKey ? "留空保持已保存密钥" : "请输入腾讯云 TokenHub API Key"} onChange={(event) => updateChannel({ apiKey: event.target.value, enabled: true })} />
                        </label>
                    </div>
                    ...
                    <div className="rounded-lg border border-zinc-200 p-3 dark:border-zinc-800">
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-2">
                            {TOKENHUB_MUSIC_MODELS.map((model) => (
                                <Checkbox
                                    key={model}
                                    checked={channel.models.includes(model)}
                                    onChange={(event) => updateChannel({ models: event.target.checked ? Array.from(new Set([...channel.models, model])) : channel.models.filter((item) => item !== model) })}
                                >
                                    {model}
                                </Checkbox>
                            ))}
                    ...
                    <Tabs
                        activeKey={tab}
                        onChange={(key) => setTab(key as "music" | "logs")}
                        items={[
                    ...
                    />
                    {tab === "music" ? (
                        <TokenHubMusicManagement records={state?.music || []} onRefresh={() => void load(state?.logs.page || 1)} onError={(text) => message.error(text)} />
                    ) : (
                        <div className="space-y-2">
                            <div className="flex items-center justify-between border-b border-zinc-200 pb-2 text-sm dark:border-zinc-800">
                                <span className="font-medium">请求日志</span>
                                <Button size="small" onClick={() => void load(state?.logs.page || 1)}>
                                    刷新日志
                                </Button>
                            </div>
                            <Table
                                ...
                                size="small"
                                loading={loading}
                                dataSource={state?.logs.items || []}
                                pagination={{ current: state?.logs.page || 1, pageSize: state?.logs.pageSize || 20, total: state?.logs.total || 0, showSizeChanger: false, onChange: (page) => void load(page) }}
                                onRow={(log) => ({ onClick: () => setSelectedLog(log), className: "cursor-pointer" })}
                                columns={[
                                ...
                                locale={{ emptyText: "暂无 TokenHub 请求日志" }}
                            />
                        </div>
                    )}
                </div>
                    ...
                    <div className="space-y-4 text-sm">
                        <div className="grid gap-3 sm:grid-cols-3">
                            ...
                            <div>
                                ...
                                <div className="break-all">POST {selectedLog.path}</div>
                            </div>
                        </div>
                        <div>
                            <div className="mb-1 text-stone-500">生命周期</div>
                            <div className="max-h-60 overflow-y-auto rounded-lg bg-stone-50 p-3 dark:bg-zinc-900">
                        ...
                        <div className="grid gap-3 md:grid-cols-2">
                            <LogPayload title="请求摘要" value={selectedLog.requestPreview} />
                            <LogPayload title="响应/错误" value={selectedLog.responsePreview || selectedLog.error} />
                        </div>
                    </div>
                ) : null}
            </Modal>
...
function TokenHubMusicManagement({ records, onRefresh, onError }: { records: MiniMaxMusicRecord[]; onRefresh: () => void; onError: (text: string) => void }) {
    ...
    return (
        <div className="space-y-2">
            <div className="flex items-center justify-between border-b border-zinc-200 pb-2 text-sm dark:border-zinc-800">
                <span className="font-medium">音乐管理</span>
                <span className="text-xs text-stone-500">仅显示 TokenHub 生成记录</span>
            </div>
            <Table
                rowKey="id"
                size="small"
                ...
                columns={[
                    ...
                    { title: "模型", dataIndex: "model" },
                    { title: "提示", dataIndex: "prompt", render: (value) => <span className="line-clamp-2 max-w-md">{value || "-"}</span> },
                    { title: "状态", dataIndex: "status" },
                    { title: "试听", dataIndex: "resultUrl", render: (value) => (value ? <audio controls preload="none" src={value} className="h-8 w-52" /> : "-") },
                    {
                        title: "操作",
                        render: (_, record) => (
                            <Space>
                                <Button
                                    ...
                                    aria-label="编辑 TokenHub 音乐名称"
                                />
                                <Popconfirm title="确认删除该 TokenHub 音乐记录？" onConfirm={() => void remove(record)}>
                                    <Button type="text" danger icon={<Trash2 className="size-4" />} aria-label="删除 TokenHub 音乐记录" />
                                </Popconfirm>
                            </Space>
                        ),
                    },
                ]}
                locale={{ emptyText: "暂无 TokenHub 音乐记录" }}
            />
            <Modal open={Boolean(editing)} title="编辑 TokenHub 音乐名称" okText="保存" cancelText="取消" onCancel={() => setEditing(null)} onOk={() => void updateName()}>
