...
export function MagicProxyTestModal({ open, onClose, nodes, delayResults, onDelayResultsChange, autoStart, initialTestGoogle = false, targetSingleNode = null, onTargetSingleNodeChange }: MagicProxyTestModalProps) {
            ...
            <div className="space-y-4 pt-2">
                ...
                {targetSingleNode && singleNodeObj ? (
                    <div className="rounded-lg border border-indigo-200/80 bg-indigo-50/40 p-3.5 dark:border-indigo-900/60 dark:bg-indigo-950/20">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                            <div className="min-w-0">
                                <div className="flex items-center gap-2">
                                    <span className="font-semibold text-sm text-zinc-900 dark:text-zinc-100">{singleNodeObj.name}</span>
                                    <Tag color="purple" className="text-[11px] font-mono">
                                        {singleNodeObj.type}
                                    </Tag>
                                </div>
                                <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">单节点独立探测 · 探测目标：https://www.gstatic.com/generate_204</div>
                            </div>
                        ...
                        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-indigo-100/80 pt-2.5 dark:border-indigo-900/40">
                            <div className="flex flex-wrap items-center gap-2">
                                <Button size="small" type="primary" icon={<Gauge className="size-3.5" />} loading={running} disabled={testingGoogle} onClick={() => void runSingleTest(singleNodeObj)}>
                                    重新测速
                                </Button>
                                <Button size="small" icon={<Globe className="size-3.5 text-emerald-500" />} loading={testingGoogle} disabled={running} onClick={handleTestGoogle}>
                                    测试 Google 可达性
                                </Button>
                    ...
                    <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-900/50">
                        <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
                            <span>
                                测试总进度 ({testedCount} / {totalNodes})
                            </span>
                            <span className="font-mono font-medium">{progressPercent}%</span>
                        </div>
                        <Progress percent={progressPercent} status={running ? "active" : undefined} strokeColor={{ "0%": "#6366f1", "100%": "#10b981" }} className="mt-1.5" />
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs sm:grid-cols-4">
                            ...
                            <div className="hidden rounded border border-zinc-200/80 bg-white p-2 dark:border-zinc-800 dark:bg-zinc-950 sm:block">
                                ...
                                <div className="mt-0.5 text-base font-bold text-indigo-600 dark:text-indigo-400">
                                    {successNodes.length ? Math.round(successNodes.reduce((acc, cur) => acc + (delayResults[cur.name]?.delay || 0), 0) / successNodes.length) + " ms" : "--"}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-zinc-200 pt-3 dark:border-zinc-800">
                            <div className="flex flex-wrap items-center gap-2">
                                <Button size="small" type="primary" icon={<RotateCcw className="size-3.5" />} loading={running} onClick={handleRetryAll}>
                                    全部重新测速
                                </Button>
                                <Button size="small" icon={<RotateCcw className="size-3.5" />} disabled={running || !failedNodes.length} onClick={handleRetryFailed}>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 text-xs text-zinc-500">
                        <div className="flex items-center gap-1.5">
                            ...
                            <Switch size="small" checked={autoScroll} onChange={setAutoScroll} checkedChildren="滚" unCheckedChildren="停" />
                        </div>
                ...
                {activeTab === "logs" ? (
                    <div className="space-y-2">
                        {/* 控制台黑色日志视窗 */}
                        <div ref={logScrollRef} className="h-80 overflow-y-auto rounded-lg border border-zinc-800 bg-zinc-950 p-3 font-mono text-xs text-zinc-300 shadow-inner select-text">
                            {logs.length === 0 ? (
                                <div className="flex h-full items-center justify-center text-zinc-600">暂无日志，点击“测速”开始探测节点可用性与实时日志</div>
                            ) : (
                                logs.map((log) => {
                                    const badgeColor =
                                        log.level === "ok"
                                    ...
                                    return (
                                        <div key={log.id} className="py-0.5 leading-relaxed hover:bg-zinc-900/60">
                                            ...
                                            <span className={`inline-block rounded px-1.5 py-0.2 mr-2 border text-[10px] ${badgeColor}`}>{log.level.toUpperCase()}</span>
                                            {log.nodeName ? <span className="text-indigo-300 font-semibold mr-1.5">[{log.nodeName}]</span> : null}
                                            <span className={log.level === "ok" ? "text-emerald-300" : log.level === "warn" ? "text-amber-300" : log.level === "error" ? "text-red-300" : "text-zinc-200"}>{log.message}</span>
                                        </div>
                                    );
                                })
                            )}
                        ...
                        <div className="h-80 overflow-y-auto space-y-1.5 rounded-lg border border-zinc-200 p-2 dark:border-zinc-800">
                            {filteredNodes.length === 0 ? (
                                ...
                                filteredNodes.map((node) => {
                                    ...
                                    return (
                                        <div key={node.name} className="flex items-center justify-between rounded border border-zinc-100 bg-white px-3 py-1.5 text-xs dark:border-zinc-800/80 dark:bg-zinc-900">
