...
type QwenState = {
    ...
    system: QwenVoice[];
    voices: QwenVoice[];
    audio: { items: QwenAudioRecord[]; total: number; page: number; pageSize: number };
    logs: {
        items: Array<{ id: string; createdAt: string; model: string; path: string; phase: string; statusCode: number; durationMs: number; error?: string; lifecycle: Array<{ at: string; message: string }> }>;
        total: number;
        page: number;
        pageSize: number;
    };
    ...
    return (
        <div className="space-y-4">
            ...
            <Panel>
                <PanelHeader
                    title="阿里云百炼语音控制台"
                    description="管理 Qwen-Audio-TTS、CosyVoice、Qwen-TTS 的音色复刻、音色设计、模型启用状态和请求日志。"
                    actions={
                        <div className="flex gap-2">
                            <Button
                                onClick={() => {
                                    void load();
                                    message.success("已获取百炼官方音色目录");
                                }}
                            >
                                获取全部音色
                            </Button>
                ...
                <div className="space-y-4 p-4 sm:p-5">
                    <div className="grid gap-3 sm:grid-cols-3">
                        ...
                        <label className="space-y-1.5 text-sm">
                            ...
                            <Input.Password value={channel.apiKey} placeholder={channel.hasApiKey ? "留空保持已保存密钥" : "请输入百炼 API Key"} onChange={(event) => updateChannel({ apiKey: event.target.value, enabled: true })} />
                        </label>
                    </div>
                    ...
                    <Tabs
                        ...
                        onChange={setTab}
                        items={[
                            { key: "system", label: `系统音色（${state?.system.length || 0}）` },
                            { key: "voices", label: `个人创建（${state?.voices.length || 0}）` },
                            { key: "audio", label: `音频管理（${state?.audio?.total || 0}）` },
                            { key: "logs", label: "请求日志" },
                        ]}
                    />
                    {tab === "system" ? (
                        <QwenVoiceTable voices={state?.system || []} emptyText="暂无官方音色目录" />
                    ) : tab === "voices" ? (
                        <QwenVoiceTable
                            voices={state?.voices || []}
                            emptyText="暂无阿里云百炼个人音色"
                            onEdit={(voice) => {
                                setEditingVoice(voice);
                                setVoiceCategory(voice.category || voice.feature || "其他");
                            }}
                        />
                    ) : tab === "audio" ? (
                        <AudioManagement state={state} onPageChange={(page) => void load(page)} />
                    ) : (
                        <Table
                            rowKey="id"
                            size="small"
                            dataSource={state?.logs.items || []}
                            pagination={{ current: state?.logs.page, pageSize: state?.logs.pageSize, total: state?.logs.total, showSizeChanger: false, onChange: (page) => void load(page) }}
                            ...
                            locale={{ emptyText: "暂无阿里云百炼请求日志" }}
                        />
                    )}
                </div>
            </Panel>
            <ModalLog log={selectedLog} onClose={() => setSelectedLog(null)} />
            <Modal open={Boolean(editingVoice)} title="编辑百炼音色分类" okText="保存" cancelText="取消" onCancel={() => setEditingVoice(null)} onOk={() => void saveVoiceCategory()}>
                <Select className="w-full" value={voiceCategory} options={MINIMAX_VOICE_CATEGORIES.map((item) => ({ value: item, label: item }))} onChange={setVoiceCategory} />
            </Modal>
        </div>
    );
}
        ...
        <Table
            ...
            columns={[
                { title: "生成时间", dataIndex: "createdAt", width: 170, render: (value) => new Date(value).toLocaleString() },
                {
                    ...
                    render: (value, record) => (
                        <div>
                            <div>{value || record.username || "已删除用户"}</div>
                ...
                { title: "音色", dataIndex: "voice", width: 160, render: (value) => value || "-" },
                {
                    ...
                    dataIndex: "status",
                    width: 90,
                    render: (value, record) => <Tag color={value === "success" ? "green" : value === "failed" ? "red" : "gold"}>{value === "success" ? "成功" : value === "failed" ? record.error || "失败" : "生成中"}</Tag>,
                },
        ...
        <div>
            {log ? (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
                    <div className="max-h-[80vh] w-full max-w-2xl overflow-auto rounded-xl bg-white p-5 dark:bg-zinc-900" onClick={(event) => event.stopPropagation()}>
                        ...
                        <div className="grid gap-3 text-sm sm:grid-cols-3">
                            <div>状态：{log.phase}</div>
                            <div>HTTP：{log.statusCode || "-"}</div>
                            <div>耗时：{log.durationMs} ms</div>
                        </div>
                        <div className="mt-4 space-y-2 text-sm">
                            {log.lifecycle.map((item, index) => (
                                <div key={`${item.at}-${index}`} className="border-b border-zinc-200 py-2 dark:border-zinc-800">
                                    <span className="mr-2 text-zinc-500">{new Date(item.at).toLocaleString()}</span>
                                    {item.message}
                                </div>
                            ))}
                        </div>
                        {log.error ? <pre className="mt-4 whitespace-pre-wrap break-all rounded-lg bg-red-50 p-3 text-xs text-red-700">{log.error}</pre> : null}
                    </div>
                </div>
            ) : null}
        </div>
