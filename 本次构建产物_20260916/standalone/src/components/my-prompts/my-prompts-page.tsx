...
export function MyPromptsPage() {
    ...
    return (
        <div className="prompt-library-page flex h-full flex-col overflow-hidden bg-background text-stone-800 dark:text-stone-100" data-my-prompts-ready={loading ? "loading" : "ready"}>
                ...
                <div className="mx-auto max-w-7xl space-y-3 sm:space-y-6">
                    ...
                    <section className="overflow-hidden rounded-lg border border-border bg-card">
                        ...
                        <Table
                            ...
                            rowKey="id"
                            loading={{ spinning: loading, indicator: <DreamyoWaitingIcon frame={2} size={30} label="正在加载我的提示词" /> }}
                            columns={columns}
                            dataSource={items}
                            tableLayout="fixed"
                            pagination={{ current: page, pageSize: PAGE_SIZE, total, hideOnSinglePage: true, showSizeChanger: false, onChange: setPage }}
                            locale={{ emptyText: <CompactEmptyState title="还没有保存提示词" description="添加一条提示词，之后可以复制使用或沉淀到我的素材。" className="border-0 bg-transparent shadow-none" /> }}
                        />
                    </section>
                </div>
            </main>
