...
OctalAICanvas 是面向图片、视频、短剧与品牌物料生产的 AI 创作工作台。当前仓库：`dreamwho/OctalAICanvas`。
...
- 应用镜像为 `ghcr.io/csyqlz/octalaicanvas`，文档镜像为 `ghcr.io/csyqlz/octalaicanvas-docs`。
- 环境变量统一使用 `OCTALAICANVAS_` 前缀；数据库默认名称与用户为 `octalaicanvas`。
