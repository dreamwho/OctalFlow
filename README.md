...
<p align="center">
  <a href="https://www.octalaicanvas.com">演示站</a> ·
...
OCTALAICANVAS_ENCRYPTION_KEY=replace-with-openssl-rand-hex-32
OCTALAICANVAS_INSTALL_TOKEN=replace-with-one-time-openssl-rand-hex-32
OCTALAICANVAS_MAINTENANCE_TOKEN=replace-with-another-openssl-rand-hex-32
OCTALAICANVAS_WORKER_TOKEN=replace-with-a-distinct-openssl-rand-hex-32
```
...
`OCTALAICANVAS_INSTALL_TOKEN` 只用于初始化数据库和创建首个管理员，必须从服务器 `.env` 粘贴到安装向导；安装完成后可从环境变量中移除。`OCTALAICANVAS_MAINTENANCE_TOKEN` 只授权外部计划维护任务，`OCTALAICANVAS_WORKER_TOKEN` 只授权 App 与生成 Worker 的内部任务领取、心跳和回调。Worker 不读取包含数据库、支付、安装令牌或外部维护令牌的完整 `.env`。完整变量说明见[配置说明](docs/content/docs/overview/configuration.mdx)。
...
- 外部存储关闭时新媒体只写 `OCTALAICANVAS_DATA_DIR`；开启时新媒体只写 S3 兼容对象存储。历史媒体按登记 Provider 读取。
