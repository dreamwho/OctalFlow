<p align="center">
  <img src="web/public/logo.svg?v=0.0.6" width="108" alt="dreamyo logo">
</p>

<h1 align="center">dreamyo</h1>

<p align="center">面向统一创作 Agent、Canvas 与短剧生产的开源 AI 创作平台</p>

<p align="center">
  <a href="https://github.com/dreamwho/dreamyo"><img src="https://img.shields.io/github/stars/dreamwho/dreamyo?style=flat-square&logo=github" alt="GitHub stars"></a>
...
<p align="center">
  <a href="https://www.dreamyo.com">演示站</a> ·
...
![dreamyo 首页](docs/public/screenshots/pages/01-home.webp)

dreamyo 把统一创作 Agent、画布、短剧生产、素材库和商业运营后台放在同一套 Next.js 全栈应用中。PostgreSQL 保存账号与业务数据；媒体可写入服务器本地目录或 S3 兼容对象存储；模型、支付和存储密钥只在服务端使用。
...
dreamyo 调用外部 AI 模型，不要求 GPU。服务器主要承担 Web、PostgreSQL、媒体下载/存储和可选 FFmpeg 转码。
...
```bash
git clone https://github.com/dreamwho/dreamyo.git
cd dreamyo
cp .env.example .env
```
...
```dotenv
NEXT_PUBLIC_SITE_URL=https://dreamyo.example.com
POSTGRES_PASSWORD=replace-with-a-strong-password
DREAMYO_ENCRYPTION_KEY=replace-with-openssl-rand-hex-32
DREAMYO_INSTALL_TOKEN=replace-with-one-time-openssl-rand-hex-32
DREAMYO_MAINTENANCE_TOKEN=replace-with-another-openssl-rand-hex-32
DREAMYO_WORKER_TOKEN=replace-with-a-distinct-openssl-rand-hex-32
```
...
`DREAMYO_INSTALL_TOKEN` 只用于初始化数据库和创建首个管理员，必须从服务器 `.env` 粘贴到安装向导；安装完成后可从环境变量中移除。`DREAMYO_MAINTENANCE_TOKEN` 只授权外部计划维护任务，`DREAMYO_WORKER_TOKEN` 只授权 App 与生成 Worker 的内部任务领取、心跳和回调。Worker 不读取包含数据库、支付、安装令牌或外部维护令牌的完整 `.env`。完整变量说明见[配置说明](docs/content/docs/overview/configuration.mdx)。
...
脚本默认生成复用服务器已有 Docker PostgreSQL 的 `本次修改需上传文件_YYYYMMDD`，其中包含主应用、GeminiAI sidecar 镜像归档和 `一键部署.sh`。上传整个目录到 Linux 服务器后执行 `./一键部署.sh` 即可自动加载镜像、创建应用持久化卷、生成内部密钥并启动服务；首次部署时需要在 `.env` 提供已有 PostgreSQL 的 `DATABASE_URL`。该包默认面向 `linux/amd64`；其他架构可通过 `DREAMYO_DOCKER_PLATFORM=linux/arm64` 重新构建。

没有已有 PostgreSQL 时，可构建自包含数据库包：

```bash
DREAMYO_DATABASE_MODE=embedded ./scripts/build-docker-offline-package.sh
```
...
DATABASE_URL=postgres://user:password@127.0.0.1:5432/dreamyo
...
`http://localhost:3000` 必须显示 dreamyo 主应用；如果看到“dreamyo 文档中心”，说明启动的是 `docs/` 子项目或旧版文档脚本，请停止该进程并从 `web/` 启动主应用。独立文档站只使用 `http://localhost:3001`。
...
- 外部存储关闭时新媒体只写 `DREAMYO_DATA_DIR`；开启时新媒体只写 S3 兼容对象存储。历史媒体按登记 Provider 读取。
...
<table>
  <tr>
    <td width="260"><a href="https://qm.qq.com/q/9MVLTxuRd6"><img src="docs/public/community/qq-dreamyo-group-1049777515.webp" width="240" alt="dreamyo 开源交流 QQ 群二维码"></a></td>
    <td>
      <strong>dreamyo 开源交流</strong><br>
