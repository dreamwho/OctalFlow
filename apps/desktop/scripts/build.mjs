...
import { validateMacReleaseEnvironment } from "./macos-release.mjs";
...
let macIdentity;
if (target === "mac" && process.env.DREAMYO_DESKTOP_UNSIGNED_TEST !== "1") {
    const identities = execFileSync("security", ["find-identity", "-v", "-p", "codesigning"], { encoding: "utf8" });
    macIdentity = validateMacReleaseEnvironment({ identities });
...
const child = spawn(process.execPath, [builder, `--${target}`, "--config", "electron-builder.config.mjs"], {
    ...
    env: { ...process.env, ...(macIdentity ? { DREAMYO_MAC_SIGN_IDENTITY: macIdentity, CSC_NAME: macIdentity } : {}), DREAMYO_DESKTOP_EDITION: edition.id },
    stdio: "inherit",
});
